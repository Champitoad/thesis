...
invertible are the \kl{weakening} rules $\{\rsf{w{-}},\rsf{w{+}}\}$, all the
...
\begin{description}
  \item[\textbf{Weakening}]
    ...
    neutral bubbles; thus we incorporate \kl{weakenings} in all rules that solve
    \kl{subgoals} by closing solutions with no neutral bubbles. Those are the rules
  ...
  \item[\textbf{Flow}]
    ...
    duplicated \kl{subgoal} $S$ cannot be weakened because it lives in a neutral
...
\kl{contraction} rules are admissible, in order to solve the aforementioned problem
...
immediately get our desired result: \kl{B_{inv}} is a fully invertible calculus,
...
\begin{description}
  ...
  \item[\textbf{Popping}] Pop every closed empty bubble in the \kl{goal} with the
