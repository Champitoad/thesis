...
\begin{figure*}
  ...
  \caption{Rules for the invertible bubble calculus \kl{B_{inv}}}
...
of \kl{DBiInt} in system $\sysB$, and get a complete, fully invertible
...
can identify which rules of \kl{system B} are invertible, and which are
...
\begin{description}
  ...
  \item[\textbf{Flow}]
    ...
    \kl{B_{inv}}. Fortunately, we showed that it is admissible in
  ...
  \item[\textbf{Implication/Exclusion}]
    ...
    introduction rules of \kl{DBiInt}. Also like in \sys{DBiInt}, we removed
    ...
    \kl{DBiInt}, we did not incorporate contractions in other
...
of simulating \kl{DBiInt}'s introduction rules:
...
introduction rules of \kl{DBiInt}, which always duplicate formulas. For now we
only conjecture completeness of \kl{B_{inv}}, since it not clear what method
...
of \kl{DBiInt} preserve the principal formula; but this is precisely what we
are trying to avoid with our version of the rules. Of course, if we either give
up on this constraint or include contraction rules in \kl{B_{inv}}, then we
immediately get our desired result: \kl{B_{inv}} is a fully invertible
calculus, where the same fragments as system $\sysB$ capture \kl{intuitionistic},
...
\begin{description}
  ...
  \item[\textbf{Linking}] Try to bring together every pair of dual atoms, so
    ...
    \rsf{i{\da}}. Thus the user can choose the subgoal to solve by
...
\begin{remark}
  By adopting $\mathbb{H}$-rules in the style of \kl{DBiInt}'s introduction
...
disjunctions). Thus \kl{B_{inv}} fails to be \emph{fully iconic}, because it
...
\begin{marginfigure}
  ...
  $$
  \caption{Left introduction rule for $\limp$ in \kl{JN}}
  \labfig{jn-rule-e}
\end{marginfigure}
...
introduction rule \rsf{e} (\reffig{jn-rule-e}). But we cannot do this in system
...
of nested sequents, capturing only \kl{intuitionistic} logic instead of
\kl{bi-intuitionistic} logic. But this is a small price to pay, since
\kl{bi-intuitionistic} logic does not (currently) have any applications in the realm
