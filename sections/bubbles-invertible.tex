...
\begin{description}
  \item[\textbf{Weakening}]
    ...
    \kl{subgoals} by closing \kl{solutions} with no \kl{neutral} bubbles. Those are the rules
    $\{\kl{i{\da}},\kl{p{-}},\kl{p{+}}\}$, which for the occasion have
    also been generalized to arbitrary \kl{solutions}. Indeed in \kl{system~B} we
    restricted them to open \kl{solutions} to make them \emph{local}, but here the
  ...
  \item[\textbf{Flow}]
    ...
    duplicated formula. Thus we can make all $\mathbb{F}$-rules of \kl{system
    B} \kl{invertible} by systematically duplicating the moved formula, although
    ...
    by encoding the path to the destination as a second \kl{solution} \kl{context} inside
  ...
  \item[\textbf{Implication/Exclusion}]
    ...
    rules} of \kl{DBiInt} in \kl{B_{inv}}.

\end{description}

\begin{remark}
  We also changed the \kl{\bot{-}} and \kl{\top{+}} rules, so that they create
  \kl{polarized}, closed empty \kl{solutions}. This makes them both local, and generic
  with respect to the status of the ambient \kl{solution}. The previous version can
  then be simulated by combination with the popping rules \kl{p{-}} and
  \kl{p{+}}.
\end{remark}
...
where the same fragments as \kl{system~B} capture \kl{intuitionistic},
...
\emph{phases}, applied successively in a loop until the empty closed \kl{solution} is
reached:
\begin{description}
  \item[\textbf{Decomposition}] Decompose $A$ by applying recursively
    ...
    when the formula is in an \emph{open} \kl{solution}. A first option is to let the
    system automatically distribute them in all open \kl{subsolutions} that are
  ...
  \item[\textbf{Linking}] Try to bring together every pair of dual atoms, so
    that they annihilate each other in an instance of the \kl{i{\da}}
    ...
    closing the \kl{subsolution} at the location where it ends with
    \kl{i{\da}}. Thus the user can choose the \kl{subgoal} to solve by
  ...
  \item[\textbf{Application}] When there are no more pairs of dual atoms, or all
    ...
    \Delta}{\sdepth{I}} = 0$ (\refdef{item-depth}), otherwise in some subgoal
    ...
    $\mathbb{F}$-rules \kl{f{-}{\da}} and \kl{f{-}{+}{\da}}.
    Ideally, she would also pick the most general $T\hole$ to factorize the
    proof, by minimizing its depth $\sdepth{T\hole}$ (\refdef{solctx-depth}).

    A second possibility is to duplicate eagerly every $\hypo{A \limp B}$ of
    $\imps{S}$ in every open \kl{subsolutions} of $S$ where it can be so, and then
...
\kl{solution} by deterministically applying a sequence of $\mathbb{H}$-rules (and
...
This can be understood as resulting from the inability of \kl{solutions} to represent
...
equivalent \kl{solutions}, which is really just the $\mathbb{H}$-rules of \kl{system~B} (\reffig{graphical-B}) where the right-hand \kl{solution} is enclosed in a
