...
\begin{description}
  \item[\textbf{Weakening}]
    ...
    \kl{subgoals} by \kl[saturated]{saturating} \kl{solutions} with no \kl{neutral} bubbles. Those are the rules
    $\{\kl{i{\da}},\kl{p{-}},\kl{p{+}}\}$, which for the occasion have
    also been generalized to arbitrary \kl{solutions}. Indeed in \kl{system~B} we
    restricted them to \kl{unsaturated} \kl{solutions} to make them \emph{local}, but here the
...
\begin{remark}
  ...
  \kl{polarized}, \kl{saturated} empty \kl{solutions}. This makes them both local, and generic
...
In the \kl{intuitionistic} (propositional) fragment of \kl{B_{inv}}, a canonical
...
\begin{description}
  \item[\textbf{Decomposition}] Decompose $A$ by applying recursively
    ...
    when the formula is in an \emph{\kl{unsaturated}} \kl{solution}. A first option is to let the
    system automatically distribute them in all \kl{unsaturated} \kl{subsolutions} that are
  ...
  \item[\textbf{Linking}] Try to bring together every pair of dual atoms, so
    ...
    \kl[saturated]{saturating} the \kl{subsolution} at the location where it ends with
  ...
  \item[\textbf{Popping}] Pop every \kl{saturated} empty \kl{bubble} in the \kl{goal} with the
...
\begin{itemize}
  ...
  create a \emph{partition} of the atoms of the \kl{goal} by en\kl[saturated]{saturating} them in
...
$\mathbb{H}$-rules of \kl{system~B} (\reffig{graphical-B}) where the right-hand
