...
\begin{definition}[Dual solution]\labdef{dual-solution}
  ...
  \end{align*}
  For solution \kl{contexts}, the \kl{hole} is self-dual: $\soldual{\hole} = \hole$. This
  entails in particular that $\soldual{S}\select{\soldual{T}} =
  \soldual{S\select{T}}$.
\end{definition}

Graphically, the dual of a solution $S$ is $S$ where the colors of \kl{items} have
...
\begin{definition}\labdef{item-depth}
  The \emph{depth} $\soldepth{I}$ of an \kl{item} $I$ is defined recursively as
  follows:
  \begin{align*}
    \soldepth{A} &= 0 \\
...
In order to ease reasoning by induction on solution \kl{contexts}, we give a
formulation equivalent to \refdef{solution-context} as a context-free grammar:
\begin{fact}
  Solution \kl{contexts} $S\hole$ are generated by the following grammar:
  $$
...
The \emph{depth} $\soldepth{S\hole}$ of a solution \kl{context} $S\hole$ is defined
recursively as follows:
\begin{align*}
  \soldepth{\hole} &= 0 \\
