...
\kl{exclusion}, by just taking pairs of a Heyting and a Brouwer algebra on the same
bounded lattice:
...
\begin{definition}
  The \emph{depth} $\sdepth{\J}$ of a \kl{branching operator} $\J$ is defined
...
$\kl{\sysBHB}$ of \kl{system~B} that are sound with respect to Heyting and
...
In order to get the last missing fragment $\kl{\sysBB}$ sound with respect to Brouwer
algebras, we need dual lemmas that are relative to the \kl{negative interpretation}
$\nsint{\cdot}$ instead of the \kl{positive interpretation} $\psint{\cdot}$, since
...
\begin{proof}
  ...
  happen because formulas cannot contain \kl{exclusions}. The other cases only depend
