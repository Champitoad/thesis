...
\newcommand{\anodes}{\mathbf{N}}
\newcommand{\agraphs}{\mathbf{G}}
...
\draw (#1.left corner) ..controls (#1.left corner) and ($(#1.left corner) + (0,0.55)$).. ($(#1.left corner) + (-1,+0.5)$);
\draw (#1.right corner) ..controls (#1.right corner) and ($(#1.right corner) - (0,0.55)$).. ($(#1.right corner) + (-1,-0.5)$);
\draw ($(#1.apex) - (0.1,0)$) -- ($(#1.apex) + (1,0)$);
}
\newcommand{\coterid}[2]{
