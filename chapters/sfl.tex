...
\kl{DnD} \kl{actions}, which were introduced informally through examples in
\refch{pba}. We do this mainly from the formal perspective of \kl{deep
inference} \kl{proof theory}, following the original work of K. Chaudhuri on
\emph{\kl{subformula linking}} \cite{Chaudhuri2013}. But we always keep in mind
the intended application to \kl{proof assistants}, by motivating various design
choices --- actual or prospective --- as ways to improve the \intro{user
experience} (\intro{UX}) of interactive proof building.

The chapter is organized as follows: \refsec{linkages} introduces the notions of
\kl(sfl){context} and \kl(sfl){polarity}, and explains how \kl{DnD} \kl{actions} are
specified by the user interactively through schemas called \emph{\kl{linkages}}.
\refsec{validity} explains how one can identify a subset of \kl{linkages} that
guarantees a \kl{productivity} property on \kl{DnD} \kl{actions}. \refsec{action}
describes the overall structure of how \kl{linkages} translate into logical
steps, and \refsec{invert} discusses some subtleties of this translation that
are related to the concept of \emph{\kl{focusing}} in automated proof search.
\refsec{soundness} shows that the logical steps are sound, and
\refsec{productivity} states and proves formally the \kl{productivity} property.
Finally, \refsec{dnd-completeness} shows how \kl{DnD} \kl{actions} can be turned into
a complete deductive system without any need for click \kl{actions}.

\section{Linkages}\labsec{linkages}

Like most \kl{rewriting systems} on terms (that is, tree-shaped data), the
rewriting rules of \reffig{DISL} and \reffig{DISL-U} apply at any depth inside
formulas. However logically, the shape of the \emph{\kl{context}} in which this
rewriting occurs can provide important information, either to ensure soundness
of the performed transformation (\refsec{soundness}, \refsec{dnd-completeness}),
or to understand the status of quantified variables (\refsubsec{identity}).

% As is standard in term rewriting and \kl{deep inference} (see
% \refsubsec{structural-proof-theory}), our notion of \kl{context} will correspond
% to that of a (formula) tree with a distinguished leaf called its \kl{hole}.

\begin{scope}\knowledgeimport{sfl}

\begin{definition}[Context]\labdef{formula-context}

  A \intro{context}, written $A\hole$, is a proposition containing exactly one
  occurrence of a special formula written $\hole$, called its \reintro{hole}.
  ...
  is not a \kl{substitution} because it allows variable capture. For instance
  $\forall x.\select{P(x)}$ is the proposition $\forall x.P(x)$.
\end{definition}

\begin{definition}[Path]\labdef{path}
A \intro{path} is a proposition where one subformula has been
selected. Formally, a \kl{path} is a pair $(A\hole, B)$ formed by one
\kl{context} and one proposition:
\begin{itemize}
\item $A\hole$ is called the {\em context} of the \kl{path},
\item $B$ is called the {\em selection} of the \kl{path}.
\end{itemize}

The \kl{path} $(A\hole, B)$ can be viewed as the proposition
$A\select{B}$.  For readability, we will generally also write
$A\select{B}$ for the \kl{path} $(A\hole, B)$.
\end{definition}

\begin{definition}[Inversions]
  Given a \kl{context} $A\hole$, the number of inversions in $A\hole$,
  written $\inv(A\hole)$, is the number of subterms of $A\hole$
  which are of the form $C\hole\limp D$; that is the number of
  times the \kl{hole} is on the left-hand side of an implication.
  For instance:
  $$
  \begin{array}{rcl}
      \inv(D\land \hole)&=& 0\\
...
\begin{definition}[Polarity]\labdef{polarity}
\phantomintro{polarity}
We will write $A^+\hole$ to specify that a \kl{context} is \intro{positive},
meaning that $\inv(A^+\hole)$ is even. Symmetrically, $A^-\hole$ will be used
for \intro{negative} \kl{contexts}, meaning that $\inv(A^-\hole)$ is odd.
\end{definition}

% In demonstrating soundness, we focused solely on the two \emph{items} involved
% in a \kl{DnD} \kl{action}. But

\AP
In addition to the \kl{items} involved, every \kl{DnD} \kl{action} specifies the
\emph{selection} of a subterm in each \kl{item}, which can be expressed formally as a
\kl{path}. We call \intro{linkage} the combined data of the two \kl{items} together
with the selection, since the intent is to \emph{link} the subterms to make them
interact in some way.

\begin{remark}
In this thesis we only consider \kl{linkages} between two subterms. But as noted
in \refsec{equality}, rewriting equalities is an example of \kl{action} that can
benefit from allowing multiple selections\sidenote{A restricted kind of
multi-occurrence rewrite is already available in the standalone version of
\kl{Actema}: one needs to enter \emph{selection mode}, by either toggling the
dedicated button, or holding down the \texttt{shift} key. Then one can click
successively on all occurrences of a term $t$ that are to be rewritten, in order
to add them to the selection. To perform the rewriting to some other term $u$,
the last step is to drag an equality hypothesis $t = u$ (or $u = t$) and drop it
on any \kl{item} holding one of the selected occurrences of $t$.}.
\end{remark}

Each kind of \kl{DnD} \kl{action} is mapped in the system to a specific form of \kl{linkage},
which is designed to hold all the information necessary for the correct
execution of the \kl{action}. In this way the system can automatically search for
\kl{linkages} of a certain form, and propose to the user all well-defined \kl{actions}
associated to these \kl{linkages}.

\begin{remark}
  In the future, one can imagine several \kl{DnD} \kl{actions} associated to a given
  \kl{linkage}. In this case, the user could be queried to choose the \kl{action} to be
  performed (typically with a pop-up menu). However with the \kl{actions} considered
  in this thesis, such ambiguities never arise.
\end{remark}

On the ``\kl{items} axis'', we already distinguished between \kl(dnd){backward} and
\kl(dnd){forward} \kl{linkages}, written respectively $A \back B$ and $A \forw
B$. If the \kl{items} types are unspecified, we will write $A \link B$.

Using the ``selection axis'', we can specify a further distinction that was
informal up to now: that of \emph{logical} \kl{action} and \emph{rewrite} \kl{action}.
\begin{itemize}
  \itemAP \intro{Logical linkages} link two subformulas: they have the form
  $B\select{A} \link C\select{A'}$.
  \itemAP \intro{Rewrite linkages} link one side of an equality with a \kl{first-order}
  term. Using liberally the notations from \refdef{formula-context} and
  \refdef{path}, they thus have the form
  $$B\select{\select{t} = u} \link C
  \select{t'} ~~\left(\text{or symmetrically $B\select{u = \select{t}} \link C
  \select{t'}$}\right)$$
  % \item \emph{Instanciation} \kl{actions} correspond to specialization of universal
...
\kl{linkage} as a formula whose topmost connective is an \kl{interaction
operator} $\link \in \{\back,\forw\}$. Then it is natural to view \kl{linkages}
as the redexes of the \kl{rewriting rules} of \reffig{DISL}, although from the
user's standpoint \kl{linkages} only happen at the top-level\sidenote{In fact
this is more of a limitation of Actema's current interface: one cannot link two
subterms that live in the same \kl{item}, because dragging \kl{actions} can only be
performed on \emph{entire} \kl{items}. But in the original formulation and
implementation of \kl{subformula linking} \cite{Chaudhuri2013}, \kl{linkages}
can be created between arbitrary subformulas. We come back to this issue in
\refsec{dnd-completeness}.}.
...
\begin{mathpar}
  \begin{array}{r@{\quad}c@{\quad}l@{\quad}r}
    {A \back B}&\step{}&A \limp B &\intro{Brel}\\
    {A \forw B}&\step{}&A \land B &\intro{Frel}
  \end{array}
\end{mathpar}
...
In the original formulation of \kl{subformula linking} \sidecite{Chaudhuri2013}, a
semantics is associated to every \kl{logical linkage}, even when the selected
subformulas $A$ and $A'$ are not \kl{unifiable}. This is made possible by the
addition of so-called \emph{release} rules\sidenote{A terminology coming from
the line of works on \emph{\kl{focusing}} in \kl{proof theory} \cite{andreoli1992}, see
also \refsec{invert}.}, which simply turn \kl{interaction operators} into their
...
instead we define a \emph{\kl{validity} criterion} on \kl{linkages}, which guarantees that
...
\begin{itemize}
  ...
  \emph{\kl{polarities}}, so that the \emph{\kl{negative}} subterm justifies the
  \emph{\kl{positive}} one;
  \item \textbf{Identity:} the selected subterms must be \emph{\kl{unifiable}},
  so that after instantiating some quantifiers in their \kl{context} they can
  interact through the {\kl(sfl){id}} rule or the equality rules
  {\kl[L{=}_1]{L{=}_i}, \kl[F{=}_1]{F{=}_i}}.
\end{itemize}
\AP
One benefit of using this criterion is that it filters out all \kl{linkages}
whose semantics rely on release rules, capturing intuitively a notion of
\intro{productivity}: instead of just moving around subformulas, we know for
sure that some ``simplification'' occurs, either a justification with the
{\kl(sfl){id}} rule on \kl{logical linkages}, or a rewriting with the equality
rules on \kl{rewrite linkages}. This will be stated more formally in
\refsec{productivity}.

\kl{Validity} is very useful to support the \emph{suggestion} mechanism
implemented in \kl{Actema}. The idea is that when the user starts dragging an
\kl{item}, this indicates to the system that she wants to perform a \kl{DnD}
\kl{action} involving subterms of this \kl{item}. Then the system can suggest such
possible \kl{actions} by highlighting subterms in the \kl{goal} which form a
\kl{valid} \kl{linkage} with the dragged \kl{item}. Typically in the example of
\reffig{aristote}, dragging the hypothesis $\forall x. \human(x) \limp
...
targets. In this case this corresponds to all subterms in the \kl{goal} which
are not contained in the dragged \kl{item}. But if one were to drag the
...
the dots'' with information already at hand. Our \kl{DnD} \kl{actions} capture this
\kl{metaphor} quite directly, and thus shall be especially useful to beginners
unfamiliar with proving, who often show difficulties in understanding how to
build a proof from scratch. More generally, \kl{proof assistants} have the
...
main goal of this work, and the notion of \kl{valid} \kl{linkage} seems to be a
good candidate as a core principle for such a methodology.

\subsection{Polarity}\labsubsec{polarity}

The restrictions on \kl{polarities} are captured formally by the following condition:


\begin{condition}[Polarity]\label{cond:pol}
  
  The following must be true for a \kl{logical linkage}
  $B\select{A}\link D\select{A'}$ to be \kl{valid}:
...
equality is in \kl{negative} position. For \kl{logical linkages}, Clause
\ref{clause:opposite} ensures that the selected subformulas have opposite
\kl{polarities}, and Clause \ref{clause:intuit} ensures that the \kl{linkage}
makes sense in our \emph{\kl{intuitionistic}} setting. Indeed one could imagine
the following behavior in \kl{classical} logic:
...
back to this example in \refch{sfl-classical}, but for now we can just remark
that there is no way to handle it with the rules of \reffig{DISL} because we
lack a rule for redexes of the form $B\select{A} \limp C \back D\select{A'}$.

\subsection{Identity}\labsubsec{identity}

A \kl{context} binds variables in the selected proposition. These variables
will be \kl{unifiable} or not depending upon: (1) the nature of the quantifier
...
variables bound by a \kl{context} in two parts.


\begin{definition}[Positive and negative variables]
Given a \kl{context} $A\hole$ seen as a tree, one can always start from the root
and traverse the branch of $A\hole$ that leads to its \kl{hole} $\hole$. We
...
\lvarp(A\hole)$ (resp. $x \in \lvarn(A\hole))$ iff there are \kl{contexts}
...
\begin{definition}[Unifiable variables]\labdef{uvars} The set
  $\intro*\uvars(\mathcal{L})$ of \intro{unifiable variables} of a \kl{linkage}
...
The following notions of \kl{substitution} and \kl{unification} are the usual
ones and we do not go into details:
 
\begin{definition}[Substitution]
  A \intro{substitution} is a mapping from variables to terms such that $\{x ~|~
  \sigma(x) \not\syneq x \}$ is finite; we call this set the {\em domain} of
  $\sigma$.
  When $\sigma(x)\syneq x$ we say that $x$ is  {\em not instantiated} by
  $\sigma$.
  Given a proposition $A$ and a \kl{substitution} $\sigma$, we write
...
\begin{definition}[Unification]
  ...
  \kl{substitution} $\sigma$ \intro{unifies} $A$ and $A'$ over $l$ when
  $\sigma(A)\syneq\sigma(A')$ and the domain of $\sigma$ is a subset of $l$. 

  If such a \kl{substitution} exists, we say that $A$ and $A'$ are
  \reintro{unifiable} over $l$.
\end{definition}
Given $A$, $A'$ and $l$, the well-known \kl{unification} algorithm decides
whether $A$ and $A'$ are \kl{unifiable} over $l$ and constructs the
\kl{substitution} when it exists~\sidecite{Montanari}.

\begin{condition}[Identity]\label{cond:unif}
  For a \kl{linkage} $\mathcal{L} \deq B\select{A}\link C\select{A'}$ to be
  \kl{valid}, the following must be true:
  \begin{enumerate}
   \item There exists a \kl{substitution} $\sigma$ which \kl{unifies} $A$ and
     $A'$ over $\uvars(\mathcal{L})$.\label{clause:unif}

   \item Furthermore, the \kl{unification} respects the order
     ...
     given a \kl{unifiable variable} $x$ in the domain of $\sigma$, all
...
The last condition ensures acyclicity and will prohibit \kl{invalid}
...
Finally we can state the full \kl{validity} criterion for \kl{linkages}:

\begin{definition}[Valid linkage]\labdef{valid-linkage}
  We say that a \kl{linkage} $\mathcal{L}$ is \intro{valid} if it satisfies Conditions
  \ref{cond:pol} and \ref{cond:unif}.
\end{definition}

One can check that all the examples given up to here were based on \kl{valid}
\kl{linkages}.
...
\item It is also terminating, because the number of connectives or quantifiers
  under $\forw$ or $\back$ decreases\sidenote{Except for the \kl{Fcomm} rule
...
proof step:
\begin{enumerate}
\item \textbf{Selection:} the user selects two subterms in two \kl{items} of the current \kl{goal}; \label{step:selection}
\item \textbf{Linkage:} this either gives rise to a \kl{logical linkage} $B\select{A}
  ...
  the procedure stops here, and the system does not propose any \kl{action} to the
  user; \label{step:linkage}
\item \textbf{Validity:} the system verifies that the \kl{linkage} is \emph{\kl{valid}},
  ...
  \begin{enumerate}
    ...
    and $t'$) must be \kl{unifiable}, yielding a \kl{substitution} $\sigma$;
    \item \textbf{Dependencies:} the \kl{substitution} $\sigma$ must satisfy
...
\item \textbf{Unit elimination:} in the case of a logical \kl{action}, this creates an occurrence of $\top$,
...
\item \textbf{Goal modification:} the two previous steps produced a formula $E$.
  In the case of a \kl(dnd){forward} \kl{linkage}, a hypothesis $E$ is added to the \kl{goal}; in
  the case of a \kl(dnd){backward} \kl{linkage}, the \kl{goal}'s conclusion becomes $E$. In both
  cases, the logical soundness is guaranteed by
  \refthm{sfl-soundness}. \label{step:goal-modification}
\end{enumerate}
...
All examples up to now followed the scheme for \kl{DnD} \kl{actions} sketched in
\refsec{aristote}:
\begin{itemize}
  \item Given a blue \kl{item} $A$ and a red \kl{item} $B$, \kl(dnd){backward} proof steps produce a
  new conclusion $C$ by applying a sequence of \kl{rewriting rules} $A \back B \steps{}
  C$.
  \item Given two blue \kl{items} $A$ and $B$, \kl(dnd){forward} proof steps produce a new
  hypothesis $C$ by applying a sequence of \kl{rewriting rules} $A \forw B \steps{} C$.
\end{itemize}

Thus for such \kl{actions} to be logically sound, we have to make sure that our
\kl{rewriting system} satisfies the following property:

\begin{theorem}[Soundness]\labthm{sfl-soundness}
  ~\\\vspace{-1.5em}
  \begin{itemize}
...
We will need to reason inductively on \kl{contexts}, and more precisely on the
\emph{depth} of their \kl{hole}:

\begin{definition}[Depth]\labdef{sfl-ctx-depth}
  The \intro{depth} $\intro*\cdepth{A\hole}$ of a \kl{context} $A\hole$ is
...
\begin{lemma}[Variance]\lablemma{variance}
...
to show that the rule satisfies \refthm{sfl-soundness} locally. Formally,
we can define a mapping from formulas containing \kl{interaction operators} to usual
formulas where they have been replaced by their interpretation:

\begin{definition}[Interpretation of interaction operators]
...
consider the \kl{polarity} of their \kl{context}.

% Using the notation of definition~\refdef{polarity}, we can state:

\begin{lemma}[Local soundness]\lablemma{rules-valid-in-context}
  \phantom{a}
  \begin{itemize}
...
\begin{proof}
  ...
  \kl{interaction operator}. Thus if $C\select{A \link B} \step{} D$, the only
  possible redex is $A \link B$.}. Then by careful analysis of each rule, it is
  ...
  case by applying \reflemma{variance}.
\end{proof}
\begin{remark}
  For some rules, like \kl{R{\limp}_1}, the left-hand and
  right-hand propositions are equivalent:
  $$A \limp B \limp C ~~~\Longleftrightarrow~~~ A \land B \limp C$$
  These rules are thus {\em \kl{invertible}} and their names are tagged by *.
  This point will be relevant in \refsec{invert}.
\end{remark}


An easy but important technical point is that \kl{rewriting rules} preserve the
\kl{polarity} of \kl{contexts} around redexes, in the following precise sense:
\begin{fact}[Polarity preservation]\labfact{rules-preserve-polarity}
  \phantom{a}
  \begin{itemize}
    \item
      If $C\select{A\back B} \step{} C'\select{A'\back B'}$
      (resp. $C\select{A \forw B} \step{} $ $ C' \allowbreak\select{A' \forw B'}$) then
      $C\hole$ and $C'\hole$ have the same \kl{polarity}.
    \item
      If $C\select{A\back B} \step{} C'\select{A'\forw B'}$ (resp.
      $C\select{A\forw B} \step{} C'\select{A'\back B'}$) then $C\hole$ and
      $C'\hole$ have opposite \kl{polarities}.
  \end{itemize}
\end{fact}

Combining \reflemma{rules-valid-in-context} and
\reffact{rules-preserve-polarity}, we obtain the central soundness result about
the \kl{rewriting rules}:
\begin{lemma}[Contextual soundness]\lablemma{rewriting-valid-in-context}
  \phantom{a}
  \begin{itemize}
...
\begin{proof}
  ...
  Then we reason by case on the \kl{interaction operator} $\link$:
  \begin{itemize}
    \item $\link = {\back}$: by \reffact{rules-preserve-polarity}, $C'$ must
    be \kl{positive}. Therefore by induction hypothesis $\lint{D} \seq C'\select{A'
    \limp B'}$. By \reflemma{rules-valid-in-context} we have
    $C'\select{A' \limp B'} \seq C^+\select{A \limp B}$. Thus by transitivity
    $\lint{D} \seq C^+\select{A \limp B}$.
    \item $\link = {\forw}$: by \reffact{rules-preserve-polarity}, $C'$ must
    be \kl{negative}. Therefore by induction hypothesis $\lint{D} \seq C'\select{A'
    \land B'}$. By \reflemma{rules-valid-in-context} we have
    $C'\select{A' \land B'} \seq C^+\select{A \limp B}$. Thus by transitivity
    $\lint{D} \seq C^+\select{A \limp B}$.
  \end{itemize}
\end{proof}

Finally, soundness (\refthm{sfl-soundness}) is obtained as the special
case where the rewriting starts in the (\kl{positive}) empty \kl{context}.
...
% drag-and-drop \kl{actions} involving two subterms in a sequent, by following a set of
...
%% \end{lemma}
%% \begin{proof}
%%   By straightforward inspection of all rules except \kl(sfl){id}.
%% \end{proof}
...
\begin{lemma}[Valid Progress]\label{thm:vprogress} If a \kl{linkage} $\mathcal{L} \syneq
  ...
  \begin{enumerate}
    ...
    \item or $\mathcal{L} \step{} E\select{\mathcal{L'}}$ for some $E\hole,
      \mathcal{L'}$ with $\mathcal{L'}$ \kl{valid}.
      % and
...
\begin{itemize}
\item The rules involving a connective always preserve \kl{validity}.
\item When one can apply a rule involving a quantifier $\forall x$ (resp.
  $\exists x$), one checks whether the \kl{substitution} produced by unification
  (Condition \ref{cond:unif}) instantiates $x$ or not. In the first case one
  performs the instantiation rule \kl{L\forall i} or \kl{F\forall i} (resp.
  \kl{R\exists i}); in the second case the corresponding \kl{switch rule} in
  {\small $\{\kl{L\forall s}, \kl{R\forall s}, \kl{F\forall s}\}$} (resp.
  {\small $\{\kl{L\exists s}, \kl{R\exists s}, \kl{F\exists s}\}$}).
\end{itemize}
\begin{proof}
  Let $\mathcal{L} \syneq B\select{A} \link C\select{A'}$ be a \kl{valid} \kl{linkage}.\\
  \begin{enumerate}[itemsep=0.8em]
    ...
    \kl{substitution}, which entails that $A \syneq A'$. Therefore we are in the
    ...
    we show that we can always apply a \kl{rewriting rule} that produces a new, \kl{valid}
    \kl{linkage} $\mathcal{L'} \syneq B' \link' C'$.
    
    Let $\sigma$ and $\lvar$ be respectively the \kl{substitution} and interleaving
    ...
    \begin{itemize}
      \item If $x$ is quantified at the head of either $B\square$ or $C\square$,
        ...
        \begin{description}
          \item[Switch rule (\kl{L\forall s}, \kl{L\exists s},
          \kl{R\forall s}, \kl{R\exists s}, \kl{F\forall s},
          \kl{F\exists s})] Only if $x$ is not in the domain of $\sigma$. In
          \kl(dnd){forward} mode and when $B\square$ binds $x$, one must first apply the
          rule \kl{Fcomm} to put $B\select{A}$ on the right of $\forw$, so
          ...
          \begin{enumerate}[itemsep=0.4em]
            ...
            untouched by the rule, we can choose $\sigma$ as a \kl{valid} unifier
            that ranges over $\uvars(\mathcal{L'})$.
            
            \item In all \kl{switch rules}, we have $\lvar(\mathcal{L'}) = \lvar'$
            because the quantifier of $x$ is moved in the outer \kl{context} of the
            \kl{linkage}. Thus we can just take $\lvar'$ as interleaving, and
            Condition \ref{cond:unif} will still be verified because $\lvar'$ is
            a sublist of $\lvar$.
          \end{enumerate}

          \item[Instantiation rule (\kl{L\forall i}, \kl{R\exists i},
          \kl{F\forall i})] Only if $x$ is instantiated by $\sigma$, using
          $\sigma(x)$ as witness. Again one might need to apply \kl{Fcomm}
          first. Then we check the \kl{validity} of $\mathcal{L'}$:
      ...
      the direct subcontext where $A$ or $A'$ occurs, or the {\kl{Fcomm}}
      rule. Again we check the conditions of \refdef{valid-linkage}:
      ...
      \begin{enumerate}[itemsep=0.4em]
        ...
        exceptions are \kl{R{\limp}_1} and \kl{F{\limp}_1}, which
        decrease the number of inversions of the right \kl{context} $C\hole$ by $1$.
        But since they are also the only rules that change the \kl{interaction operator},
        ...
        \begin{itemize}
          ...
          {\kl{R{\limp}_1}} rule and $\mathcal{L'}$ is \kl(dnd){forward}, and thus
...
Then we can state the following \emph{\kl{productivity} theorem}, which is a direct
consequence of the previous lemma and the fact that the \kl{rewriting rules}
terminate:

\begin{theorem}[Productivity]\labthm{productivity}
If $\mathcal{L}$ is a \kl{valid} \kl{linkage}, then there
...
This is the formal counterpart to the notion of \kl{productivity} mentioned in
...
\kl(dnd){backward} \kl{DnD}, or the assumptions are \emph{strictly} strengthened
after a \kl(dnd){forward} \kl{DnD}, instead of having just an equivalent
\kl{goal} written in a different way\sidenote{This remark only applies to
\kl{logical linkages} however, since rewriting equalities can only produce
equivalent statements. Some \kl{proof assistants} provide facilities to rewrite
arbitrary relations in subterms of arbitrary depth, such as \kl{Coq} with its
\emph{generalized rewriting} mechanism \cite{coqman-genrew}. This includes
non-symmetric relations that can produce non-equivalent statements, and there is
no reason in principle it could not be integrated in our paradigm, in the form
of generalized substitution rules in place of {\kl[L{=}_1]{L{=}_i}} and
{\kl[F{=}_1]{F{=}_i}}.}. This again contrasts with the release semantics of
\kl{subformula linking} which do not provide this guarantee of
\kl{productivity}, or with the logical reasoning \kl{tactics} of \kl{proof
assistants} based on \kl{natural deduction} rules.

\section{Focusing}\labsec{invert}

% \sidenote{There are 7 invertible rules, and 23 non-invertible
%     rules to consider in \reffig{DISL} (we exclude the {\kl(sfl){id}} and
%     {\kl{Fcomm}} rules).}
...
\kl{L\lor_1} or \kl{R\lor_2}.

\AP
Interestingly, an answer is provided by \intro{focusing}. It has been noticed by
Andreoli~\sidecite{andreoli1992} that, in bottom-up proof search, one should
apply the \kl{invertible} \kl{inference rules} first since they preserve provability.
In our framework, this translates into first applying the \kl{invertible}
\kl{rewriting rules} (the ones marked by a *).
...
In the case of the example above, this means performing \kl{L\lor_1} first,
which leads to the following behavior:
$$\select{A}\lor B \back B\lor\select{A}\steps{} B\limp B\lor A.$$
This is indeed the ``right'' choice, since applying \kl{R\lor_2} first would
...
\begin{enumerate}
  ...
  \item If only one \kl{context} is non-empty, say $C\hole$, we look at its head
  connective as well as the side where its \kl{hole} resides:
  \begin{itemize}
  ...
  {\rsf{L\mcirc_i}}.
  \item If both \kl{contexts} are non-empty, then the previous logic determines one
...
the third case:
\begin{enumerate}
  \item if both are \kl{invertible}, then the order of application does not matter
  since we preserve provability in the end;
  \item if only one is \kl{invertible}, we apply it first following the \kl{focusing}
  discipline;
  \item if neither are \kl{invertible}, we want to choose the order that
  ...
  \kl[F\lor_1]{F\lor_i}/\kl{F{\limp}_2} for $i \in \{1,2\}$, as was noted
  independently in \sidecite{DBLP:conf/cade/Chaudhuri21}. In this case, one
  ...
  \kl{DnD} \kl{action}, that is distinguishing between the source \kl{path} and the
  destination \kl{path}\sidenote{Note that in the current implementation of
  \kl{Actema}, we instead rely on an arbitrary prioritizing fixed in the system,
  which can hinder in some cases the ability to prove a \kl{goal} through
  \kl{DnD} \kl{actions}. In practice, one rarely encounters such cases in real
  examples.}.
\end{enumerate}

\begin{remark}\labremark{permutability}
...
\kl{subformula linking} system almost identical to ours.
\end{remark}
...
\kl{actions} exposed to the user is \emph{complete} with respect to provability. That
is, any formula $A$ which is \emph{true} in our logic --- here
\kl{intuitionistic} \kl{FOL} --- can be proved by executing a sequence of
graphical \kl{actions} that reduces it to the empty \kl{goal}. We noticed in
\refremark{click-completeness} that click \kl{actions} are a sufficient basis for
completeness. While we believe that a combination of both click and \kl{DnD}
\kl{actions} is more comfortable to handle a variety of proof situations, it is still
interesting to consider the question of completeness for \kl{DnD} \kl{actions} alone.
It turns out that the answer is positive: the mechanism of \emph{\kl{subformula
linking}} underlying \kl{DnD} \kl{actions} is powerful enough to capture provability
in \kl{FOL}. This has already been shown by Chaudhuri in \cite{Chaudhuri2013}
for linear logic, and \cite{DBLP:conf/cade/Chaudhuri21} for \kl{intuitionistic}
logic. Here we give a completeness proof for a system based on a slight
extension of our \kl{rewriting rules}, following ideas from these works.
...
\begin{remark}
  ...
  \kl{subformula linking} calculus, but this derivation might not be
  constructible deterministically by the focusing procedure outlined in
  \refsec{invert}. There are two aspects that make the stronger version hard to
  prove in practice:
  \begin{itemize}
    \item To show that the choices performed by the \kl{focusing} procedure
    always allow to find a proof when there exists one, it would be necessary to
    formulate and prove a \emph{\kl{focusing} theorem} based on the
    permutability of rules mentioned in \refremark{permutability}.
    \item Even then, some additional rules of our \kl{subformula linking} system
    are not simulated in any way by the \kl{DnD} procedure of \refsec{action}.
  ...
  % implementation of subformula linking in \kl{Coq} as presented in \refch{plugin},
...
\begin{marginfigure}
  \begin{mathpar}
    \begin{array}{r@{\quad}c@{\quad}lr}
        {C^+\select{A \limp B}}&\step{}&{C^+\select{A \back B}} &\intro{B}\\
        {C^-\select{A \land B}}&\step{}&{C^-\select{A \forw B}} &\intro{F}\\
    \end{array}
  \end{mathpar}
...
To the \kl{rewriting rules} of \reffig{DISL} and \reffig{DISL-U}, we add
\emph{\kl{linkage} formation} rules (\reffig{newlink}), which are a deep
generalization of \kl{linkage} creation between two formulas of a sequent. Rule
{\kl{B}} creates a \emph{\kl(dnd){backward}} \kl{linkage} between $\limp$-linked
formulas in any \kl{positive} \kl{context} $C^+\hole$, and dually rule {\kl{F}}
creates a \emph{\kl(dnd){forward}} \kl{linkage} between $\land$-linked formulas
in any \kl{negative} \kl{context} $C^-\hole$\sidenote{This is reminiscent of the
adjunction between products and exponentials in \emph{cartesian closed
categories}, which respectively interpret conjunction and implication in the
\kl[Curry-Howard correspondence]{Curry-Howard-Lambek correspondance} for
\kl{intuitionistic} logic.}. Note that \kl{linkage} formation rules are not
closed under arbitrary \kl{contexts}: indeed the \kl{polarity} restrictions are
necessary to ensure soundness, as reviewed in \refsec{soundness}. A
\kl(dnd){backward} \kl{linkage} in a \kl{sequent} $\Gamma, D\select{A} \seq
E\select{B}$ would be encoded by the instance
$$C^+\select{D\select{A} \limp E\select{B}} \step{} C^+\select{D\select{A} \back
E\select{B}}$$ of {\kl{B}} where $C^+\hole \syneq \bigwedge Γ \limp \hole$, while
a \kl(dnd){forward} \kl{linkage} in a \kl{sequent} $\Gamma, D\select{A}, E\select{B} \seq F$ would
be encoded by the instance $$C^-\select{D\select{A} \land E\select{B}} \step{}
C^-\select{D\select{A} \forw E\select{B}}$$ of {\kl{F}} where $C^-\hole \syneq
\bigwedge \Gamma \wedge \hole \limp F$.

\begin{marginfigure}
  \begin{mathpar}
    \begin{array}{r@{\quad}c@{\quad}lr}
        {C^-\select{A}}&\step{}&{C^-\select{A \land A}} &\intro{conn}\\
        {C^-\select{A}}&\step{}&{C^-\select{\top}} &\intro{weak}\\
    \end{array}
  \end{mathpar}
...
{\kl{Fcomm}} rule to handle commutativity of the $\forw$ operator, which acts
as a kind of \kl{exchange} rule. Then we add the equivalent of \kl{contraction}
and \kl{weakening} with the rules {\kl{conn}} and {\kl{weak}}
(\reffig{reslink}). These allow to erase and duplicate hypotheses at will, by
identifying any subformula occurring in a \kl{negative} \kl{context} as a hypothesis. Thus
once again we need to be careful about \kl{polarity}, and cannot close these rules
under arbitrary \kl{contexts}.

\begin{marginfigure}
  \begin{mathpar}
    \begin{array}{r@{\,}c@{\,}lr}
        {C^+\select{A \limp B}}&\step{}&{C^+\select{A \limp (A \back B)}} &\intro{Bconn}\\
        {C^-\select{A \land B}}&\step{}&{C^-\select{A \land (A \forw B) \land B}} &\intro{Fconn}\\
    \end{array}
  \end{mathpar}
...
An alternative to the full \kl{contraction} rule {\kl{conn}} is to systematically
duplicate \kl{negative} formulas in the \kl{linkage} formation rules, giving the rules
{\kl{Bconn} and \kl{Fconn}} of \reffig{newreslink}. This models more
closely what we do in \kl{Actema}, where hypotheses involved in a \kl{DnD} \kl{action} are
always preserved in the new \kl{goal}. This is important from a usability standpoint,
because this ensures the user needs not fret with manual duplication of
hypotheses in order to complete a proof. The downside is that the \kl{context} always
grows bigger, but this can be balanced by exposing the \kl{weakening} rule in the
interface. In \kl{Actema} it is mapped to a ``delete'' button placed next to every
...
Because \kl{linkages} created by rules {\kl{B} and \kl{F}} are not
necessarily \kl{valid}, one needs to add the so-called \emph{release} rules already
...
we do not use them in the semantics of \kl{DnD} \kl{actions}. We conjecture that it should
...
Lastly, a trivial but necessary addition is the rule {\kl{refl}} of
\reffig{refl-rule} stating the reflexivity of $=$. It was not introduced before
because it is already handled by click \kl{actions} on red \kl{items} in \kl{Actema}
(\refsec{clicks}), but here we want a self-contained system that models as
closely as possible the space of proofs that can be built through \kl{DnD} \kl{actions}
only. In this context, one could imagine restricting the usage of the
{\kl{refl}} rule to the unit elimination phase (\refsec{action}), where it
would play the same role as the rules of \reffig{DISL-U}. Thus adding this rule
does not correspond morally to modelling a click \kl{action}, nor to a modification
of the semantics of \kl{DnD} \kl{actions} as for release rules.
...
choice mostly does not matter: all we need is that it is \emph{\kl{analytic}}, i.e.
satisifies the \kl{subformula property}. Indeed the very idea of \kl{subformula linking}
is based on \kl{analyticity}: one should be able to prove a statement by the sole act
...
\begin{mathpar}
  \R[\intro{Ref}]
    ...
    {\Gamma \seq C}
  \\
  \top \quad\step{} \quad t = t \quad \intro{ref}
\end{mathpar}
\caption{Non-\kl{analytic} reflexivity rules}
\labfig{ref-rule}
\end{marginfigure}

\AP
We chose the calculus \intro{G3i} from \cite{negri_structural_2001} as our
basis, because all \kl{structural rules} are \kl{admissible} in it (but they
would be straightforward to simulate apart from the \kl(rule){cut} rule). The
first modification we do is that we model hypotheses in \kl{sequents} as lists
instead of multisets, to make the translation from \kl{sequents} to formulas
completely deterministic. Thus we need to add the \kl{exchange} rule
{\kl{exch}}, which is simulated straightforwardly with the {\kl{Fcomm}} rule as
mentioned earlier. The second modification we do is adding \kl{introduction
rules} {\kl{{=}R}} and {\kl{{=}L}} for equality. The \kl{left introduction rule}
{\kl{{=}L}} captures Leibniz's elimination scheme, and is in fact the rule
{\intro{Repl}} from \cite{negri_structural_2001} (modulo the fact that we use
single-conclusion instead of multi-conclusion sequents). The \kl{right
introduction rule} {\kl{{=}R}} is a $0$-ary reflexivity rule, instead of the
{\kl{Ref}} rule from \cite{negri_structural_2001} (\reffig{ref-rule}). The
reason is that we cannot simulate the latter directly without adding its
equivalent rule {\kl{ref}} to our calculus (\reffig{ref-rule}), which we do not
want to do because it would break the \kl{subformula property}. We conjecture
that using {\kl{{=}R}} instead of {\kl{Ref}} does not break the cut
admissibility theorem from \cite{negri_structural_2001}.

\begin{theorem}[Completeness of subformula linking]\labthm{sfl-completeness} If
...
\begin{proof}
  ...
  rules {\kl{ax}}, {\kl{\top R}}, {\kl{\bot L}} and {\kl{{=}R}}. Other rules are
  simulated as usual by composing the derivations obtained from induction
  hypotheses, making a crucial use of the release rules {\kl{Brel}} and
  {\kl{Frel}}. The full mapping from \kl{sequent calculus} rules to derivations
  in our \kl{subformula linking} calculus is given in the following table. Note
...
an improvement over both our method of \kl{subformula linking} implemented in
\kl{Actema}, and the method of Chaudhuri implemented in the \intro{Profint}
prototype \sidecite{DBLP:conf/cade/Chaudhuri21}. Like us, they perform \textit{a
priori} \kl{unification} on the linked subformulas, both to determine
appropriate \kl{substitutions} for instantiating quantifiers, and rule out
\kl{invalid} linkages. But their method improves upon ours by being able to link
subformulas with non-trivial quantifier instantiations, such as the following
\kl{linkage} that currently fails in \kl{Actema}\sidenote{It would be
interesting to understand precisely why it fails, and if this invalidates the
\kl{productivity} theorem.}:
...
proving in the \intro{Iris} framework for program verification in \kl{Coq}
\sidecite{jung_iris_2018}, it is for now limited to \emph{\kl(dnd){backward}}
\kl{linkages}. They provide a detailed formalization in \kl{Coq} that relates
their method with those of \kl{Actema} and \kl{Profint}, based on \emph{linking
\kl{judgments}} of the form $H \curlywedge [O]\VDash G$ that have a derivation
precisely when $H \back G \steps{} O$\sidenote{Contrary to our usage in this
chapter, they use the terminology ``linkage'' to denote derivations of these
linking \kl{judgments}, rather than \kl{paths} to selected subformulas.}.
...
{\kl{conn}} and {\kl{weak}} (\reffig{reslink}), which is itself inspired by
...
the \kl{validity} criterion of \kl{linkages}:
\begin{itemize}
  ...
  \kl{linkages}\sidenote{Note that \kl{invalid} \kl{linkages} still give rise to \kl{CoS}-style
...
\end{itemize}
Hence, our approach to \kl{subformula linking} seems to exhibit some properties of
canonical proofs, but at the level of \emph{partial proofs}: \kl{valid} \kl{linkages} make
...
% depending on the semantics of the \kl{action} under consideration. Such a generalized
% method might be dubbed \emph{``link inference''}. Subformula linking then
% corresponds to a particular kind of \kl{action}, which justifies any occurrence of
...
\emph{tangible functional programming} \sidecite{elliott-tangible}. His concept
of \emph{deep application} of \kl{$\lambda$-terms} seems related to the notion
of \kl{subformula linking}, when viewing function and product types as
...
compute the result of a \kl{DnD}, though it follows a more complex dynamic than
our \kl{rewriting rules}. Even if the mapping between proofs and programs is not
