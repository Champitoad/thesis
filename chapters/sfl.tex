...
\begin{definition}[Context]\label{def:formula-context}
  ...
  is not a substitution because it allows variable capture. For instance
...
\begin{itemize}
  ...
  that after instantiating some quantifiers in their \kl{context} they can interact
...
Given a \kl{context} $A\hole$ seen as a tree, one can always start from
the root and traverse the branch of $A\hole$ that leads to its \kl{hole}
...
are \kl{contexts} $B\hole$, $C^+\hole$ and $D^-\hole$ such that
