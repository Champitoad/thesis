...
\emph{structural rules} of sequent calculus. We already have the {\rsf{Fcomm}}
...
\begin{proof}
  ...
  and {\rsf{Frel}}. The full mapping from sequent calculus rules to
...
and the method of Chaudhuri implemented in the \sys{Profint} prototype
...
proving in the Iris framework for program verification in Coq
\sidecite{jung_iris_2018}, it is for now limited to \emph{backward} linkages.
They provide a detailed formalization in Coq that relates their method with
that of Actema and \sys{Profint}, based on \emph{linking judgments} of the form
$H \curlywedge [O]\VDash G$ that have a derivation precisely when $H \back G
\steps{} O$\sidenote{Contrary to our usage in this chapter, they use the
...
{\rsf{conn}} and {\rsf{weak}} (\reffig{reslink}), which is itself inspired
