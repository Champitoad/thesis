...
guarantees a \emph{productivity} property on \kl{DnD} actions. \refsec{action}
...
\refsec{productivity} states and proves formally the productivity property.
...
that some simplification occurs, either a justification with the {\kl(sfl){id}}
...
in \kl{Actema}. The idea is that when the user starts dragging an \kl{item}, this
indicates to the system that she wants to perform a \kl{DnD} action involving
subterms of this \kl{item}. Then the system can suggest such possible actions by
highlighting subterms in the \kl{goal} which form a valid \kl{linkage} with the dragged
...
subterms in the \kl{goal} which are not contained in the dragged \kl{item}. But if one
...
\kl{polarities}, and Clause \ref{clause:intuit} ensures that the \kl{linkage} makes sense
...
\begin{condition}[Identity]\label{cond:unif} 
  ...
  be true:
  \begin{enumerate}
   \item There exists a \kl{substitution} $\sigma$ which \kl{unifies} $A$ and $A'$
     over the \kl{unifiable variables} of the \kl{linkage}.\label{clause:unif}
...
\item \textbf{Linking:} the system then chooses a rewriting start\-ing from the
  ...
  \label{step:linking}
\item \textbf{Interaction:} thus one can apply the {\kl(sfl){id}} rule (resp. an equality rule in
...
\kl{subformula linking} which do not provide this guarantee of productivity, or
...
\begin{enumerate}
  ...
  discipline;
  \item if neither are \kl{invertible}, we want to choose the order that maximizes
  ...
  distinguishing between the source \kl{path} and the destination \kl{path}\sidenote{Note
  that in the current implementation of \kl{Actema}, we instead rely on an arbitrary
  prioritizing fixed in the system, which can hinder in some cases the ability
  to prove a \kl{goal} through \kl{DnD} actions. In practice, one rarely encounters such
...
because all \kl{structural rules} are \kl{admissible} in it (but they would be
straightforward to simulate apart from the cut rule). The first modification we
do is that we model hypotheses in \kl{sequents} as lists instead of multisets, to
make the translation from \kl{sequents} to formulas completely deterministic. Thus we
...
an improvement over both our method of \kl{subformula linking} implemented in \kl{Actema},
...
\kl{unification} on the linked subformulas to determine appropriate \kl{substitutions} for
...
in \kl{Coq} \sidecite{jung_iris_2018}, it is for now limited to \emph{\kl(dnd){backward}}
\kl{linkages}. They provide a detailed formalization in \kl{Coq} that relates their method
with that of \kl{Actema} and \kl{Profint}, based on \emph{linking judgments} of the
...
than \kl{paths} to selected subformulas.}.
