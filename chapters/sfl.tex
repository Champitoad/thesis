...
\kl{context} and \kl{polarity}, and explains how \kl{DnD} actions are specified
by the user interactively through schemas called \emph{\kl{linkages}}.
...
We will write $A^+\hole$ to specify that a \kl{context} is {\em positive},
meaning that $\inv(A^+\hole)$ is even. Symmetrically, $A^-\hole$ will be
used for {\em negative} \kl{contexts}, meaning that $\inv(A^-\hole)$ is odd.
\end{definition}
...
\kl{path} (Definition \ref{def:path}). We call \intro{linkage} the combined data of
...
rewrite is already available in the standalone version of \kl{Actema}: one needs to
...
\kl{linkage} as a formula whose topmost connective is a \kl{interaction operator} $\link \in
...
in the original formulation and implementation of \kl{subformula linking}
...
in \kl{Actema}. The idea is that when the user starts dragging an item, this
...
subterms in the \kl{goal} which are not contained in the dragged item. But if one
...
proof step:
\begin{enumerate}
\item \textbf{Selection:} the user selects two subterms in two items of the current \kl{goal}; \label{step:selection}
...
consider the polarity of their \kl{context}.
...
polarity of \kl{contexts} around redexes, in the following precise sense:
...
case where the rewriting starts in the (positive) empty \kl{context}.
...
\emph{\kl(dnd){backward}} \kl{linkage} between $\limp$-linked formulas in any positive \kl{context}
$C^+\hole$, and dually rule {\rsf{F}} creates a \emph{\kl(dnd){forward}} \kl{linkage} between
$\land$-linked formulas in any negative \kl{context} $C^-\hole$\sidenote{This is
...
rules are not closed under arbitrary \kl{contexts}: indeed the polarity restrictions
...
identifying any subformula occurring in a negative \kl{context} as a hypothesis. Thus
