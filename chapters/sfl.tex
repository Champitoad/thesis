...
\kl{DnD} actions, which were introduced informally through examples in
...
\kl(sfl){context} and \kl(sfl){polarity}, and explains how \kl{DnD} actions are
...
guarantees a \kl{productivity} property on \kl{DnD} actions. \refsec{action}
...
Finally, \refsec{dnd-completeness} shows how \kl{DnD} actions can be turned into
...
\AP
In addition to the \kl{items} involved, every \kl{DnD} action specifies the
...
in \refsec{equality}, rewriting equalities is an example of action that can
...
subterms that live in the same \kl{item}, because dragging actions can only be
...
action involving subterms of this \kl{item}. Then the system can suggest such
possible actions by highlighting subterms in the \kl{goal} which form a
...
\begin{definition}[Depth]\labdef{sfl-ctx-depth}
  The \intro{depth} $\intro*\sdepth{A\hole}$ of a \kl{context} $A\hole$ is
  ...
  \begin{enumerate}[itemsep=0.8em]
    ...
    \begin{itemize}
      ...
      \begin{enumerate}[itemsep=0.4em]
        ...
        % Thus for \rsf{R{\limp}_1} which is backward, we start with either
        % $(1,1)$ or $(0,2)$, and obtain $(1,0)$ or $(0,1)$ which are valid
        % according to Condition \ref{cond:pol} since $\mathcal{L'}$ is forward.
        % Conversely for \rsf{F{\limp}_1} which is forward, we must start with
...
\begin{enumerate}
  ...
  \kl{DnD} action, that is distinguishing between the source \kl{path} and the
  ...
  \kl{DnD} actions. In practice, one rarely encounters such cases in real
  examples.}.
\end{enumerate}
...
graphical actions that reduces it to the empty \kl{goal}. We noticed in
\refremark{click-completeness} that click actions are a sufficient basis for
...
interesting to consider the question of completeness for \kl{DnD} actions alone.
It turns out that the answer is positive: the mechanism of \emph{\kl{subformula
linking}} underlying \kl{DnD} actions is powerful enough to capture provability
...
closely what we do in \kl{Actema}, where hypotheses involved in a \kl{DnD} action are
...
because it is already handled by click actions on red \kl{items} in \kl{Actema}
