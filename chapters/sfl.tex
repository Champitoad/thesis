...
following the original work of K. Chaudhuri on \emph{\kl{subformula linking}}
...
\kl(dnd){forward} \kl{DnD}, instead of having just an equivalent \kl{goal} written in a different
...
arbitrary depth, such as \kl{Coq} with its \emph{generalized rewriting} mechanism
...
release semantics of \kl{subformula linking} which do not provide this guarantee of
productivity, or with the logical reasoning \kl{tactics} of \kl{proof assistants} based on
...
apply the \kl{invertible} logical rules first since they preserve provability. In our
framework, this translates into first applying the \kl{invertible} \kl{rewriting rules} (the
ones marked by a *).
...
\begin{enumerate}
  ...
  \item if only one is \kl{invertible}, we apply it first following the focusing
  discipline;
