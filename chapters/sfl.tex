...
We do this mainly from the formal perspective of \kl{deep inference} proof theory,
...
Each kind of \kl{DnD} action is mapped in the system to a specific form of linkage,
    ...
    \begin{itemize}
      ...
      \begin{enumerate}[itemsep=0.4em]
        ...
        \begin{itemize}
          ...
          {\rsf{R{\limp}_1}} rule and $\mathcal{L'}$ is forward, and thus
          ...
          \begin{itemize}
            ...
            forced to apply {\rsf{Fcomm}} so that we can apply the forward
...
backward \kl{DnD}, or the assumptions are \emph{strictly} strengthened after a
forward \kl{DnD}, instead of having just an equivalent goal written in a different
...
$C^+\hole$, and dually rule {\rsf{F}} creates a \emph{forward} linkage between
...
\emph{structural rules} of \kl{sequent calculus}. We already have the {\rsf{Fcomm}}
...
\emph{weakening} with the rules {\rsf{conn}} and {\rsf{weak}}
...
An alternative to the full contraction rule {\rsf{conn}} is to systematically
...
Because linkages created by rules {\rsf{B} and \rsf{F}} are not
...
with the {\rsf{Fcomm}} rule as mentioned earlier. The second modification we
...
equivalent rule {\rsf{ref}} to our calculus (\reffig{ref-rule}), which we do
...
instantiation, such as the following linkage that currently fails in
...
in Coq \sidecite{jung_iris_2018}, it is for now limited to \emph{backward}
linkages. They provide a detailed formalization in Coq that relates their method
...
recently been extended to handle first-order \kl{classical} quantifiers
...
\begin{itemize}
  ...
  linkages\sidenote{Note that invalid linkages still give rise to \kl{CoS}-style
