...
\begin{definition}[Unifiable variables]\label{def:uvars} The set
...
\begin{condition}[Identity]\label{cond:unif}
  ...
  \begin{enumerate}
   ...
   \item \label{lab:cond} Furthermore, the \kl{unification} respects the order
...
\begin{proof}
  ...
  \kl{interaction operator}. Thus if $C\select{A \link B} \step{} D$, the only possible
...
\begin{remark}
  ...
  called {\em \kl{invertible}} and their names are tagged by *. This point will be
...
\item When one can apply a rule involving a quantifier $\forall x$ (resp.
  $\exists x$), one checks whether the \kl{substitution} instantiates $x$ or not. In
  the first case one performs the instantiation rule \kl{L\forall i} or
  \kl{F\forall i} (resp. \kl{R\exists i}); in the second case the
  corresponding \kl{switch rule} in {\small $\{\kl{L\forall s}, \kl{R\forall
...
\kl{subformula linking} which do not provide this guarantee of \kl{productivity}, or
with the logical reasoning \kl{tactics} of \kl{proof assistants} based on
...
\kl{subformula linking} system almost identical to ours. We hope we will be able to
...
it to the empty \kl{goal}. We noticed in Remark \ref{rem:click-completeness} that
...
the mechanism of \emph{\kl{subformula linking}} underlying \kl{DnD} actions is powerful
...
\begin{remark}
  ...
  that for any true formula, there always exists a derivation in our \kl{subformula
  ...
  \begin{itemize}
    \item To show that the choices performed by the \kl{focusing} procedure always
    allow to find a proof when there exists one, it would be necessary to
    formulate and prove a \emph{\kl{focusing} theorem} based on the permutability of
    rules mentioned at the end of \refsec{invert}.
    \item Even then, some additional rules of our \kl{subformula linking} system are
...
creation between two formulas of a sequent. Rule {\kl{B}} creates a
\emph{\kl(dnd){backward}} \kl{linkage} between $\limp$-linked formulas in any \kl{positive} \kl{context}
$C^+\hole$, and dually rule {\kl{F}} creates a \emph{\kl(dnd){forward}} \kl{linkage} between
$\land$-linked formulas in any \kl{negative} \kl{context} $C^-\hole$\sidenote{This is
...
rules are not closed under arbitrary \kl{contexts}: indeed the \kl{polarity} restrictions
are necessary to ensure soundness, as reviewed in \refsec{soundness}. A \kl(dnd){backward}
\kl{linkage} in a \kl{sequent} $\Gamma, D\select{A} \seq E\select{B}$ would be encoded by
the instance
...
modification we do is that we model hypotheses in \kl{sequents} as lists instead
of multisets, to make the translation from \kl{sequents} to formulas completely
deterministic. Thus we need to add the \kl{exchange} rule {\kl{exch}}, which is
simulated straightforwardly with the {\kl{Fcomm}} rule as mentioned earlier. The
second modification we do is adding \kl{introduction rules} {\kl{{=}R}} and
{\kl{{=}L}} for equality. The \kl{left introduction rule} {\kl{{=}L}} captures
Leibniz's elimination scheme, and is in fact the rule {\intro{Repl}} from
...
{\kl{{=}R}} is an axiomatic reflexivity rule, instead of the {\kl{Ref}} rule
...
{\kl{ref}} to our calculus (\reffig{ref-rule}), which we do not want to do
because it would break the \kl{subformula property}. We conjecture that using
{\kl{{=}R}} instead of {\kl{Ref}} does not break the cut admissibility theorem
...
\begin{proof}
  ...
  rules {\kl{ax}}, {\kl{\top R}}, {\kl{\bot L}} and {\kl{{=}R}}. Other rules
  are simulated as usual by composing the derivations obtained from the
  induction hypotheses, making a crucial use of the release rules {\kl{Brel}}
  and {\kl{Frel}}. The full mapping from \kl{sequent calculus} rules to
  derivations in our \kl{subformula linking} calculus is given in the following
...
priori} \kl{unification} on the linked subformulas to determine appropriate
\kl{substitutions} for instantiating quantifiers and rule out unwanted links.
But their method improves upon ours by being able to link subformulas with
non-trivial quantifier instantiation, such as the following \kl{linkage} that
currently fails in Actema\sidenote{It would be interesting to understand
precisely why it fails, and if this invalidates the \kl{productivity} theorem.}:
...
proving in the Iris framework for program verification in \kl{Coq}
...
their method with that of \kl{Actema} and \kl{Profint}, based on \emph{linking
...
notion of \kl{subformula linking}, when viewing function and product types
