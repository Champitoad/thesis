...
% Following the mapping of click \kl{actions} to \kl{sequent calculus} rules
% (\reftab{click-rules}),
...
by assumption. We come back to these multi-conclusion click \kl{actions} in
...
directly the two occurrences of $A$, as we do with \kl{DnD} \kl{actions} in
...
logic, the most famous being \intro{GHCP} from Dragalin
...
\intro{HOL Light} ask for the position in the list as an integer to designate a
...
click and \kl{DnD} \kl{actions} could be triggered by voice commands once the
...
graphical proof \kl{actions}, as outlined in the introductory example of this
...
In \reftab{click-rules}, we showed how click \kl{actions} in \kl{Actema} are in direct
...
\begin{itemize}
  ...
  $A$ and $B$. This is the dual behavior to click \kl{actions} on blue conjunctions,
  ...
  \reffig{multi-imp-intro}), and our set of \kl{actions} becomes a \kl{proof system}
...
rules are \kl{invertible}, like the \intro{G3c} calculus of
...
makes sense to also allow for \kl{DnD} \kl{actions} between two conclusions. But we
...
The rules governing $\para$ are presented in \reffig{parallel}. A
\kl(dnd){parallel} \kl{linkage} can be created either by
\kl[drag-and-drop]{drag-and-dropping} two conclusions together, or through an
instance of the new \kl(dnd){backward} rule \kl{L{\limp}_1}. It is important to
note that this rule is only sound \emph{\kl{classically}}; indeed we can now
come back to the example of \refsubsec{polarity} and give the following
...
interesting to compare the behaviors of \kl{intuitionistic} and \kl{classical} \kl{DnD} \kl{actions}
