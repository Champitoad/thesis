...
In virtually every \kl{proof assistant}, the goals the user is faced with are
...
directly the two occurrences of $A$, as we do with \kl{DnD} actions in Actema.
...
and the restriction of \kl{intuitionistic} sequents to one conclusion, as ways to
...
Coq and Lean, but some provers like \kl{HOL Light} ask for the position in the list
...
\begin{itemize}
  ...
  the additional behavior that all the other conclusions of the goal are
...
\begin{remark}
  In the special case of \kl{intuitionistic} sequents with one conclusion, the two
...
items until all goals are solved. The procedure ends because all \kl{introduction
...
invertible \kl{introduction rules}, it is necessary to duplicate the quantified
...
original $\forall x. A$ item. This corresponds to the invertible
...
to the invertible \kl{right introduction rule} of \kl{G3c} ({\rsf{\exists R*}} in
\reffig{multi-inst}).
...
\kl{rewriting rules} for a new linking operator, by following the same recipe we used
...
\kl(dnd){forward} \kl{linkage} combines two negative subformulas in the same context to produce
a new \emph{hypothesis}, a parallel \kl{linkage} combines two positive subformulas in
the same context to produce a new \emph{conclusion}. \kl(dnd){Backward} \kl{linkages} can then
...
\kl{rewriting rules} that change linking operators is provided in
...
{\rsf{L{\limp}_1}} is sound \kl{classically}. Note that now sequents have multiple
...
that were not invertible in \kl{intuitionistic} logic become invertible in \kl{classical}
