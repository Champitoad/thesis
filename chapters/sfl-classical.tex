...
A$, which now appear as two separate red \kl{items} in the same tab. Then we try to
...
Actema for the universal quantifier: dropping a term $t$ on a blue \kl{item} $\forall
x. A$ will produce a new hypothesis $\subst{A}{t}{x}$, while keeping the
original $\forall x. A$ \kl{item}. This corresponds to the \kl{invertible}
\kl{left introduction rule} of \kl{G3c} ({\rsf{\forall L*}} in \reffig{multi-inst}).
But in the single-conclusion framework, dropping a term $t$ on a red \kl{item}
...
a new \emph{hypothesis}, a parallel \kl{linkage} combines two \kl{positive} subformulas in
...
drop Clause \ref{clause:intuit} of Condition \ref{cond:pol} about the \kl{polarities}
...
\kl{Polarity} preservation (Fact \ref{prop:rules-preserve-polarity}) is also true
