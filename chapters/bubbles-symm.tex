...
which requires a new distinction between \emph{closed} and \emph{open}
...
kinds of \kl{solutions}, by assigning them one of two \emph{statuses}\sidenote{In the
terminology of Martin-Löf, we could say that we now have two distinct forms of
\emph{judgment}.}:
\begin{itemize}
  \item \emph{closed} \kl{solutions} $\Gamma \piq{\cS} \Delta$ correspond
  ...
  closed \kl{solutions} were encoded by \kl{solutions} with no conclusions;
  \item \emph{open} \kl{solutions} $\Gamma \seq \Delta$ correspond to open leaves
  in the proof tree (i.e. unsolved \kl{subgoals}). In the asymmetric interpretation,
  they were encoded by \kl{solutions} with one conclusion.
\end{itemize}
Then we keep the last proposed interpretation for closed \kl{solutions}, and
interpret open \kl{solutions} like usual sequents:
...
\end{align*}
Graphically, closed \kl{solutions} with no \kl{bubbles} can be distinguished from open
...
closed \kl{solutions} through \emph{non-structural} recursion. This gives the
...
\kl{single-conclusion} (resp. no-conclusion) \kl{solutions} with open (resp.
closed) ones (more details will be given in the next section), the ${\limp}{+}$
...
definition of \kl{solutions}, which can now be open or closed:

\begin{definition}[Solution]\labdef{pol-solution}
  
  A \intro{solution} is a multiset of \kl{ions} and bubbles. Its \emph{status}
  is either \emph{closed} or \emph{open}, and open \kl{solutions} cannot contain
...
\begin{description}
  ...
  \item[Factorizability]
    ...
    multi-conclusion \kl{solutions}, we will now be able to factorize \emph{\kl{backward}}
