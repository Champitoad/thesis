...
logic that conservatively extends \kl{intuitionistic} logic, and in particular
does not prove the \kl{law of excluded middle}.

An underlying thread of our investigation will be the quest for a \emph{fully
\kl{iconic}} \kl{proof system}, where all logical connectives can be replaced by
appropriate new kinds of bubbles. This will make us rediscover many principles
already studied in the \kl{deep inference} literature, with topological
intuitions of the \kl{bubble} \kl{metaphor} shedding a new light on them. We
will end up with two symmetric \kl{bubble calculi}, each with its own tradeoff
on the properties satisfied by \kl{inference rules}. In particular, the ability
of the calculi to factorize \emph{\kl{backward}} in addition to \kl{forward}
proof steps might prove useful to build concise proofs, all through \kl{direct
manipulation}.
...
\refsec{symm-interp} the restriction to \kl{single-conclusion} \kl{solutions},
which requires a new distinction between \emph{\kl{saturated}} and
\emph{\kl{unsaturated}} \kl{solutions}. This gives rise in \refsec{colors} to an
extension of the syntax of \kl{solutions}, where \kl{bubbles} can themselves be
\emph{\kl{polarized}}. In \refsec{design-props} we identify key properties that
will guide the design of \kl{inference rules}, some of which were already aimed
for implicitly throughout the evolution of our concept of bubble.

\AP
In \refsec{symmetric-calculus} we introduce a core \emph{symmetric \kl{bubble
calculus}} for \kl{classical} logic called \kl{system~B}, in reference to the
symmetric \intro{system~L} of Herbelin \sidecite{herbelin_duality_nodate}. Then
in \refsec{bubbles-soundness} we prove the soundness of \kl{system~B}, and show
that by removing selectively some \kl{inference rules} that define the
...
\kl{bi-intuitionistic} fragment is not only sound, but also \emph{cut-free
complete} with respect to the cut-free nested \kl{sequent calculus}
\intro{DBiInt} of Postniece \sidecite{postniece_deep_2009}.

Finally in \refsec{invertible-calculus}, we introduce a fully \kl{invertible}
variant of \kl{system~B} that we conjecture to be complete, and present a
canonical way to search for proofs in this system. Unfortunately, invertibility
does not entail the full \kl{iconicity} of the system, and we reflect on the
fundamental reasons that might prevent any variant of \kl{system~B} from being
fully \kl{iconic}.
...
\begin{enumerate}
  \itemAP a \kl{left introduction rule} for $\limp$ (there might be many ones, as in
  the calculus \intro{LJT} of Dyckhoff \sidecite{dyckhoff_contraction-free_1992});
  \item the \kl{right introduction rule} for either:
    \begin{itemize}
    ...
    \end{itemize}
\end{enumerate}
In \kl{BJ}, this means that click \kl{actions} on blue $\hypo{\limp}$ and red
$\conc{\lor}$ need to be performed in a specific order to be able to complete
proofs.

In his thesis \sidecite{guenot_nested_2013}, Guenot introduced a specific kind
of \kl{nested sequent} system where --- like in \kl{BJ} --- \kl{inference rules}
can be expressed as \kl{rewriting rules}. An interesting feature of these
systems is that they satisfy a \emph{decomposability} property: all
\kl{introduction rules} for connectives are \emph{\kl{invertible}}, and formulas
can be completely decomposed by using them until atoms are reached, before
applying other rules. Thus \kl{introduction rules} are in a sense
\emph{\kl{admissible}} in these systems, because every formula can be translated
into an equivalent pure \kl{nested sequent} with the same number of
atoms\sidenote{As far as we know, the admissibility of \kl{introduction rules}
is not proved, let alone mentioned in \cite{guenot_nested_2013}. This is our own
observation which lacks a proper formal proof, and is thus subject to caution.}.
...
\kl{actions} are redundant. Although we already considered this possibility in
\refsec{dnd-completeness}, it goes further here by making even \emph{logical
...
\section{Symmetric interpretation}\labsec{symm-interp}
...
\kl{solution} $S$ as a formula $\sint{S}$. If we just take the \kl{asymmetric
...
{\kl{\land R{*}}} rule (\reffig{multi-and-intro}). Then our idea was that
instead of implementing distribution/sharing of conclusions inside \kl{inference
rules}, we could do it implicitly in the interpretation of \kl{solutions}. This
is already what happens in the \kl{asymmetric interpretation} for hypotheses;
indeed the \kl(sequent){context} $\Gamma$ is shared among \kl{subgoals},
because:
\begin{enumerate}
  \item it appears on the left side of an implication $\limp$;
  \item \kl{bubbles} are joined conjunctively;
...
consider the two \kl{solutions} of \reffig{distr-concl}, we would like to have
...
a distinction between two kinds of \kl{solutions}, by assigning them one of two
statuses\sidenote{In the terminology of Martin-Löf, we could say that we now
have two distinct forms of \emph{\kl{judgment}}.}:
\begin{itemize}
  ...
  $\sint{\Gamma \piq{} \Delta} = \top$. In the \kl{asymmetric interpretation},
  ...
  open leaves in the proof tree (i.e. unsolved \kl{subgoals}). In the \kl{asymmetric
  interpretation}, they were encoded by \kl{solutions} with one conclusion.
\end{itemize}

Then we keep the last proposed interpretation given in \refeq{interp2} for
\kl{saturated} \kl{solutions}, and interpret \kl{unsaturated} \kl{solutions}
like usual sequents:
$$\sint{\Gamma \seq \Delta} = \bigwedge{\Gamma} \limp \bigvee{\Delta}$$
\AP To be able to abstract from the \kl[saturated]{saturation} status of
\kl{solutions}, we reframe the syntax of \kl{solutions} with so-called
\intro{branching operators} $\J$:
\begin{align*}
...
\begin{remark}
  ...
  More complex reactions through \kl{SFL} are still possible though and may
...
Now coming back to our motivating example of \reffig{distr-concl}, the
interpretation of \refeq{interp2} still fails, because we associate two
...
turns out that this equivalence is true in \kl{classical} logic, but \emph{not}
in \kl{intuitionistic} logic. More precisely, it is the implication $G \defeq (A
\limp (B \lor C)) \limp ((A \limp B) \lor C)$ which is not provable
\kl{intuitionistically}, since it can easily be shown equivalent to the \kl{law
of excluded middle}\sidenote{\AP This was already noticed in
\cite{clouston-annotation-free-2013}, with the linear version $(A \multimap (B
\parr C)) \multimap ((A \multimap B) \parr C)$ of $G$ called \intro{Grishin (a)}
and its converse \intro{Grishin (b)}. More precisely, it is affirmed that while
\kl{Grishin (b)} is valid in \intro{FILL}, which is the extension of
intuitionistic multiplicative linear logic (\intro{IMLL}) with $\parr$, adding
\kl{Grishin (a)} makes \kl{FILL} collapse to \kl{MLL}.}. Thus according to this
...
\kl{subgoals}, we hardcode this semantics by defining the interpretation
function over \kl{saturated} \kl{solutions} through \emph{non-structural}
recursion. This gives the following final definitions:

\begin{definition}[Mix operator]\labdef{mixop}
  The commutative \intro{mix operator} $\intro*\mix$ on \kl{solutions} is defined by:
  \begin{align*}
...
\begin{definition}[Symmetric interpretation]\labdef{sinterp}
  \AP The \intro{symmetric interpretation} $\intro*\sint{-}$ of a \kl{solution}
...
With our new \kl{symmetric interpretation}, we can start generalizing the rules
of \kl{BJ} to multiple conclusions. While for most rules one just has to replace
\kl{single-conclusion} (resp. no-conclusion) \kl{solutions} with
\kl{unsaturated} (resp. \kl{saturated}) ones\sidenote{More details will be given
in the next section}, the ${\limp}{+}$ rule stands out as particularly
problematic. Indeed if we content ourselves with the natural generalization
{\kl{{\limp}{+}c}} of \reffig{wrong-imp-pos}, then we can easily build a proof
of the excluded middle like in \reffig{lk-tnd}, and thus collapse to
\kl{classical} logic. This fact is well-known in the literature on
multi-conclusion \kl{intuitionistic} \kl{sequent calculi}, and the solution is
usually to discard the \kl(sequent){context} of conclusions $\Delta$, as in the
{\kl{{\limp}R{*}i}} rule of \reffig{multi-imp-intro}. But this would make our
rule both non-local and non-\kl{invertible}.
...
\begin{marginfigure}
  ...
  $$
  \caption{\kl{Classical} multi-conclusion version of \kl{{\limp}{+}}}
...
\begin{marginfigure}
  ...
  \end{array}
  $$
  \caption{\kl{$\mathbb{F}$-rules} for red bubbles}
...
\emph{conclusions} of other sequents. In our chemical \kl{metaphor}, this
corresponds to having \emph{red \kl{bubbles}}. Then the key idea is to allow
\emph{hypotheses} to flow into \kl{sequents} that appear as
conclusions\sidenote{This corresponds to the {\rnm{Lift}} rule of
\cite{fitting-nested-2014} and {\rnm{pl_1}} rule of
\cite{clouston-annotation-free-2013}.}, but to forbid \emph{conclusions} to do
so. Graphically, this means that blue \kl{items} can enter red \kl{bubbles}
(rule {\kl{f{-}{+}{\da}}} of \reffig{flow-red-bubbles}), but red \kl{items} cannot:
...
\kl{bubbles} the \kl{classical} equivalence $ (A \limp B) \lor C \semequiv{} A
...
parallely in \reffig{bubbles-grishin}. Indeed both proofs have a very similar
structure:
\begin{enumerate}
  ...
  \kl{solution}. And since \kl{items} are trapped within bubbles, the only way
  to go is to move the blue hypothesis inside the red \kl{bubble} with the
  {\kl{f{-}{+}{\da}}} rule;
  ...
  those of \kl{BJ}: we encapsulate both subformulas in separate bubbles, but
  without touching the conclusions of the ambient \kl{solution};
  \item now that all formulas have been decomposed, it only remains to bring
  together dual atoms for annihilation, and pop all empty bubbles. In \kl{Grishin
  (b)} this is easy, because all necessary movements (indicated by green arrows)
  are valid: they only cross gray \kl{bubbles} inward. In \kl{Grishin (a)} this works for
...
\begin{figure*}[h!]
  ...
  \caption{Proof attempts for \kl{Grishin (a)} and \kl{Grishin (b)}}
  \labfig{bubbles-grishin}
\end{figure*}

Thus in order to prove \kl{Grishin (a)} and recover \kl{classical} logic, it suffices
either to add the {\kl{f{+}{+}{\da}}} rule allowing red \kl{items} to enter red \kl{bubbles}
(\reffig{flow-red-bubbles}), or to use the {\kl{{\limp}{+}c}} rule which
...
\begin{marginfigure}
  ...
  \end{array}
  $$
  \caption{\kl{$\mathbb{F}$-rules} for blue bubbles}
...
that ``blue items can enter red bubbles, but red \kl{items} cannot'' becomes a
new law that ``red items can enter blue bubbles, but blue \kl{items} cannot'',
which is enforced by allowing only the use of the {\kl{f{+}{-}{\da}}} rule in
...
logical connective! Since red \kl{bubbles} are produced by the implication
connective $A \limp B$, we define a dual \intro{exclusion} connective $A \lsub
B$ (read ``$A$ excludes $B$''\sidenote{We ask for the reader's leniency
regarding our choice of \kl{symbol} and terminology: in \kl{set theory} this
would be total nonsense, since $A \subset B$ would read ``$A$ is included in
$B$''. Even worse, in the boolean algebra induced by set operations, $A \subset
B$ is interpreted as $A$ \emph{implies} $B$\ldots~But we want to emphasize the
duality between \kl{exclusion} and implication by mirroring the \kl{symbol}, as
it is traditionally done with conjunction $\land$ and disjunction $\lor$.}),
whose \kl{heating rules} are those of $\limp$ where blue and red have been swapped
(\reffig{heating-exclusion}).

\begin{marginfigure}
  $$
  \begin{array}{rclr}
    \hypo{A \lsub B} &\step{} &\hbubble{\hypo{A}~~~\conc{B}} &\kl{{\lsub}{-}}\vspace{1em}\\
    \conc{A \lsub B} &\step{} &{\bubble{\conc{A}}}~~~\bubble{\hypo{B}} &\kl{{\lsub}{+}}
  \end{array}
  $$
  \caption{\kl{$\mathbb{H}$-rules} for \kl{exclusion} $\lsub$}
  \labfig{heating-exclusion}
\end{marginfigure}

Not very surprisingly, the \kl{exclusion} connective has already been studied in the
...
we add \kl{exclusion} \sidecite{Rauszer1974-RAUSAA}. In this paper, \kl{exclusion} was
...
then deepened the \kl{proof theory} of \kl{exclusion}, whether in isolation from
...
\kl{exclusion} through the \kl{Curry-Howard correspondence}, which is claimed by the first
...
\begin{definition}[Bubble]\labdef{pol-bubble}
  ...
  A \intro{bubble} is a \kl{solution} enclosed in a membrane. The membrane can
  be either \intro{unpolarized} (\reintro{neutral}), charged
  \reintro{positively}, or charged \reintro{negatively}.
\end{definition}
...
\begin{definition}[Solution]\labdef{pol-solution}
  ...
  \begin{align*}
    ...
    \J, \JB &\Coloneq {\seq} \mid {\piq{\cS}} &&
  \end{align*}
  In particular, an \intro{item} $I$ can be either a formula $A$ or a
  bubble/solution $S$.
\end{definition}
...
relative to \kl{branching operators}; that is, for any \kl{solutions} $S, T, U$ such that
...
Then we need to split our \kl{symmetric interpretation} accordingly, so that \kl{positive}
\kl{bubbles} are mapped to implications, and \kl{negative} \kl{bubbles} to
\kl{exclusions}\sidenote{Here we took inspiration from the work of Clouston et al. on
...
\begin{definition}[Polarized symmetric interpretation]\labdef{pol-sinterp}
  \AP
  The symmetric \intro{positive interpretation} and \intro{negative
  interpretation} of \kl{solutions} $\intro*\psint{-}$ and $\intro*\nsint{-}$
...
(resp. \kl{positive}) \kl{subsolution} will not contain any occurrence of the \kl{exclusion}
...
With our new syntax and interpretation of \kl{solutions} at hand, we can design
a new proof calculus that includes the rules previously discussed for
manipulating \kl{polarized} bubbles. The rich structure of \kl{solutions} offers
...
\begin{description}
  ...
  \item[Decomposability]
    ...
    connectives ``for free'', and thus reason solely on \kl{solutions} that
    comprise only \kl{bubbles} and atomic formulas. As far as we know, it has
    ...
    \kl{calculus of structures}
    \sidecite[][Theorem~4.1.3]{tubella:hal-02390267}.

    \begin{digression}
    One could argue that more ``semantic'' approaches in \kl{proof theory} have
    ...
    in \intro{transcendental syntax} \cite{eng_exegesis_2023}, where both
    frameworks are founded upon the syntactic mechanisms of proof search
    (\kl{focusing} in \kl{sequent calculus}, and \kl{unification} in the
    ...
    \emph{proofs} and not \emph{\kl{proof states}}. Also Girard embraces the
    ...
    \emph{primitive} in the tradition of \kl{mathematical logic}: they
    ...
    interesting viewpoint on the problems usually studied in \kl{proof theory}.
    In our case, it is motivated by a very concrete application: making formal
    proofs accessible to a broader audience, by replacing \kl{symbolic} and
    linguistic means of representation by \kl{iconic} and directly manipulable
    ones.
  \item[Factorizability]
    ...
    at any stage of the proof construction. With our new \kl{symmetric
    ...
    the main motivation behind the example of \reffig{distr-concl}.
  \item[Locality]
    ...
    \cite{tiu_local_2006}.}. Finally in his \kl{transcendental syntax}, Girard evokes
    ...
    is guided by the idea of \kl{direct manipulation} of logical entities by
    humans, rather than purely \kl{proof-theoretical} considerations. For
    instance, \kl{BJ} has some locality in the \kl{deep inference} sense because
    all rules are applicable in arbitrary \kl{contexts}; but we relax the
    \emph{atomicity} constraint that reduces \kl{$\mathbb{I}$-rules} and
    \kl{$\mathbb{R}$-rules} to their atomic version, because it would be
    ...
    Girard's locality. A typical example is the \kl{elimination rule} \kl{\lor
    e} for disjunction in \kl{natural deduction}, corresponding to the
    {\kl{\lor{-}}} rule of \kl{BJ} that involves an arbitrary conclusion
    $\Delta$. The benefits of locality from a \kl{UX} point of view have already
    been discussed in \refsec{bubbles-pba}.
  \item[Linearity] 
    We consider an \kl{inference rule} to be \emph{linear} when it preserves the
    number of atomic formulas in \kl{solutions}. This is a strong requirement,
    which for instance excludes the \kl{$\mathbb{I}$-rules} of \kl{BJ} since they can
    ...
    Guglielmi, \textit{``[...] \kl{deep inference} is obtained by applying some
    of the main concepts behind linear logic to the formalisms, i.e., to the
    rules by which \kl{proof systems} are designed.''}
    \sidecite{deep_inference}.
    
    The second reason is \emph{computational}: linearity can enable a measure on
    \kl{solutions} that is strictly decreasing with the application of rules,
    avoiding infinite loops during proof search as in the calculus \kl{LJT} of
    Dyckhoff \cite{dyckhoff_contraction-free_1992}.
    
    The third reason is \emph{ergonomical}: as already remarked by the authors
    of the \kl{Proof-by-Pointing} paradigm\sidenote{Section 4.1 of \cite{PbP}.},
    rules that systematically duplicate formulas can quickly overload the
    \kl{goal} with useless copies, making it harder to read and navigate.
  \item[Symmetry] 
    ...
    \emph{symmetries}. In fact, one of our ambitions with \kl{bubbles} was to
    bring back the symmetry of \kl{classical} logic in a constructive setting,
    without resorting to linear logic. This chapter stems in great part from our
    lack of satisfaction with the asymmetry at work in the \kl{BJ} calculus,
    which looked quite unnatural. Of course we will not be able to completely
    eliminate it, but it will be distilled into the \kl{flow rules} governing the
    \emph{porosity} of \kl{bubbles} that were hinted at in \refsec{colors},
    rather than through the arbitrary restriction of \kl{sequents} to one
    conclusion\sidenote{Whether it is enforced in the syntax of \kl{sequents}
    themselves, or through restriction on rules that manipulate conclusions like
    \kl{contraction} or the \kl{right introduction rule} for $\limp$.}.
    
    Our treatment of \kl{dual-intuitionistic} and \kl{bi-intuitionistic} logic
    through blue \kl{bubbles} is also motivated by this quest for symmetry. It
    should be noted that although we use naming conventions for rules that
    resemble those of \kl{CoS} (e.g. with \kl{$\mathbb{I}$-rules}), we do not aim for
    ...
    hypothesis/conclusion symmetry coming from \kl{sequent calculus}.
    Interestingly, the calculus \intro{ISgq} of Tiu for \kl{intuitionistic}
    \kl{predicate logic} does the opposite, by having a perfect dual system
    \intro{cISgq}, but no symmetries among its \kl{switch rules} (the equivalent of
    our \kl{$\mathbb{F}$-rules}) \sidecite{tiu_local_2006}.
\end{description}
