...
quest for a \emph{fully \kl{iconic}} proof system, where all logical connectives
...
\kl{bi-intuitionistic} logic supporting them, at least none suitable for our
...
\emph{not} a conservative extension of \kl{intuitionistic} \kl{predicate logic}, but only
of \emph{constant-domain} \kl{predicate logic} (see
...
nested sequent system, where like in \kl{BJ} inference rules can be expressed
...
quest for a bubble calculus in the style of Guenot capturing full \kl{intuitionistic}
logic\sidenote{Other nested sequent systems for full \kl{intuitionistic} logic exist
...
\kl{intuitionistically}, since it can easily be shown equivalent to the law of
...
bubbles the \kl{classical} equivalence $ (A \limp B) \lor C \semequiv A \limp (B \lor
...
\begin{enumerate}
  ...
  blue $\hypo{A}$ inside the red bubble with the {\rsf{f{-}{+}}} rule;
...
either to add the {\rsf{f{+}{+}}} rule allowing red items to enter red bubbles
...
literature on \kl{intuitionistic} logic, starting with the seminal paper of Rauszer
on \emph{Heyting-Brouwer logic}, i.e. \kl{intuitionistic} logic to which we add
...
interpretation in boolean algebras, this corresponds to the \kl{classical} connective
...
nested sequents for \kl{FILL} \cite{clouston-annotation-free-2013}.}:
...
\begin{description}
  ...
  \item[Factorizability]
    ...
    \kl{BJ} enables the factorization of \emph{forward} reasoning steps at any
  ...
  \item[Locality]
    ...
    the sequents of that rule, by opposition to \kl{natural deduction} rules in their
    labelled presentation where hypotheses are located in arbitrary distant
    leaves of the derivation. In the setting of \kl{deep inference}, local rules are
    those that can be applied without ``inspection of expressions of arbitrary
    size''\sidenote{Definition 2.1.1 in \cite{tubella:hal-02390267}. The same
    definition is used in \cite{tiu_local_2006}.}. Finally in his transcendental
    ...
    section \emph{Globality and locality in logical systems} in \cite[Chapter
    6]{eng_exegesis_2023}.}.
    ...
    rather than purely proof-theoretical considerations. For instance, \kl{BJ}
    has some locality in the \kl{deep inference} sense because all rules are
    ...
    the elimination rule for disjunction in \kl{natural deduction}, corresponding to
    the {\rsf{{\lor}{-}}} rule of \kl{BJ} that involves an arbitrary
  ...
  \item[Linearity] 
    ...
    proof systems are designed.''} \sidecite{deep_inference}. The second reason
  ...
  \item[Symmetry] 
    ...
    \emph{porosity} of bubbles that were hinted at in \refsec{colors}, rather
    than through the arbitrary restriction of sequents to one
    conclusion\sidenote{Whether it is enforced in the syntax of sequents
    themselves, or through restriction on rules that manipulate conclusions like
    contraction or the introduction rule for $\limp$.}. Our treatment of
    \kl{dual-intuitionistic} and \kl{bi-intuitionistic} logic through blue bubbles is
    also motivated by this quest for symmetry. It should be noted that although
    we use naming conventions for rules that resemble those of \kl{CoS} (e.g.
    ...
    dual system \kl{cISgq} but no symmetries among its switch rules (the
