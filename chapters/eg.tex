...
\paragraph{Predication} In the propositional system \sys{Alpha}, atomic graphs
...
``mortals'' \sidecite{smith_aristotles_2022}.

\paragraph{Spots} In \sys{Beta} there is no such restriction, and the above
sentence can be represented by the following graph:
\begin{equation}
...
placeholders for the argument (subject) of each predicate, and the data of a
predicate together with its hook is called a \emph{spot}\sidenote{The use of
diamonds to depict hooks is our own addition, Peirce never drew them
explicitly.}.

\paragraph{Lines of Identity} Then to assert that there exists an individual who
is both the person sitting down and Socrates, we connect the two hooks with a
so-called \emph{line of identity} (LoI hereafter). In Peirce's view, each point
in a LoI denotes an individual of the universe represented by $\SA$. Since
scribing anything on $\SA$ means asserting its truth in the universe, then it
suffices to reduce the truth of an individual to its existence, in order to
interpret the marking of LoI as having existential force. This is actually the
origin of the ``existential'' qualificative in the denomination ``existential
graph''. Note that no information is given but the individual's existence: in
particular, two distinct points on $\SA$ might or might not denote two distinct
individuals, just as with usual variables. It is the \emph{continuity} of a LoI
that signifies, in an iconic way, the identity of every point/individual
constituting, and connected by the line. Hence the following graph
$$\tikzfig{1}{0.5}{sitting-socrates-disconnected}$$
expresses that both the person sitting down and Socrates exist, but we do not
know whether they are the same individual or not.
...
\end{equation}
Hooks can then be seen as encoding the concept of \emph{arity} associated with a
predicate symbol, and LoI the concept of \emph{existential quantification} over
a single \emph{variable} $x$, where the two occurrences of $x$ correspond to the
two extremities of the line connected to the hooks. As in \sys{Alpha}, the
conjunctive aspect of the sentence is accounted for by the fact that the two
spots are juxtaposed in the same area on $\SA$.
...
\mathsf{Socrates}(y) \wedge x = y$$
In Peirce's original notation, there is no way to distinguish between these two
formulations, and they would both be represented by the first graph above.
However in \sidecite{pietarinen_compositional_2020}, in order to have a rigorous
interpretation of the syntax of \sys{Beta} in category theory, the authors
propose to analyze LoI into essentially three distinct icons, from which every
LoI can be reconstructed:

\begin{description}
  \item[Binder] As we have seen, one function of LoI is to \emph{quantify} over
  individuals. For now we have only considered LoI located at the top-level of
  $\SA$, i.e. in a \emph{positive} area, where they are given \emph{existential}
  force. If we were to negate the graph (\ref{eq:sitting-socrates}) by enclosing
  it in a cut, we would get a graph expressing the negation of formula
  (\ref{eq:sitting-socrates-fo}), which by De Morgan duality is equivalent to
  $$\forall x. \neg \mathsf{PersonSittingDown}(x) \vee \neg
  \mathsf{Socrates}(x)$$
  
  \begin{figure}
    \begin{center}
    \tikzfig{1.25}{0.5}{eg-forall}
    \end{center}
  ...
  correctly graph (\ref{eq:eg-forall}), one needs to adopt what Peirce calls an
  ...
  through within''. This physically-flavored terminology is disturbingly
  reminiscent of the intuition we developed within our own bubble calculus and
  illustrated in \reffig{bubbles-flow} and \reffig{bubbles-porosity}, as we were
  not aware of the existence of EG at the time!} reading of EG, where they are
  inspected starting from the top-level of $\SA$, and then descending
  (recursively) into their various cuts. In particular, the location of a LoI
  should be identified with its \emph{outermost} end, as illustrated in graph
  \ref{eq:eg-forall-out}; if we were to associate it instead with e.g. its
  innermost end as in graph (\ref{eq:eg-forall-in}), then we would swap the
  positions of $\exists$ and $\neg$ in the associated statement, giving the
  ...
  universal in a negative area. This leads us to split the syntax of LoI into
  ...
  cuts.}; and \emph{wires} that connect binders to the hooks of predicates,
  encoding the \emph{identity} between multiple occurrences of the same
  variable.
...
introduced in \sys{Beta} compared to \sys{Alpha}. We will see shortly that they
are sufficiently powerful to express and combine all features of first-order
logic in an elegant and intuitive way, particulary so at the \emph{illative}
level.
...
so\sidenote{A very similar metaphor is brought up by Girard in
\cite{girard:hal-01322183}, where the fact that a variable can have infinitely
many occurrences is identified with the possibility of \emph{splitting} an
electrical cable indefinitely into subcables. He explains that this is because a
cable is just a bundle of smaller wires, akin to the microscopic points of
teridentity of Peirce.} \cite[p.~117]{Roberts+1973}:
...
\paragraph{N-ary relations}

\todo{ Illustrate n-ary relations, and the directionality of hooks, with the
  Factorio example}
...
of proof for \sys{Alpha} (\refdef{eg-proof}), this constitutes the only axiom in
