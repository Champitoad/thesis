...
But far less widespread are his achievements in the realm of \kl{diagrammatic}
logic, or \emph{\kl{iconic} logic} as Shin calls it
\sidecite{10.7551/mitpress/3633.001.0001}. He dedicated a large chunk of his
life to the investigation of graphical systems, starting in 1882 with the
...
variant of the \kl{(De)iteration} principle described by Peirce in
...
\emph{\kl{analyticity}}, as popularized by Gentzen with the \emph{\kl{subformula
property}} in \kl{sequent calculus}: the latter is concerned with the analysis
...
comparing rules in different \kl{proof systems}. Apart from Peirce, the only
other logician we know of who attempted to give a non-trivial account of
\kl{illative atomicity} is J.-Y. Girard. In fact it can be argued that it is the main
...
rules'' \sidecite{girard_locus_2001}.
...
transformation that ``is not strictly an Insertion or a Deletion''. Another way
to interpret this, is that \kl{Double{-}cut} is the only \emph{linear} rule of
the system, in the sense that the premiss and conclusion contain exactly the
same atomic \kl{graphs}. Contrast this with linear logic, which instead takes
linearity as the criterion for \kl{illative atomicity}, as exemplified by the linear
decomposition of implication $A \limp B \defeq \oc A \multimap B$. This might be
the consequence of an opposite treatment given to \emph{negation}: while in
\kl{EGs} it is the only primitive constructor of the system --- remember that
the only way to increase the \kl{depth} of a \kl{graph} is with a \kl{cut}, in
\sys{LL} negation is the only defined notion through De Morgan dualities. Thus
\kl{EGs} are closer (at least syntactically) to \emph{\kl{type theories}}, which
also take a negative operation (the arrow or dependent product \kl{type}) as
their sole primitive construct.
...
\kl{(De)iteration} principle is based on a relation of \emph{local
...
Let us now go back to the ``more scientific'' \kl{(De)iteration} principle
...
Condition 4 that will account for the principle of \kl{Double{-}cut}. Then the
new rules of iteration \kl{Iter{+}}, \kl{Iter{-}} and deiteration \kl{Deit{+}},
\kl{Deit{-}} are given in a so-called \emph{atomic} variant of \kl{Alpha}, that
we name \intro{Alpha^a} in \reffig{eg-atomic-alpha}. As promised, \kl{Alpha^a}
only comprises rules that truly are insertions and omissions of \emph{arbitrary}
\kl{graphs}\sidenote{The terminology ``atomic'' might be a bit confusing: here
we think of \emph{illative} atomicity in Peirce's sense, not the fact that the
\kl{graphs} manipulated by rules are atomic, which might be termed
\emph{structural} atomicity. There seems to be a symmetric tradeoff when
comparing \kl{EGs} to the \kl{calculus of structures}: in the former, one
maximizes \kl{illative atomicity} by minimizing linearity and structural atomicity;
while in the latter, one maximizes structural atomicity and linearity by
minimizing \kl{illative atomicity}. This will become explicit in
\refsec{eg-completeness}, when simulating the \kl{calculus of structures}
...
\AP
To assert that there exists an individual who is both the person sitting down
and Socrates in \kl{graph} (\ref{eq:sitting-socrates}), we connect the two hooks
with a so-called \intro{line of identity} (\reintro{LoI}). In Peirce's view,
each point in a \kl{LoI} denotes an individual of the universe represented by
$\SA$. Since scribing anything on $\SA$ means asserting its truth in the
universe, then it suffices to reduce the truth of an individual to its
existence, in order to interpret the marking of \kl{LoI} as having existential
...
of a \kl{LoI} that signifies, in an \kl{iconic} way, the identity of every
...
\end{equation}
\kl{LoI} can then be seen as encoding the concept of \emph{existential
...
propose to analyze \kl{LoI} into essentially two distinct \kl{icons}, \emph{binders} and
...
pair of a Frobenius algebra.}, from which every \kl{LoI} can be reconstructed:
...
\begin{description}
  \item[Binder] As we have just seen, one function of \kl{LoI} is to \emph{quantify}
  over individuals. For now we have only considered \kl{LoI} located at the top-level
  ...
  particular, the location of a \kl{LoI} should be identified with its
  ...
  \kl{LoI} is fully captured by its location: existential in a \kl{positive} area,
  universal in a \kl{negative} area. This leads us to analyze the syntax of \kl{LoI} into
  two components: so-called \emph{binders} that encode \emph{quantifiers} as
  distinguished heavy dots on $\SA$ like those of
  \reffig{eg-forall}\sidenote{Peirce liked to insist that \kl{LoI} should be drawn as
...
\kl{LoI} (or their decomposition into the above constructs) constitute the only \kl{icons}
...
The fact that Peirce took \kl{LoI} to be a primitive, unanalyzed \kl{icon} can be seen as
...
expositions, Peirce always required every hook to be filled with a \kl{LoI}, that is
...
to wires, as Peirce does with \kl{LoI}: then one can replace every binder by a
...
couldn't we just reduce the full syntax of \kl{LoI} to wires? A remarkable insight of
...
by seeing \kl{LoI} as made out of \emph{pipes} rather than wires. Indeed, just gluing
...
pipes\sidenote{If one wants to keep the wire \kl{metaphor}, joining two \kl{LoI} can be
...
Then every \kl{LoI} can be built out of teridentities, which Peirce expressed like
...
\kl{Alpha} to account for \kl{LoI}\sidenote{Thus in a sense, the \kl{first-order}
individuals denoted by \kl{LoI} behave in the exact same way as the propositions
...
\begin{description}
  ...
  copy must be connected to the same hook of the original copy with a \kl{LoI}.
  Conversely, when a spot is deiterated, every \kl{LoI} of the deleted copy must be
  ...
  \kl{Insertion} and \kl{Deletion} on \kl{LoI} can be understood as capturing
  respectively the operations of \emph{anti-unification} and \emph{unification}
  on two variables. That is:
  \begin{description}
    \item[Unification] adding a wire between two disconnected binders $x$
  and $y$ is equivalent in purpose to substituting $x$ for $y$ (resp. $y$ for
  $x$) in every spot/predicate connected by a \kl{LoI} to $y$ (resp. $x$);
    \item[Anti-unification] while severing a wire connected to a binder
...
correspond to splits, merges and rewirings of \kl{LoI}, and a more rigorous set of
...
indeed we should only need \kl{Deletion} on \kl{LoI}, which connects already
...
resulting from the composition of \kl{LoI}; or at the level of \emph{proofs}, with a
...
tedious management of \kl{LoI}. Our analysis into binders and teridentities gives us
...
simple:
\begin{enumerate}
  \item Take any complex \kl{LoI} connected to $n$ binders and $m$ hooks;
...
\kl{LoI} crossing \kl{cuts} \cite[p.~531]{peirce_prolegomena_1906}:
\begin{quote}
...
EGs such as Quine, who devised a notation similar to \kl{LoI}, and deemed it ``too
...
their hooks by sending water through the (now invisible) \kl{LoI}, seen as hoses.
...
remains to account for \kl{Insertion} and \kl{Deletion} on \kl{LoI}, which is done
...
\begin{remark}
  ...
  $$\bcut{\garden{{}}{\bcut{\garden{x}{\mathsf{TheSittingPerson}(x),\mathsf{Socrates}(x)}}}}$$
