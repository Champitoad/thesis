...
algebra of relations and quantification \sidecite{peirce_algebra_1885}. But far
less widespread are his achievements in the realm of graphical logic, or
\emph{\kl{iconic} logic} as Shin calls it \sidecite{10.7551/mitpress/3633.001.0001}.
...
the \kl{existential graphs} (\kl{EGs}), which he developed from 1896 until his
death in 1914 \sidecite{Roberts+1973}. Interestingly, Peirce perceived
\kl{existential graphs} as his \textit{``chef d'oeuvre''}, and that they
\textit{``ought to be the logic of the future''}\sidenote{Both citations are
sourced in \cite[p.~11]{Roberts+1973}.}.
...
Melliès, Zeilberger \sidecite{mellies_bifibrational_2016} and Bonchi, Di Giorgio
...
Hilbert and Gentzen proof systems, and even deep inference proof systems like
the calculus of structures. This shall prove useful to get a good understanding
of the historical and technical foundations behind our \emph{flower calculus},
to be introduced in \refch{flowers}.

The chapter is organized as follows: we start in \refsec{alpha} by presenting
the diagrammatic syntax of the system \kl{Alpha} of \kl{EGs} for classical
propositional logic. In \refsec{illative}, we introduce the inference rules of
\kl{Alpha} for manipulating \kl{existential graphs}, called \emph{illative
transformations} by Peirce. In \refsec{multisets}, we give an equivalent
formulation of the syntax and rules of \kl{Alpha} as a \emph{multiset} rewriting
system. In \refsec{atomicity}, we formalize a variant of the (de)iteration
principle described by Peirce in \sidecite{peirce_prolegomena_1906} that
eliminates the need for the double-\kl{cut} principle, and discuss how it was
...
proof of soundness for \kl{Alpha}, based on a direct truth-evaluation of graphs.
In \refsec{eg-completeness} we give a syntactic proof of completeness for
\kl{Alpha}, by simulating the calculus of structures \intro{SKS} of Brünnler and
Tiu \sidecite{brunnler_local_2001}. In this way, \kl{Alpha} is shown to have
subsystems that inherit the \emph{locality} property of \kl{SKS}, and where the
deletion and insertion rules are respectively admissible for \emph{provability}
and \emph{refutability}, making \kl{Alpha} \emph{analytic}. In \refsec{beta}, we
illustrate the original mechanism of \emph{lines of identity} used by Peirce to
handle quantifiers and equality in his \kl{Beta} system. We end in
\refsec{gardens} by showing how to recast lines of identity in a more
...
built, and can today be understood as a diagrammatic calculus for classical
...
\sidecite{zeman_graphical_1964} and \emph{higher-order} logics.
...
graphical depiction of the \emph{status} of solutions in \reffig{graphical-B},
...
display. In a proof assistant, this would either be the buffer of a text editor
...
that one can perform as much computation as needed. In symbolic logic, this is
...
denote vacuous truth, traditionally symbolized by the formula $\top$.
...
given to instances of the \kl(rule){cut} rule in sequent calculus.}, because
...
negations and built from atomic propositions. Importantly, the only symbols
needed for doing so are letters $a, b, c\ldots$ denoting atomic propositions,
that is ``pure'' symbols that do not have any logical meaning associated to
them.
...
classically equivalent formula, as has been shown for instance in
...
\sidecite{minghui_graphical_2019}, and is captured in traditional proof theory
...
\begin{remark}\labremark{eg-entitative}
  ...
  \kl{juxtaposition} to denote \emph{disjunction} instead of conjunction. Although
  $\{\lor,\neg\}$ is also functionally complete, Peirce quickly grew unsatisfied
  with these entitative graphs, stating that \kl{EGs} formed ``a far preferable system
...
\AP In order to have a proof system, one needs a collection of \emph{inference
rules} for deducing true statements from other true statements. In \kl{Alpha},
inference rules are implemented by what Peirce called \intro{illative
transformations} on \kl{graphs}. In modern terminology, they correspond to
\emph{rewriting} rules that can be applied to any \kl{subgraph}. By measuring
the depth of a \kl{subgraph} as the number of \kl{cuts} in which it is enclosed,
we thus have that the rules of \kl{Alpha} are applicable on \kl{subgraphs} of
arbitrary depth. This makes \kl{Alpha} deserving of the title of \kl{deep
inference} system.
...
\emph{polarity} of any \kl{subgraph} $G$, understood as the \emph{parity} of the
...
\begin{remark}\labremark{eg-polarity}
  ...
  opposite colors (blue and red), the previous notations for \kl{graphs} suggest that
...
\begin{itemize}
  \item[\textbf{Insertion}]
    Any \kl{graph} $G$ may be inserted in a negative area:
    $$\nsheet{~~~\step{}~~~ G}$$
  ...
  \item[\textbf{Deletion}]
    Any \kl{graph} $G$ occurring in a positive area may be erased:
    $$G ~~~\step{}~~~$$
...
\begin{itemize}
  \item[\textbf{Double-\kl{cut}}]
    ...
    is not true intuitionistically. Topollogically, the double-\kl{cut} forms a
...
premiss, starting from the goal to prove --- here the \kl{graph} associated to the
...
(\refdef{solution}). The main difference between \kl{graphs} and solutions,
...
The five rules of \kl{Alpha} can now be formalized as multiset rewriting rules
...
\begin{definition}[Context]
  ...
  always be \emph{filled} (substituted) with any other \kl{graph} $H$ or context
  $K\hole$, producing a new \kl{graph} $\cfill{G}{H}$ or context $\cfill{G}{K\hole}$.
  In particular, filling with the empty \kl{graph} $\emptyset$ will yield a \kl{graph}
...
a set of unary inference rules on \kl{graphs}: when read \emph{top-down},
...
answer to the question of analyticity in \kl{Alpha} at the end of
\refsec{eg-completeness}.}. Here there is a conceptual \emph{bottleneck},
...
the lines of identity of \kl{Beta} handling predicate logic, to be introduced
...
calculus of structures \kl{SKS} in \kl{Alpha}.}. The atomic (de)iteration
...
\begin{description}
  ...
  informally for an intuitionistic variant of \kl{EGs}.}.
  ...
  context can be trivially performed by an instance of the \kl{Ins} (resp.
...
\emph{invertible} rules of \kl{Alpha}, that is those who satisfy $v(G) = v(H)$
for every valuation $v$ if $G \step{} H$.

\begin{lemma}[Iteration]\lablemma{eg-iteration}
  For every \kl{graph} $G$, context $H\hole$ and valuation $v$, we have
...
\begin{lemma}[Variance]\lablemma{eg-variance}
  ...
  For every context $K\hole$, \kl{graphs} $G, H$ and valuation $v$ such that $v(G)
  \leq v(H)$, we have:
  \begin{enumerate}
...
We propose in this section a proof by simulation of a \emph{deep inference}
system, more specifically the calculus of structures \kl{SKS} first introduced
...
they exemplify two distinct approaches to \emph{symmetry} in a deep inference
setting.
...
\begin{kaonote}
  ...
  \begin{description}
    ...
    section that \kl{Alpha} is \emph{analytic}. Ma and Pietarinen discuss this
    question very quickly in their paper, by affirming that \kl{Alpha} is
...
\begin{figure*}
  ...
  \caption{Inference rules of \kl{SKS}}
  \labfig{sks}
\end{figure*}
...
\begin{description}
  \item[Symmetry] 
    ...
    obtain dual rules in the intuitionistic calculus of structures \kl{SISa} of
...
\end{lemma}
\begin{proof}
  Since all rules of \kl{Alpha} apply in a context of unbounded depth, we know
  that there are some \kl{graphs} $G_0, H_0$ and context $K'\hole$ such that $G =
...
\begin{proof}
  ...
  Then onto weakening $\{\kl{aw{\da}},\kl{aw{\ua}}\}$ and
  contraction
  ...
  For the switch rule \kl{s}, we give two dual derivations: the first uses the
...
medial rules \kl{s} and \kl{m}. Thus deletion is admissible in \kl{Alpha}, a
...
Corollary \ref{cor:adm-era} is what allows us to conclude that \kl{Alpha} is an
...
reduce the subformula property to \emph{atomic} \kl{graphs}. Then it is easy to see
...
% Thanks to \reffact{eg-duality}, it can easily be shown that \kl{Alpha}
...
% \kl{Dcut{\da}}: indeed, from $[H] \steps{} [G]$ one gets $[[G]] \steps{} [[H]]$
% through the \emph{if} direction, and then $G \xstep{\kl{Dcut{\ua}}} [[G]]
% \steps{} [[H]] \xstep{\kl{Dcut{\da}}} H$.

% When taking $H = \emptyset$, this property tells us that in \kl{Alpha},
% provability coincides with refutability. Now thanks to Corollary
% \ref{cor:adm-era}, we know that provability in $\kl{Alpha} \setminus
% \{\kl{Del}\}$ coincides with provability in \kl{Alpha}, and thus with
% refutability in \kl{Alpha}, which thanks to Corollary \ref{cor:adm-ins}
% coincides with refutability in
% $\kl{Alpha}\setminus\{\kl{Ins},\kl{Dcut{\da}}\}$. Hence provability
...
Before working on \kl{EGs}, Peirce had already developed a deep understanding of the
...
However in his system \kl{Beta} of \kl{EGs}, Peirce gives a very different account of
...
we illustrate informally the principles of \kl{Beta}, and how they are able to
...
\paragraph{Spots} In the propositional system \kl{Alpha}, atomic \kl(alpha){graphs}
...
\begin{description}
  ...
  \begin{remark}\labremark{eg-forall-type-theory}
    ...
    captured by \kl{Beta}, the graphical representation makes this fact more
...
\kl{Alpha} to account for LoI\sidenote{Thus in a sense, the first-order
individuals denoted by LoI behave in the exact same way as the propositions
denoted by the \kl{graphs} of \kl{Alpha}. This can be interpreted as a manifestation
...
\begin{description}
  ...
  is remarkable that they constitute a core illative principle of \kl{Beta}.

\end{description}

\begin{remark}
In the original \kl{Beta} system, Peirce enforces the usual model-theoretic
...
of proof for \kl{Alpha} (\refdef{eg-proof}), these are the only axiom in all
...
contrary to \kl{Alpha}, it seems that \kl{Deletion} is not admissible in
\kl{Beta} anymore. Because of the \emph{subterm property} of first-order logic
...
illative transformations through direct manipulation of \kl{graphs}. In the words of
...
Still, the complexity of \kl{Beta} from a UX perspective stems mostly from the
...
and \kl{cuts}). Anticipating our flower metaphor of \refch{flowers}, we call
...
\begin{remark}
  As in \kl{Alpha}, \kl{graphs} $G, H, K$ and their one-holed contexts
