...
deletion and insertion rules are respectively \kl{admissible} for \emph{provability}
and \emph{refutability}, making \kl{Alpha} \emph{\kl{analytic}}. In \refsec{beta}, we
...
\begin{remark}\labremark{eg-polarity}
  Whereas in \kl{bubble calculi} the concept of polarity was understood as a property
...
the definitions of \kl{bubbles} (enclosures) and solutions (areas)
...
\begin{definition}[Polarity]
  We say that a \kl{context} $G\hole$ is \emph{positive} if $\inv(G\hole)$ is
  even, and \emph{negative} otherwise. We denote positive and negative \kl{contexts}
...
\begin{description}
  ...
  justified \kl{context} must be positive (resp. negative), while it can have an
  arbitrary polarity in the original rules. This is expressed by splitting each
  of the latter into two rules, one where the outer \kl{context} $K\hole$ is positive
...
\begin{remark}\labremark{sks-ctx-pol}
  \kl{Contexts} for structures are always \emph{positive}, since negation is pushed
...
polarity of the \kl{context} in which they apply.
\end{remark}
...
\begin{description}
  \item[Symmetry] 
    ...
    duality is captured in the \emph{polarity} of \kl{contexts} rather than through
    De Morgan's laws:
    ...
    Contrary to De Morgan duality, the notion of polarity of a \kl{context} also
...
the \kl{hole} is translated as itself) will always be positive:

\begin{fact}\labfact{strans-pos}
  For every \kl{context} $S\hole$, $\strans{S\hole}$ is positive.
\end{fact}
\begin{proof}
...
positive) \kl{context}, the former (resp. latter) action corresponds to forgetting a
