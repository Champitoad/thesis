...
meaning of $\in$ in a set theory such as \kl{ZF} comes from the various axioms
...
\begin{description}
  \item[In \kl{ZF} set theory]
    ...
    and $(1)$ can interact through the following forward \kl{DnD}:
...
\end{itemize}
Then we can use injectivity with the following forward \kl{DnD} between $(0)$ and
$(3)$:
...
\end{itemize}
doing a backward \kl{DnD} with $(5)$ to rewrite $x$ in the conclusion into $a$:
$$a \in C \land \select{x} = a ~~\back~~ \select{x} \in C$$
and finally a backward \kl{DnD} with $(5)$ to conclude the proof:
...
$(c)$, which we can use through the following backward \kl{DnD}:
...
$(1)$, with the following backward \kl{DnD}:
