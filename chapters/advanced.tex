...
\begin{kaonote}
  ...
  the structure of the graphical proof, for the sake of comparison with a more
  standard text-based interface. But this would obviously compare differently
  with other textual interfaces like the \kl{Isar} proof language, which is more
...
\begin{itemize}
  ...
  the blue \kl{items} involved in an action. It appears clearly in
...
predicate symbol, the definition's \intro{head}, and a universally parametrized
equality or equivalence between the \kl{head} and the \intro{body} of the definition.
...
point at different subterms appearing in the \kl{goal} --- either in the \kl(sequent){context} or
...
\cdot$, and thus will replace the \kl{head} of this definition by its \kl{body},
...
\kl{axioms} involving it. One might call such a definition \intro{behavioral},
...
definition we tackle in this chapter might be called \intro{nominal}, because it
essentially amounts to giving a name (the \kl{head}) to a reoccurring pattern of
statement (the \kl{body}). Note that the syntax of \kl{first-order logic} is
unaware of this distinction, since in both cases the defined concepts are
encoded as \emph{atomic} predicates. This is usually not the case of logical
frameworks found in \kl{proof assistants}: for instance, the duality between
\emph{judgmental} (\kl{nominal}) and \emph{propositional} (\kl{behavioral}) equality is at
the heart of \kl{Martin-Löf type theory}, and it is used extensively in \kl{Coq}
to perform automation, both in the computation of expressions and the matching
of statements modulo definitions.}. But we can still unfold some definitions in
the \kl(sequent){context}, which might suggest further possible interactions.
First we can unfold the definition of preimage used in hypothesis~\ref{hyp::1}
by selecting the precise corresponding subterm:
...
\begin{description}
  \item[In \kl{ZF} set theory]
    ...
    click on the lemma she is interested in, in order to add it as a blue \kl{item}
    ...
    \emph{patterns}, i.e. \kl{terms} with \kl{holes} or so-called \emph{metavariables}, in
  ...
  \kl{MLTT}, it is merged with \kl{nominal} definitions in the concept of judgmental
  equality. In \kl{Coq} they are distinguished: computational and \kl{nominal} definitions
...
Going back to the proof, we now have the following \kl(sequent){context} of hypotheses:
\begin{itemize}
  \item[(0)] $\injective(f)$
  \item[(2)] $f(x) \in f(C)$
\end{itemize}
...
schemes as blue \kl{items}, in the same way we did for the axiom of comprehension in
...
In the first case, one can access the contextual action in \kl{Actema} by
...
contextual action.}. This could also work by selecting any occurrence of the
...
also be of an \emph{inductive} \kl{type}, i.e. one which is mapped to an
induction scheme in the system\sidenote{The standalone version of \kl{Actema}
only supports induction on natural numbers, but in a \kl{proof assistant} like
\kl{Coq} or Lean one can easily check if a given \kl{type} is inductive.}. Then
the \action{Induction} action will simply apply the associated induction scheme.
