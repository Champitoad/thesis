...
The rules of the \kl{flower calculus} have an interesting property: they are mostly
\emph{arity-agnostic}, i.e. they work uniformly on flowers, \kl{bouquets} and gardens
with any number of \kl{petals}, flowers and \kl{binders}. In particular, this means that
...
\kl{petals}) prevents $\forall$ from being expressible (i.e. provable) without
...
relate \kl{equivalent} \kl{bouquets}. Indeed, as soon as $\flower{\Psi}{\Phi}$
...
\kl{pollination} rules $\{\text{\kl{poll{\da}}}, \text{\kl{poll{\ua}}}\}$ and
...
mirroring the distinction between \kl{cross-pollination} and \kl{self-pollination}, but
...
\begin{description}
  \item[Generalization of EGs]
    ...
    flower with no \kl{petals}). Similarly, it is well-known that \kl{Kripke structures}
    enable a natural generalization of truth valuations, where \kl{classical}
    valuations are those limited to ``unary'' structures with a single possible
    \kl{world}\sidenote{It is tempting to draw a parallel between \emph{possible
    \kl{worlds}} and the \emph{\kl{petals}} of flowers, where the \kl{accessibility} relation
    $\access$ between \kl{worlds} might be reflected to some extent in the
    \kl{pollination} relation illustrated in \reffig{pollination} (or more
    accurately, in a justification relation between \kl{contexts} in the style
    of \refdef{eg-local-justification}). In particular, transitivity of
    $\access$ could correspond to the possibility of \kl{cross-pollinating} a \kl{petal}
    by first \kl{cross-pollinating} the \kl{pistil} to which it is attached, and then
    performing \kl{self-pollination}. An analogy between possible \kl{worlds} and
...
\begin{definition}[$w$-evaluation]
  ...
  $\mathcal{K}$, a \intro*\evaluation{w} is a function $e : \vars \to M_w$.
  % We write $e : \bx \to M_w$ if $e(x) = \ast$ for every $x \not\in
  % \bx$ and some distinguished element $\ast \in M_w$.
  The \kl{interpretation} map of $M_w$ is extended to terms and \kl{substitutions} with
...
%   For any closed flower $\phi \in \closed{\flowers}$ and \evaluation{w}s $e,
...
%   $\eforces{w}{\phi}{e}$ for some \evaluation{w} $e$.
% \end{corollary}
...
formalism. It requires an induction principle for \emph{\kl{contexts}}:
      ...
      \begin{description}
        ...
        \item[Pistil]
          ...
          \begin{enumerate}
            ...
            $\eforces{w}{\rfillXi{}}{e}$. Let $w' \revaccess w$ and $e'$ a
            \evaluation{w'} such that
            $\eforces{w'}{\cfill{\Xi_0}{}}{\update{e}{\bx}{e'}}$. By IH we have
            $\Phi, \cfill{\Xi_0}{} \kentails{} \Phi, \cfill{\Xi_0}{\Phi}$, and
            thus by \reflemma{flowers-functoriality} $\rFillXi{\Phi,
            \cfill{\Xi_0}{\Phi}} \kentails{} \rFillXi{\Phi, \cfill{\Xi_0}{}}$.
            By \reflemma{flowers-weakening} and \reflemma{flowers-functoriality}
            we have $\eforces{w}{\rFillXi{\Phi, \cfill{\Xi_0}{\Phi}}}{e}$, and
            thus $\eforces{w}{\rFillXi{\Phi, \cfill{\Xi_0}{}}}{e}$. Then since
            $\eforces{w'}{\cfill{\Xi_0}{}}{\update{e}{\bx}{e'}}$, and since by
...
\begin{proof}
  ...
  \cfill{\Xi}{}$ for any flower $\phi$ and \kl{context} $\Xi\hole$: then
...
\begin{lemma}[Universal instantiation]\lablemma{flowers-univ-inst}
  ...
  If $\sigma : \by$ is \kl{capture-avoiding} in $\flower{\Phi}{\Delta}$,
...
\begin{lemma}[Existential instantiation]\lablemma{flowers-ex-inst}
  ...
  If $\sigma : \by$ is \kl{capture-avoiding} in $\Phi$, then
...
\begin{theorem}\labthm{flowers-soundness}
...
this technique to our \kl{flower calculus}, based on a completeness proof for
...
the \kl{forcing} relation on flowers and the rules of the \kl{flower calculus} differ
significantly from the \kl{forcing} relation on formulas and \kl{sequent calculus} rules.
...
Intuitively, a \kl{theory} $\tT$ is \consistent{\psi} when one cannot deduce $\psi$
...
%   \closed{\terms}$, if $T, \sigma(\Phi) \nentails \psi$ then there are some $1
...
\begin{proof}
  ...
  that is there is some \kl{bouquet} $\Phi \subseteq \completion{\tT}$ such that $\Phi
...
this also yields a strong completeness theorem for the full \kl{flower
calculus}\footnote{Actually it already works for the fragment $\Nature \cup
...
We identify two essential arguments inherent to any \kl{bouquet}:

\begin{definition}[Vehicle and Anchor]\labdef{vehicle} Let $\Phi$ a \kl{bouquet}, and
...
Intuitively, the vehicle of a \kl{bouquet} $\Phi$ is the set of possible
...
The vehicle and anchor of any \kl{bouquet} $\Phi$ can easily by computed, by noticing
...
the \intro{pollination phase}\sidenote{It corresponds roughly to a combination of
