...
nested sequents of \kl{deep inference} \kl{proof theory}, and the geometric/coherent
...
notion of proof geared towards analyticity results à la Gentzen, suggesting new
...
Furthermore, the kernel of rules targetted by completeness is fully invertible,
a desirable property in both automated and interactive proof search. This is
illustrated by our implementation of the Flower Prover, an early prototype of
...
we give an overview of the Flower Prover, a prototype of \kl{GUI} in the
...
accounting for it with blank space as is done in sequent notation or in \kl{EGs}; in
...
Contrarily to \kl{EGs} (\refdef{eg-inv}), the number of inversions of a context does
...
importing a statement (the source of the \kl{DnD}) into a new context (the
...
\emph{subformula linking} (\refch{sfl}): self-pollination and cross-pollination
correspond respectively to the \emph{\kl(dnd){backward}} $\back$ and
\emph{\kl(dnd){forward}} $\forw$ linking operators, which are collapsed into a
single interaction operator $\ast$ in the original formulation of subformula
...
that sequents capture only the \intro(implication){first-order} implicative
...
\kl{B}, we gave a \kl{bi-intuitionistic} (resp. \kl{classical}) semantics to bubbles in
...
\begin{itemize}
  ...
  \item[\textbf{Cut-free completeness}]
    ...
    well-known questions of \emph{proof normalization} (\kl{natural deduction}) and
    \emph{cut admissibility} (\kl{sequent calculus}). Gentzen originally proved cut
    admissibility through a syntactic procedure of \emph{cut-elimination}, which
    is very hard to transpose in our \kl{deep inference} setting, especially since
    there is no known internal cut-elimination procedure in the (sparse)
    literature on \kl{deep inference} systems for full \kl{intuitionistic} logic. In fact,
    ...
    \kl{sequent calculus} as in \sidecite{tiu_local_2006}; and we do have a working,
    verified implementation of this procedure in \kl{Coq} \cite{flowers-metatheory}.
    ...
    provable if it is the direct translation of a \kl{symbolic} formula. But formulas
    ...
    prove completeness of the analytic fragment of the \kl{proof system} with
...
invertible \kl{proof system} for \kl{intuitionistic} (predicate) logic that we know of is
...
the current \kl{goal}, until there is no more context in which it is
...
the \rsf{epet} rule in every applicable context in the \kl{goal}, which amounts to
...
interaction we are interested in is that of \emph{\kl{direct manipulation}} in a \kl{GUI}.
This was our initial motivation for studying the graphical formalism of \kl{EGs}, and
...
of \kl{GUI} in the \kl{Proof-by-Action} paradigm based on the \kl{direct manipulation} of
...
would make the \kl{subgoal} take up the whole screen, hiding the outer context from
...
In the \kl{proof view} of modern \kl{proof assistants} like \kl{Coq} and \kl{Lean},
there is no distinction between local and global contexts: a \kl{subgoal} will
inherit automatically every hypothesis from its parent \kl{subgoals}, which are
flattened into a big unstructured list. To recreate this distinction and reduce
the size of \kl{goals} to a manageable level, the only interface mechanism offered to
...
\emph{non}-invertible, but the calculus is \kl{contraction}-free. One advantage of
...
\kl{iconic} framework of the Flower Prover.
...
definition of analyticity for \kl{deep inference} \kl{proof systems} (especially
...
parts of the \kl{deep inference} literature concerned with analyticity and its
...
the Flower Prover: in the \kl{calculus of structures}, this role is fulfilled by
...
showed cut-elimination in \kl{sequent calculus}\sidenote{A sort of
...
cut-elimination proofs of \kl{calculi of structures}, which are indeed notoriously
involved.
...
Flower Prover with \kl{proof assistants} based on the latter such as \kl{Coq}, \kl{Lean} and
...
that emphasizes \kl{goal}-reduction through invertible and analytic rules, enabled us
