...
version of the algorithm seems to be neither sound (it does not terminate on
some goals) nor complete. Then in \refsec{flowers-prover} we give an overview of
the Flower Prover, a prototype of GUI in the Proof-by-Action paradigm whose
...
\begin{itemize}
  ...
  \item[\textbf{Cut-free completeness}]
    ...
    \cite{flowers-metatheory} and \refsec{app:flowers-coq}). However, we
...
%   $\eforces{w}{\psi}{e}$ where $\psi \coloneq
...
propositional logic is decidable, at least since Statman
\cite{statman1979intuitionistic}.}. Indeed in sequent calculi, analyticity (the
...
the sequent calculus \sys{G3IntQ} from Lyon's thesis \cite{lyon_refining_2021}
(we thank Lutz Stra{\ss}burger for pointing that to us). However as many systems
for modal logics, \sys{G3IntQ} needs to incorporate semantical notions like the
worlds and accessibility relation of Kripke structures into the syntax of
sequents in order to satisfy cut-elimination, and thus support efficient proof
search.}.
...
\begin{definition}[Justification]\labdef{justification}
  ...
  A \emph{justification} is an oriented pair of atomic occurrences $(\Phi\hole,
  a) \justi (\Psi\hole, b)$.
\end{definition}
...
\begin{definition}[Vehicle and Anchor]\labdef{vehicle} Let $\Phi$ a bouquet, and
  ...
  \begin{itemize}
    ...
    negative;
    \item[\textbf{(Interaction)}] $\Psi\hole \sinterneg \Psi'\hole$ and
    $\Psi\hole = \hole$.
  \end{itemize}
\end{definition}
...
The vehicle of any bouquet $\Phi$ can easily by computed, by noticing that the
context $\Xi\hole$ is simply the \emph{least common ancestor} of the atoms
$\Psi\select{a}$ and $\Psi'\select{a'}$ in the tree representation of $\Phi$,
which we write $\lca{\Psi\hole}{\Psi'\hole}$. This forms the basis for the first
\emph{phase} of our algorithm, called accordingly the \emph{pollination}
phase\sidenote{It corresponds roughly to a combination of the \emph{application}
and \emph{linking} phases of the proof search algorithm for \sys{B_{inv}}
described in \refsubsec{bubbles-search}.}. It is best described by the
following \emph{imperative} procedure, that modifies in-place
the goal represented by the input bouquet $\Phi$:

\DontPrintSemicolon
% \LinesNumberedHidden

\begin{procedure}[H]
  \caption{pollination($\Phi$)\labproc{phase-pollination}}
  \BlankLine

  \ForEach{edge $(\Psi\hole, a) \to (\Psi'\hole, a') \in \vehicle{\Phi}$}{
...
$\lor$ formulas.}. In fact even if the invertibility of \rsf{srep} makes it
sound in contexts of any polarity, we restrict its application to \emph{strictly
positive} contexts, i.e. contexts $\Xi\hole$ such that $\parity{\Xi\hole} = 0$.
Indeed as mentioned in \refsec{Calculus}, it has been designed as a
generalization of the left introduction rules for $\bot$ and $\lor$ in sequent
calculus, which like every sequent calculus rule work in the (strictly positive)
empty context. Thus we conjecture that this restriction should not impact
completeness. The reproduction phase can be computed by the following tail
recursive procedure:
...
\begin{procedure}[H]
  \caption{lifecycle($\Phi$)\labproc{lifecycle}}
  \BlankLine
...
Note that the specific order in which phases are sequenced does not matter.
Indeed the final algorithm, that we call the \refproc{life} procedure, is simply
the fixpoint of the \refproc{lifecycle} procedure:
...
\begin{figure*}
  \begin{mathpar}
    \begin{array}{rl}
    &(\flower{a}{(\flower{(\flower{a}{b\sep})}{b\sep})\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{a}{(\flower{(\flower{}{b\sep})}{b\sep})\sep})\\
    \xstep{\mathtt{R}} &(\flower{a}{(\flower{}{(\flower{b}{b\sep})\sep})\sep})\\
    \xstep{\mathtt{D}} &(\flower{a}{(\flower{}{(\flower{b}{b\sep})\sep})\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{a}{(\flower{}{(\flower{b}{\sep})\sep})\sep})\\
    \xstep{\mathtt{R}} &(\flower{a}{(\flower{}{(\flower{b}{\sep})\sep})\sep})\\
    \xstep{\mathtt{D}} &(\flower{a}{(\flower{}{\sep})\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{a}{(\flower{}{\sep})\sep})\\
    \xstep{\mathtt{R}} &(\flower{a}{(\flower{}{\sep})\sep})\\
    \xstep{\mathtt{D}} &(\flower{a}{\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{a}{\sep})\\
    \xstep{\mathtt{R}} &(\flower{a}{\sep})\\
    \xstep{\mathtt{D}} &
    \end{array}
    \and
    \begin{array}{rl}
    &(\flower{(\flower{}{a\sep b\sep})}{(\flower{}{b\sep a\sep})\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{(\flower{}{a\sep b\sep})}{(\flower{}{b\sep a\sep})\sep})\\
    \xstep{\mathtt{R}} &(\flower{}{(\flower{a}{(\flower{}{b\sep a\sep})\sep}), (\flower{b}{(\flower{}{b\sep a\sep})\sep})\sep})\\
    \xstep{\mathtt{D}} &(\flower{}{(\flower{a}{(\flower{}{b\sep a\sep})\sep}), (\flower{b}{(\flower{}{b\sep a\sep})\sep})\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{}{(\flower{a}{(\flower{}{b\sep \sep})\sep}), (\flower{b}{(\flower{}{\sep a\sep})\sep})\sep})\\
    \xstep{\mathtt{R}} &(\flower{}{(\flower{a}{(\flower{}{b\sep \sep})\sep}), (\flower{b}{(\flower{}{\sep a\sep})\sep})\sep})\\
    \xstep{\mathtt{D}} &(\flower{}{(\flower{a}{\sep}), (\flower{b}{\sep})\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{}{(\flower{a}{\sep}), (\flower{b}{\sep})\sep})\\
    \xstep{\mathtt{R}} &(\flower{}{(\flower{a}{\sep}), (\flower{b}{\sep})\sep})\\
    \xstep{\mathtt{D}} &(\flower{}{\sep})
    \\[1em]\hline\\
    \xstep{\mathtt{P}} &(\flower{}{\sep})\\
    \xstep{\mathtt{R}} &(\flower{}{\sep})\\
    \xstep{\mathtt{D}} &
    \end{array}
  \end{mathpar}
  \caption{Life traces for modus ponens (left) and identity expansion of disjunction (right)}
  \labfig{lifetrace-modus-ponens}
\end{figure*}

\paragraph{Failure of life}
...
the invertible bubble calculus in \refsubsec{bubbles-search}.}
...
\begin{description}
  \item[Intuitionistic EG]
    ...
    system is shorter than in Oostra's, thanks to the powerful \rsf{srep} rule.
  \item[Proof search]~\\
    \begin{itemize}
      ...
      \item Our notion of \emph{vehicle} (\refdef{vehicle}) takes its
      terminology from Girard, who started giving this name to the set of axiom
      links of a proof structure in his transcendental syntax\sidenote{Also, it
      conveys nicely the idea that the vehicle is the fundamental structure that
      \emph{drives} the proof search algorithm.} \sidecite{girard_2017}. But the
      ...
      independently by Bibel and Andrews in the 1970s \sidecite{Bibel:2009}.
      Otten and Kreitz have adapted the connection method to intuitionistic
      logic \sidecite{10.1007/3-540-59338-1_32}, stating that it is especially
      ...
      \sidecite{bruscoli_analyticity_2019}, Bruscoli and Guglielmo propose such
...
\begin{description}
  ...
  whether for bubble calculi (\refsubsec{bubbles-search}) or the flower calculus
  (\refsec{flowers-search}), rest on the fundamental observation pioneered by
  the subformula linking methodology of Chaudhuri \sidecite{Chaudhuri2013}, that
  ...
  procedure. In \refsubsec{bubbles-search}, we sketched some ideas that blur the
  ...
  further how to incorporate our drag-and-drop proof tactic (\refch{pba}), but
...
between proofs and types}''\sidenote{Another recent, related incarnation of this
phenomenon is the correspondence uncovered by Haydon between a linear version of
EG already appearing in Peirce's writings, and the proof nets of Girard for
linear logic \cite{haydon_eg_pn}.} \cite{miquel_implicative_2020}. Or as he put
