...
We introduce the \emph{flower calculus}, a novel \kl{proof system} for
...
the \emph{\kl{existential graphs}} of C. S. Peirce proposed by A. Oostra. Then we
present our inductive syntax for flowers, reminiscent at the same time of the
\kl{nested sequents} of \kl{deep inference} \kl{proof theory}, and the geometric/coherent
formulas of categorical logic. A salient feature of our calculus inherited from
\kl{EGs}, is that it is \emph{fully \kl{iconic}}: it dispenses completely with
the traditional notion of \kl{symbolic} formula, operating instead as a
\kl{rewriting system} on flowers containing only atomic predicates. We also propose a
notion of proof geared towards \kl{analyticity} results à la Gentzen, suggesting new
rules absent from other works on \kl{intuitionistic} \kl{EGs}. This allows us to
...
Furthermore, the kernel of rules targetted by completeness is fully \kl{invertible},
a desirable property in both automated and interactive proof search. This is
illustrated by our implementation of the \kl{Flower\,\,Prover}, an early prototype of
\kl{GUI} for \kl{ITPs} that uses the rules of the flower calculus both for \kl{direct
manipulation} of flowers in its frontend, and automated simplification of \kl{goal}s
in its backend.

The chapter is organized as follows: in \refsec{IEGs}, we retrace the origin of
Oostra's syntax for \kl{intuitionistic} \kl{EGs} (hereafter ``\kl{IEGs}'') as a
natural generalization of the \emph{scroll}, an \kl{icon} for implication introduced
by Peirce that inspired the very creation of \kl{EGs}. In \refsec{Flowers}, we
explain how flowers are really just a fun and \kl{metaphorical} way to draw
\kl{IEGs}, and proceed to give them an inductive, multiset-based syntax as in
\refsec{multisets}. In \refsec{Calculus}, we introduce the full set of \kl{inference
rules} of the flower calculus as well as our notion of proof, and prove a few
syntactic properties, including two deduction theorems. In \refsec{Semantics},
we give a direct Kripke semantics to flowers, avoiding the need for translations
to and from formulas. In \refsec{Soundness}, we show that the rules of the
flower calculus are valid with respect to our Kripke semantics, and in
\refsec{Completeness} we identify a complete fragment of the system where all
rules are both \emph{\kl{analytic}} and \emph{\kl{invertible}}. This entails the
admissibility of all rules outside of this fragment, and as a consequence the
\kl{analyticity} of the system. We exploit these properties in
...
algorithm is neither terminating nor complete. Then in \refsec{flowers-prover}
we give an overview of the \kl{Flower\,\,Prover}, a prototype of \kl{GUI} in the
\kl{Proof-by-Action} paradigm whose actions map directly to the rules of the flower
calculus, and which integrates nicely with (a restricted version of) our search
procedure. We conclude in \refsec{Conclusion} by a comparison with some related
...
\emph{\kl{juxtaposition}}, and \emph{\kl{cuts}} in $\SA$ that signify the denial or
...
the illative transformations of \kl{EGs} are a necessary consequence of the
...
Thus the conjunctive reading of \kl{juxtaposition} itself stemmed from ``a series of
...
insight that kickstarted the whole development: the discovery of the \kl{icon} called
...
This shows the \kl{classical} view of Peirce on \kl{EGs}, who interprets the scroll as
...
\kl{symbolically} by $A \limp B \defeq \neg A \lor B$. This is no coincidence that
Peirce based his most fundamental \kl{icon} on implication: according to Lewis
\sidecite[][p.~79]{Lewis1920-LEWASO-4}, he was the one who introduced the
``illative relation'' of implication into \kl{symbolic} logic in the first place, by
giving it a distinguished \kl{symbol}, and studying extensively the algebraic laws
...
Antecedant} in \cite{minghui_graphical_2019}, and corresponds \kl{symbolically} to
...
case of the Insertion principle of \kl{Alpha} (\refsec{alpha}). However, we
stress again that Peirce first identified this clause as a feature of the
scroll, seen as the \kl{diagrammatic} embodiment of the ``general form of argument''
...
nested \kl{cuts} \cite[p.~535]{peirce_prolegomena_1906}:
...
To emphasize this point, we will from now on depict scrolls as two nested \kl{cuts}
...
\kl{cuts}. If we reject this interpretation by requiring that the Fence (the
...
conclusion, rather than \emph{\kl{backward}} proofs (\refdef{eg-proof}) that rewrite
a \kl{goal} into the empty $\SA$, as we usually did in \refch{eg}. \kl{Forward} proofs
correspond to Peirce's usage of the illative transformations --- and thus to
what can be found in most of the literature on \kl{EGs}, and have the advantage
of being more economical in space by leaving the \kl{goal} implicit. One can easily
go from a \kl{forward} proof to a \kl{backward} one as shown by the \emph{deduction
theorem} of Sowa \cite[Section 6]{sowa_peirces_2011}, which also applies in the
\kl{intuitionistic} setting by substituting the rule of \kl{Double-cut} with the rule
of Blank Antecedant.
\end{remark}
...
seriously, by reifying the scroll as a primitive \kl{icon} of \kl{EGs} (``\emph{rizo}'' in
Spanish), that exists alongside the \kl{cut} (``\emph{corte}''), and is distinguished
from it. In fact he goes further than this, and proposes to generalize both the
\kl{cut} and the scroll into an $n$-ary construction called the \emph{curl}
...
the loops of Oostra) called the \emph{inloops}. Then \kl{cuts} and scrolls are indeed
...
neutral element $\bot$. This coincides with the \kl{intuitionistic} reading of
negation $\neg A \defeq A \limp \bot$, and is thus consistent with the
interpretation of \kl{cuts} as negations.
...
With this interpretation of the $n$-ary scroll, the \kl{Alpha} encodings of
disjunction and implication as nested \kl{cuts} are no longer valid, because they are
not \kl{intuitionistically} equivalent to the associated binary and unary scrolls.
This is illustrated in \reffig{eg-disj-imp}, where the closeness in meaning is
reflected \kl{iconically} (but not \kl{symbolically}) in the fact that the graphs only
differ in the \emph{continuity} (or lack thereof) between inloops and their
outloop. Indeed, contrary to nested \kl{cuts}, any $n$-ary scroll can be drawn by a
...
\kl{intuitionistic} logic, such as the well-known Stone-Tarski interpretation of
...
\kl{IEGs}, Zalamea gave a detailed analysis of Peirce's philosophy of
the \emph{continuum}, how it relates to modern developments in mathematics, and
how it is embodied in \kl{existential graphs} \cite{zalamea_peirces_2003}. Actually
according to Oostra \sidecite[][p.~162]{oostra_advances_2022}, ``the
possibility of developing \kl{intuitionistic} existential graphs was first suggested
...
obtained by enclosing the previous graphs in a \kl{cut} are interpreted with the
...
\kl{intuitionistic} interpretation of $n$-ary scrolls, where the
...
difference however: the full syntax of \kl{IEGs} allows for arbitrary \emph{nestings}
...
Coherent formulas have some nice properties, which might also apply to \kl{IEGs} to
some extent. We only mention two important ones:
\begin{description}
  \item[Completeness] Every \kl{first-order} theory has a coherent conservative
  ...
  of \kl{FOL}; and avoiding complex encodings (as can be found e.g. in SMT solvers)
  is crucial in \emph{interactive} theorem proving, where the user and the
  computer manipulate the same formulas in \kl{goal}s
...
As we have seen, the ($n$-ary) scroll is a powerful \kl{icon}, because it captures
the distinction between \kl{classical} and \kl{intuitionistic} logic as being a matter of
...
later in the denotational semantics of the \kl{$\lambda$-calculus}. However as a
\kl{diagrammatic} component to be operated upon through \kl{direct manipulation}, it has
one notable flaw, also shared with the \kl{classical} \kl{cut}-based syntax: it quickly
...
\kl{icon} is reminiscent of a \emph{flower}, as if it had bloomed from its curled
...
deterministic relations, one can in principle formalize any \kl{first-order} theory
...
mathematics in \emph{\kl{type theories}}.}. However it is much more convenient to
...
accounting for it with blank space as is done in \kl{sequent} notation or in \kl{EGs}; in
...
\begin{definition}[Substitution]
  ...
  $\sigma$ whose support is $\bx$. The domain of \kl{substitutions} is extended to
...
rules} of our flower calculus, just as we did for \kl{Alpha}
(\refsec{multisets}) and \kl{Beta} (\refsec{gardens}). There, graphs and their
\kl{contexts} were defined as multisets of nodes, which have now turned into bouquets
of flowers:

\begin{definition}[Context]
  A \intro{context} $\Phi\hole$ is a bouquet which contains exactly one
  occurrence of a special flower written $\hole$, called its \reintro{hole}. The
  \kl{hole} can always be \emph{filled} (substituted) with any other bouquet
  $\Psi$ or \kl{context} $\Xi\hole$, producing a new bouquet
  ...
  removed. A \emph{flower context} $\phi\hole$ is a \kl{context} with exactly one
  flower.
\end{definition}

\begin{definition}[Depth]
  The \emph{depth} $\cdepth{\Phi\hole}$ of a \kl{context} $\Phi\hole$ is
...
not coincide with its depth, since petals increase depth but preserve \kl{polarity}:

\begin{definition}[Inversions]
  The number of \emph{inversions} $\inv(\Phi\hole)$ of a \kl{context} $\Phi\hole$ is
...
\begin{definition}[Pollination]\labdef{pollination}
  ...
  We say that a flower $\phi$ can be \emph{pollinated} in a \kl{context} $\Phi\hole$,
  written $\chyp{\phi}{\Phi\hole}$, when there exists a bouquet $\Psi$ with
  $\phi \in \Psi$ and \kl{contexts} $\Xi\hole$ and $\Xi_0\hole$ such that either:
  \begin{itemize}
...
We now employ the \kl{metaphor} of \emph{pollination} to speak about (de)iteration in
...
the flow of information in \kl{EGs}, and \reffig{bubbles-porosity} in bubbles.
From a UI point of view, all these figures can be understood as kind of
...
\kl{classical} \kl{EGs}, because pistils and petals are both identified as
instances of \kl{cuts}\sidenote{The same phenomenon is at work in
\emph{\kl{subformula linking}} (\refch{sfl}): self-pollination and cross-pollination
correspond respectively to the \emph{\kl(dnd){backward}} $\back$ and
\emph{\kl(dnd){forward}} $\forw$ \kl{interaction operators}, which are collapsed into a
single interaction operator $\ast$ in the original formulation of \kl{subformula
linking} for classical linear logic \cite{Chaudhuri2013}.}. If we were to replace
binders by LoI (lines of identity, see \refsec{beta}), then the pollination
relation would also prescribe in which areas LoI can be extended/iterated,
providing an explanation for the scope of binders (\refremark{flower-scope}).

\paragraph{Rules}

As has become standard in this thesis, we define the flower calculus as a
\emph{\kl{rewriting system}} on bouquets, presented in \reffig{flower-calculus} as a
...
\begin{description}
  \item[Blank Antecedant (\kl{epis})]
    ...
    \textsf{(pis)}til. This is one direction of the rule \kl{BA} of
    (\reffig{rule-empty-antecedant}), which is a weaker, \kl{intuitionistic} version
    of the \kl{classical} rule \kl{Dcut{\ua}} of \kl{Alpha} and \kl{Beta}. The
    other direction (rule \kl{epis{\da}} in \reffig{flowers-episda}) is
    actually \kl{admissible}, which might be related to the co-admissibility of
    \kl{Dcut{\da}} in \kl{Alpha} (\refcor{adm-ins}).

  \item[(De)iteration (\kl{poll{\da}}, \kl{poll{\ua}})]
    Renamed \emph{\textsf{(poll)}ination} rules, they correspond to the rules
    \kl{Iter} and \kl{Deit} of \kl{Alpha} and \kl{Beta}, but reformulated
    ...
    bouquet $\Phi$ to be \emph{scattered} in the \kl{context} $\Xi\hole$, i.e. its
  ...
  \item[Insertion/Deletion (\kl{grow}, \kl{crop}, \kl{pull}, \kl{glue})]
    ...
    distinction between pistils and petals. More precisely, rules \kl{grow} and
    \kl{crop} allow to insert and delete entire flowers, while rules \kl{pull}
    and \kl{glue} deal with petals. As for pollination rules, manipulating
  ...
  \item[Unification (\kl{ipis}, \kl{ipet}, \kl{apis}, \kl{apet})]
    Rules \kl{ipis} and \kl{ipet} allow to \textsf{(i)}nstantiate a sprinkler
    located respectively in a \textsf{(pis)}til ($\forall$) and a
    \textsf{(pet)}al ($\exists$) with an arbitrary \kl{substitution}, while rules
    \kl{apis} and \kl{apet} do the opposite operation of
    ...
    becomes an arbitrary \kl{substitution} $\sigma$. Once again, we have twice the
    amount of rules to account for the pistil/petal distinction, which is not
    surprising since in the LoI syntax of \kl{EGs}, they are special cases of
    Insertion/Deletion. Note that for the instantiation rules
    \kl{ipis}/\kl{ipet} to be \kl{invertible}, we duplicate the whole flower/petal
    where the sprinkler occurs, mirroring what is done in multi-conclusion
    \kl{sequent calculi} (see \reffig{multi-inst}).
\end{description}
...
of \kl{intuitionistic} \kl{predicate logic}, understood as any subset $\mathfrak{F}
\subset \flowers$ of the set of all flowers\sidenote{The only exception seems to
be the rule (\kl{ipis}) (resp. \kl{ipet}), whose duplication of flowers (resp.
...
polarizing the \kl{context} of application as for the rule \kl{apis} (resp.
\kl{apet}), at the cost of making the rule non-\kl{invertible}.}.
\reftab{flowers-fragments} shows how all the usual \kl{symbolic} connectives can be
...
\begin{definition}[Derivation]
  ...
  \Psi$. Finally, natural derivations are closed under arbitrary \kl{contexts}: for
  every \kl{context} $\Xi\hole$, $\Phi \step{\Nature} \Psi$ implies
  $\cfill{\Xi}{\Phi} \step{\Nature} \cfill{\Xi}{\Psi}$. We write $\Phi
  \lstep{\Nature} \Psi$ to denote a \emph{local} natural step, i.e. a direct
  instance of a natural rule in the empty \kl{context}.
\end{definition}
...
In Peircean terms, the empty bouquet is the blank \kl{sheet of assertion}, which as
we have seen in \refsec{alpha} can be interpreted as the \kl{icon} of \emph{absolute
truth}. Then in our proof construction paradigm, proving a bouquet amounts to
erasing it completely from the \kl{sheet of assertion}, thus reducing it to absolute
truth.
...
direction relies on the $\Culture$-rule \kl{grow}, a close analog to the cut
rule in \kl{sequent calculus}. Also as already mentioned, the natural rules of the
flower calculus are both \emph{\kl{invertible}} and \emph{complete} for provability.
...
\begin{definition}[Hypothetical provability]
  ...
  \entails{\mathsf{R}} \Phi$, if for every \kl{context} $\Xi\hole$ such that
...
of hypothetical provability, as found in Gentzen systems or \kl{type theories}: while
in these settings it is characterized as the existence of a proof for a
\emph{single} hypothetical \kl{judgment} $\Gamma \seq C$ which constrains the space
of derivations, here we have the stronger requirement that there exist proofs
for a \emph{class} of \kl{judgments} $\cfill{\Xi}{\Phi}$, whose hypothetical shape
comes from the condition that $\chyp{\Psi}{\Xi\hole}$. In practice, the
pollination rules $\{\text{\kl{poll{\da}}}, \text{\kl{poll{\ua}}}\}$ and
the {\kl{epis}} rule make this equivalent to the existence of a proof for
$\flower{\Psi}{\Phi}$. But we conjecture that the {\kl{epis}} rule might be
\kl{admissible} modulo the addition of the distributivity rule
\kl{crep}\sidenote{\kl{crep} stands for \textsf{c}ross-\textsf{rep}roduction,
mirroring the distinction between cross-pollination and self-pollination, but
with respect to the self-reproduction rule \kl{srep}.}\sidenote{It seems that
...
Both can be used to enable a local simulation of \kl{sequent calculus} rules, but do
...
\begin{itemize}
  \item[\textbf{Generalization of \kl{EGs}}]
    As mentioned in \refsec{IEGs}, the syntax of \kl{intuitionistic} \kl{EGs} (and thus of
    flowers) subsumes that of Peirce's \kl{classical} \kl{EGs}, by seeing the \kl{cut} as a
    ...
    where \kl{classical} valuations are those limited to ``unary'' structures with a
    ...
    more accurately, in a justification relation between \kl{contexts} in the style
    ...
    \kl{intuitionistic} disjunction is also proposed by Girard in
  ...
  \item[\textbf{Cut-free completeness}]
    ...
    respect to some semantic models (in our case, $\vDash \phi$ implies
    ...
    the models used are most of the time Kripke structures\sidenote{We know of
...
\begin{definition}[$w$-evaluation]
  ...
  The interpretation map of $M_w$ is extended to terms and \kl{substitutions} with
...
\emph{functoriality} lemma at the basis of every \kl{deep inference}
system\sidenote{Other instances in this thesis are \reflemma{variance},
\reflemma{bubbles-functoriality}, \reflemma{bubbles-cofunctoriality}, and
\refcor{eg-functoriality}.}:
...
\begin{lemma}[Functoriality]\lablemma{flowers-functoriality} If $\Phi \kentails{}
  ...
  \Xi\select{\Psi}$ if $\Xi\hole$ is \kl{positive}, or $\Xi\select{\Psi} \kentails{}
  \Xi\select{\Phi}$ if $\Xi\hole$ is \kl{negative}.
\end{lemma}
\begin{proof}
  By induction on $\cdepth{\Xi\hole}$.
\end{proof}

Weakening lemmas are also standard in \kl{proof theory} and \kl{type theory}. Here they
are purely semantic, but they will participate in the admissibility of the
\kl{weakening} rules $\{\kl{grow},\kl{crop},\kl{pull},\kl{glue}\}$:
...
$\{\kl{poll{\da}},\kl{poll{\ua}}\}$, because of the arbitrary \kl{context} $\Xi$
and reliance on the pollination relation. The following lemma can be compared to
\reflemma{eg-iteration} for the (de)iteration rules in \kl{EGs}:
...
\begin{proof}
  ...
  \Xi\select{\phantom{\phi}}$ for any flower $\phi$ and \kl{context} $\Xi\hole$: then
  ...
  By \refdef{pollination}, there are a bouquet $\Psi$ and two \kl{contexts} $\Xi',
...
Proving the soundness of rules involving binders (\kl{ipis}, \kl{ipet},
\kl{apis}, \kl{apet}) is also quite tedious, which can be understood as
stemming from the fact that \kl{substitutions} simulate the complex dynamics of the
lines of identity of \kl{EGs} in a \emph{global} rather than local way. In particular,
one needs to be careful about the scope of bound variables, which in \kl{EGs} would be
...
prove more: that every $\Nature$-rule is \emph{\kl{invertible}}, i.e. its conclusion
entails its premiss.
...
\begin{proof}
  ...
  \begin{itemize}
    \item[\textbf{(\kl{poll{\da}}, \kl{poll{\ua}})}]
      By \reflemma{flowers-pollination}.
    
    \item[\textbf{(\kl{epis})}]\sbr
    \begin{enumerate}
    ...
    \item[\textbf{(\kl{ipis})}] We trivially have $\flower{\garden{\bx,\by}{\Phi}}{\Delta}
      \kentails{}\flower{\garden{\bx,\by}{\Phi}}{\Delta}$ by
      \reffact{kentails-refl}, and thus we can conclude by
      \reflemma{flowers-univ-inst}.

    \item[\textbf{(\kl{ipet})}] The first direction is trivial by
...
for a \emph{semantic} proof of \kl{cut-elimination}. Here we propose an adaptation of
this technique to our flower calculus, based on a completeness proof for
cut-free \kl{sequent calculus} given by Hermant in
...
significantly from the forcing relation on formulas and \kl{sequent calculus} rules.
...
\refsec{IEGs}): $\phi$ is true if the content $\Phi_i$ of one of its petals
...
\end{proposition}
\begin{proof}
  Suppose the contrary, i.e. there is a \kl{substitution} $\sigma$ such that $\tT
  ...
  \entails{\Nature} \psi$. Let $\Xi$ be a \kl{context} such that \Hyp{$\chyp{\Psi,
...
\begin{proposition}[Analytic refutation]\labprop{inv-deriv-flower}
  ...
  theory, and $\phi = \flower{\garden{\bx}{\Phi}}{\Delta}$ with $\Delta
  = \fset{i}{n}{\delta_i} = \fset{i}{n}{\garden{\bx_i}{\Phi_i}}$ such
  that $\tT \nvdash{\Nature} \phi$. Then for every $1 \leq i \leq n$ and \kl{substitution}
...
instantiation of pistils with the \kl{ipis} rule, and the existential
instantiation of petals with the \kl{ipet} rule. Thus there seems to be some
...
We get completeness as the direct (and \kl{classical}) contraposition of
...
sprinkler is empty\sidenote{It is well-known that provability in \kl{intuitionistic}
...
the pollination relation (\refdef{pollination}), and is close to the
...
% \kl{Alpha^a} of \reffig{eg-atomic-alpha}.} \cite[Definition~2.13]{lmcs:1089}.
...
\begin{definition}[Interaction]\labdef{interaction}
  ...
  $\sinterneg$) to further specify when $\Xi\hole$ is \kl{positive} (resp. \kl{negative}).
\end{definition}
...
Then, two atomic occurrences can interact when their \kl{contexts} can. In the
...
\begin{definition}[Vehicle and Anchor]\labdef{vehicle} Let $\Phi$ a bouquet, and
  ...
  \begin{itemize}
    ...
    \item[\textbf{(Polarity)}] $\Psi\hole$ is \kl{negative} and $\Psi'\hole$ is
    \kl{positive};
  ...
  \begin{itemize}
    \item[\textbf{(Polarity)}] $\Psi\hole$ is \kl{positive} and $\Psi'\hole$ is
    \kl{negative};
...
to justify every \kl{positive} atom; while the anchor is the set of justifications
that \emph{must} be provided on demand by the environment/opponent, whose task
is to justify every \kl{negative} atom. The reason that we identify this structure,
...
that the \kl{context} $\Xi\hole$ in \refdef{interaction} is simply the \emph{least
...
for \kl{B_{inv}} described in \refsubsec{bubbles-search}.}. Roughly, the idea
is that given a \kl{goal} $\Phi$, we want to perform eagerly every justification
...
implemented by an instance of the \kl{poll{\da}} rule, sometimes preceded by an
instance of the \kl{poll{\ua}} rule. This is best described by the following
...
\kl(bubble){negative} $\lor$ formulas.}. The reproduction phase can be computed
...
decomposition phase: here the term ``decomposition'' refers \kl{metaphorically} to
...
\kl{intuitionistically} valid, while the other 7 formulas are only \kl{classically} valid.
...
pollination phase. In the case of the \kl{double-negation elimination law}
...
flower calculus. In this section, we present ongoing work on the \kl{Flower\,\,Prover}, a prototype of \kl{GUI} in the \kl{Proof-by-Action} paradigm based on
...
\begin{itemize}
  ...
  calculus, which proposes a quite unusual, non-exegetic take on \kl{EGs}. It does so
  both at the level of \emph{statements}, by building upon the \kl{intuitionistic}
  \kl{icon} of the $n$-ary scroll; and at the level of \emph{proofs}, by focusing on
  a \kl{goal}-oriented reading of rules that emphasizes the importance of
  \emph{invertibility} and \emph{\kl{analyticity}} in the inference process, through
...
\begin{itemize}
  \item[\textbf{\Proof mode}] this is the main mode, where \kl{goals} can be proved by
...
\begin{itemize}
  ...
  the pollination rules \kl{poll{\da}} and \kl{poll{\ua}}, called respectively
  ...
  orange box covering the empty pistil for \kl{EFQ}, \kl{Case} and
  \kl{Unlock} actions, or a green box covering the empty petal for
  \kl{QED} actions.
...
\begin{itemize}
  ...
  insert any flower in a \kl{positive} bouquet (i.e. add a new \kl{subgoal}, just like the
  ...
  all \kl{negative} corollas: those have the effect of growing a new empty petal,
  ...
  pistil. Thus areas subject to deletion are \kl{negative} atoms and flowers
  (\kl{crop} rule), \kl{positive} petals (\kl{pull} rule), as well as any area
...
\kl{contexts} of \emph{arbitrary} depth.
...
like \kl{Isar}: the point is that the \emph{proof} language, be it \kl{imperative} or
\kl{declarative}, is separated both conceptually and through its available means
...
present themselves: \emph{natural language}. Indeed, \kl{symbolic} formulas
...
potentially alternative \kl{goals} (adjacent petals) and further \kl{subgoals} (\kl{positively}
...
\begin{itemize}
  ...
  \item[\textbf{By selection}] the user can select subterms of the \kl{goal}, and
...
% Let us give an example of how to \emph{simulate} a rule from \kl{natural deduction}
...
% \emph{modus ponens}. A crucial step is the use of the \kl{grow} rule, which
...
% \refsec{Completeness} that the \kl{grow} rule is admissible, it was for the
...
% $A$ that justifies the negative one with the \kl{poll{\ua}} rule, and not
% the other way around. Indeed, the presence of the pistil around the negative $A$
% makes it deeper than the positive $A$, preventing the \kl{poll{\ua}} rule
...
% \todo{ Elaborate further on this computational aspect of \kl{EGs}. }
% \todo{ Investigate the ``scientific'' iteration rule of Peirce as a means to
% avoid the need for a computational interpretation of the \kl{epis} rule. }
...
(natural fragment): indeed to simulate \kl{srep} with petal (de)iteration, one
...
\begin{figure}
  ...
  \caption{Simulating the \kl{srep} rule by iterating petals}
  \labfig{flowers-srep-free}
\end{figure}

\paragraph{Focusing}

There is a formal connection between the \kl{poll{\ua}} rule of the flower
...
\kl{context} in one go, just as the \kl{epet} rule of the flower calculus renders
the \kl{crop} rule \kl{admissible}.

Sonia Marin has noticed the connection between \emph{bipoles} in \kl{focused} proofs,
...
generalization of coherent formulas at the end of \refsec{IEGs}.

Two years earlier, Brock-Nannestad and Ilik had already made some implicit
connections between \kl{focused} proofs and coherent formulas, through their
...
natural fragment $\Nature$ of the flower calculus: every \kl{inference rule} is
...
particular type of \kl{judgment}. Thus it might be possible to fit the flower
...
flower calculus allows for a particularly elegant implementation of \kl{subformula
...
\kl{poll{\da}} in pistils would be a kind of \emph{function application}
...
\emph{\kl{term} annotation}, that should be read in context as ``this occurrence of
...
The next step would be to support \kl{first-order} reasoning by adding sprinklers and
...
dependently-typed version of the flower calculus sketched above, where \kl{proof
...
that emphasizes \kl{goal}-reduction through \kl{invertible} and \kl{analytic} rules, enabled us
...
of identity, trading a heavy graphical apparatus with local \kl{inference rules} for
a simple, well-known textual syntax with complex (but automated) global dynamics
in the form of \kl{substitutions}. Hence we believe the \emph{opposite}, that \kl{EGs}
