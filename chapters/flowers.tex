...
In \refsec{alpha}, we presented the syntax of existential graphs (\kl{EGs}) as
...
\begin{definition}[Depth]
  The \emph{depth} $\soldepth{\Phi\hole}$ of a \kl{context} $\Phi\hole$ is
...
\begin{description}
  \item[Blank Antecedant (\rsf{epis})]
    ...
    of the \kl{classical} rule \rsf{Dcut{\ua}} of \kl{Alpha} and \sys{Beta}. The
    ...
    \rsf{Dcut{\da}} in \kl{Alpha} (Corollary \ref{cor:adm-ins}).
  ...
  %   a weaker, intuitionistic version of the \kl{classical} rules \rsf{Dcut{\ua}} and
  %   \rsf{Dcut{\da}} of \kl{Alpha} and \sys{Beta}. \rsf{epis{\da}} will be shown
  %   to be admissible in \refsec{Completeness}, which might be related to the
  %   co-admissibility of \rsf{Dcut{\da}} in \kl{Alpha} (Corollary
  %   \ref{cor:adm-ins}).

  \item[(De)iteration (\rsf{poll{\da}}, \rsf{poll{\ua}})]
    Renamed \emph{\textsf{(poll)}ination} rules, they correspond to the rules
    \rsf{Iter} and \rsf{Deit} of \kl{Alpha} and \sys{Beta}, but reformulated
  ...
  \item[Insertion/Deletion (\rsf{grow}, \rsf{crop}, \rsf{pull}, \rsf{glue})]
    They correspond to the rules \rsf{Ins} and \rsf{Del} of \kl{Alpha} and
  ...
  \item[Unification (\rsf{ipis}, \rsf{ipet}, \rsf{apis}, \rsf{apet})]
    ...
    \rsf{Unif{\da}} of \kl{Beta}, where the variable \kl{substitution} $\{z/y\}$
    ...
    \rsf{ipis}/\rsf{ipet} to be \kl{invertible}, we duplicate the whole flower/petal
...
\begin{description}
  ...
  \item[Disjunction/Absurdity Elimination (\rsf{srep})]
    ...
    its converse. The name \rsf{srep} is short for
...
polarizing the \kl{context} of application as for the rule \rsf{apis} (resp.
\rsf{apet}), at the cost of making the rule non-\kl{invertible}.}.
...
\kl{weakening} rules $\{\rsf{grow},\rsf{crop},\rsf{pull},\rsf{glue}\}$:
...
$\{\rsf{poll{\da}},\rsf{poll{\ua}}\}$, because of the arbitrary \kl{context} $\Xi$
...
the \rsf{epet} rule in every applicable \kl{context} in the \kl{goal}, which amounts to
...
\begin{itemize}
  ...
  \rsf{epis} rule might be \kl{admissible}. However if it is not, one needs a
  ...
  to select a set of flowers in the \kl{goal}. Then, we could add a \Action{Fence}
...
\begin{itemize}
  ...
  (\rsf{crop} rule), \kl{positive} petals (\rsf{pull} rule), as well as any area
...
\intro{zoomable user interfaces} (\reintro{ZUI}), a strand of \kl{GUI} that has been developed
by many pioneers in the field of human-computer interaction such as Ivan
Sutherland in his \intro{Sketchpad} system \sidecite{10.1145/800265.810742}, and
Alan Kay in his \intro{Smalltalk} system
\sidecite[4em]{goldberg_smalltalk-72_1976}.
\end{remark}
...
considered is the only \kl{DnD} action, \Action{Import}. This is not surprising,
...
\emph{theories}, and an IDE for writing and executing \kl{proof scripts} that
modify \emph{\kl{goals}}, themselves visualized in a separate \kl{proof view}.
...
dedicated \texttt{move} \kl{tactic}. Other examples can be found for renaming
...
In the original \kl{IEGs} system of Oostra, the \rsf{srep} rule is replaced by an
...
also needs the non-\kl{invertible} $\Culture$-rule \rsf{crop} (as well as the
...
\kl{context} in one go, just as the \rsf{epet} rule of the flower calculus renders
the \rsf{crop} rule \kl{admissible}.
...
style of \kl{G4ip}\sidenote{This is just another name for the system \sys{LJT}
...
in the \kl{calculus of structures}, where one component of \rsf{poll{\ua}}, the
...
\emph{moving} flowers deeper, as witnessed by the \kl{DnD} \Action{Import} action of
...
$$
This could also form the basis for further studies on the connections between \kl{EGs}
and (dependent) \kl{type theory}, and ultimately lead to a tight integration of the
\kl{Flower\,\,Prover} with \kl{proof assistants} based on the latter such as \kl{Coq}, \kl{Lean} and
Agda. Existing explorations of the links between \kl{type theory} and \kl{deep inference}
...
\kl{first-order} terms, and devising graphical actions for the rules $\{\rsf{ipis},
