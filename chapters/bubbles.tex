...
chemical diagrams to devise his \emph{existential graphs} --- see pp. 17-18 in
\cite{Roberts+1973}, or our own presentation in \refsec{Quantifiers} for more
details.}. Click actions are then a mean to ``heat'' molecules to the point of
...
blue conjunctions for forward DnDs\sidenote{In predicate logic, one would also
need the right (resp. left) introduction rule for $\forall$ (resp. $\exists$).
It might also be the case that backward DnDs alone are sufficient for
completeness, since a linkage of the form $A \back \select{B} \limp C$ will
involve a forward phase. In this case only the right introduction rules for
$\limp$ and $\forall$ would be required.}.
...
\begin{itemize}
  ...
  precisely, rules {\rnmsf{i{\da}}} and {\rnmsf{i{\ua}}} correspond
  ...
  $\mathbb{M}$-rule {\rnmsf{p}} allowing to \emph{pop} any empty bubble, which
...
\begin{definition}[\sys{BJ}-step]\labdef{BJ-step}
  We write $S \lstep T$ to denote the existence of a \sys{BJ}-rule instance
  $\irule{r}{S}{T}$ in the empty context, i.e. $S$ and $T$ are respectively the
  conclusion and premiss of the rule {\rnmsf{r}} in \reffig{sequent-BJ}, modulo
  ...
  building setting where inference rules are seen as goal-modifying actions.}.
...
\begin{definition}[\sys{BJ}-derivation]\labdef{BJ-deriv}
  ...
  $\mathcal{D}$ of \sys{BJ}-steps with premiss $T$ and conclusion $S$.
\end{definition}
...
inference proof theory. It is Jean-Baptiste Joinet who shared with us his idea
of applying it in this setting, based on his own work interpreting CoS for
\sys{MLL} as a system for building \emph{multiplicative proof nets}
...
\refch{bubbles-symm}, but we think a better answer might have been found with
the proof system introduced in \refch{flowers}, and its associated prototype of
GUI presented in \refch{flowerprover}.
...
of formulas and $\cS$ of bubbles. That is, the rules of \sys{BJ} are
...
({\rnm{\mcirc{+}}} rules), and the {\rnmsf{i{\da}}} rule corresponds to
...
\begin{description}
  \item[\textbf{\flow}]
    ...
    of {\rnmsf{f{-}}} followed by {\rnmsf{i{\da}}} could be performed as
  ...
  \item[\textbf{\membrane}]
    ...
    the {\rnmsf{p}} rule can be understood as resulting from a process of
    \emph{contraction}\sidenote{Not to be confused with the contraction rules
    \rnmsf{c} and \rnmsf{w}.} of the membrane into a single point: if the bubble
  ...
  \item[\textbf{\resource}]
    ...
    blue items.
  \item[\textbf{\identity}]
    Although the {\rnmsf{i{\da}}} rule only corresponds to the base case
    ...
    {\rnmsf{i{\ua}}} rule could be mapped instead to a contextual action
    ...
    % Finally, an analog to the cut rule {\rnmsf{i{\ua}}} is the rule
    % {\rnmsf{d\ua}} for adding new definitions (\reffig{bubbles-vars}).
  ...
  \item[\textbf{\heating}]
    ...
    %   are the {\rnmsf{i{\da}}} rule which will be shown to be admissible
    %   in \refsec{bubbles-completeness}, and the {\rnmsf{d{\ua}}} rule of
    %   \reffig{bubbles-vars}.}.
    % \end{itemize}
\end{description}
...
\begin{description}
  ...
  \item[\textbf{Navigation}] The tree structure of subgoals is immediately
    ...
    existential graphs (pp. 112-113 in \cite{Roberts1973-inactu}). We will come
    back to this in \refch{eg}.}.
