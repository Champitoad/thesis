...
In this chapter, we focus on how both \emph{click} and \emph{\kl{drag-and-drop}}
(\kl{DnD}) \kl{actions} upon the formulas of a \kl{sequent} can implement proof
...
feedback for crafting the way \kl{DnD} \kl{actions} are to be translated into proof
...
\kl{Proof-by-Pointing} capabilities through click \kl{actions}. The two following
sections explain, through further examples, how \kl{drag-and-drop} \kl{actions} work;
first for so-called \emph{rewrite} \kl{actions} involving equalities, then for
\kl{actions} involving logical connectives and quantifiers. We end in \refsec{pba-rw}
...
% question of integrating drag-and-drop \kl{actions} and powerful proof automation
...
\kl{inference rules} of \kl{natural deduction} (\reffig{calculi-NJ}) and/or
\kl{sequent calculus} (\reffig{calculi-LJ}).
...
\begin{figure*}
 ...
 gray dotted arrows have been added to show the two possible \kl{actions}.
...
\AP
In this example, there are two possible \kl{actions}.
...
Furthermore, the \kl{action} actually links together two {\em subterms} of each of
...
\begin{itemize}
  \item These elementary rewrites are not visible for the user. What she sees is
    the final result of the \kl{action}, that is the last expression of the rewrite
    sequence.
...
In general, the \kl{action} solves the \kl{goal} when the interaction ends with
the trivially true proposition $\top$. The base case being the \kl{action}
corresponding to the \kl(rule){axiom}/\kl(sequent){identity rule} \kl(sfl){id}:
$A\back A \step{} \top$.
...
\kl{Drag-and-drop} \kl{actions} involve two \kl{items}. Some proof steps involve only
one \kl{item}; they can be associated to the \kl{action} of clicking on this
...
\begin{margintable}
\caption[]{Mapping of click \kl{actions} to \kl{inference rules}}
\labtab{click-rules}
\begin{tabular}{ccc}
	\toprule
...
One can see that these \kl{actions} correspond essentially to the \kl{right
...
\kl{LJ} for hypotheses. The exact mapping between click \kl{actions} and
...
\begin{itemize}
  ...
  \item There is no \kl{action} mapped to red $\bot$ \kl{items}, simply because
  $\bot$ does not have any \kl{introduction rule}.

  \item There is currently no \kl{action} mapped to blue $∀$ and red $∃$ \kl{items}.
  ...
  and then \kl{drag-and-drop} the \kl{item} holding the selected subexpression
  instead.
\end{itemize}

\begin{remark}[Click completeness]\labremark{click-completeness}
  From the previous remarks and the completeness of the cut-free \kl{sequent
  calculus} \kl{LJ}, it follows that click \kl{actions}, when combined with new
...
It is possible to associate some more complex effects to click \kl{actions} performed
...
here focus more on \kl{drag-and-drop} \kl{actions}, we do not detail further more advanced
\kl{PbP} features. The version of \kl{Actema} presented in \refch{plugin}
provides an implementation of \kl{PbP} available as a contextual menu \kl{action}.
...
\kl{drag-and-drop}. The user points at the occurrence(s) of $t$ to be
...
connectives through click \kl{actions}. In many cases however, this will not be
necessary: because a \kl{drag-and-drop} involves subterms of the \kl{items} involved, one
...
\kl{L\land_2}. A very simple \kl{action} is thus:
$$
\begin{array}{llll}
...
simplify or solve one of the branches by a \kl{DnD} \kl{action}. This involves
...
usual \kl{subgoal} semantics associated with click \kl{actions}. More interesting is
...
We have noticed that on some examples, such \kl{actions} could provide a
...
As the first example of this chapter shows, \kl{drag-and-drop} \kl{actions} work through
...
\textsf{id} rule. This can be detected by the system because the \kl{unification} of
...
that this example requires to use full (\kl{first-order}) \kl{unification}, not only
matching.}. The system thus refuses this \kl{action}.
...
\kl{actions}. One key difference is that window \kl{inference rules} can be
...
There are other proving interfaces which include \kl{drag-and-drop} features. Two of
...
refinement, and uses \kl{DnD} \kl{actions} between an expression and a transformation, in
...
\kl{actions}, which requires far less textual inputs from the user. However, they
only consider point-and-click \kl{actions}, and rely on a text-heavy presentation at
two levels:
\begin{enumerate}
...
exploit the same idea of associating click \kl{actions} on head connectives to
