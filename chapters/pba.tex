...
(\kl{DnD}) actions upon the formulas of a sequent can implement proof construction
...
\kl{proof-theoretical} foundations for the semantics of \kl{DnD} actions
...
\kl{DnD} actions are to be translated into proof construction steps in an intuitive
and practical way. A description of the overall architecture and implementation
design of \kl{Actema} will be provided in \refch{plugin}.
...
\kl{Proof-by-Pointing} capabilities through click actions. \refsec{newitems} shows
very briefly how one can add new formulas and objects to the current \kl{goal}. The
...
Any \kl{proof system} must implement a given logical formalism. What we describe here
...
\kl{first-order logic}. We assume a \kl{first-order} language (function and predicate
symbols) is given. Provability is defined over sequents $\Gamma\seq C$ by the
usual logical rules of \kl{natural deduction} (\reffig{calculi-NJ}) and/or
...
ancient example from Aristotle. A \kl{goal} appears as a set of {\em items}
...
Finally, note that each \kl{goal} is displayed on a tab.
...
This will transform the \kl{goal} by changing the conclusion to $\human(\socrates)$.
...
The first case is what we call a {\em \intro(dnd){backward} step} where the
...
these items; this is written by squaring these subterms. The \kl{symbol} ${\back}$,
used as an operator, is meant to describe the result of the interaction.
Internally, the behavior of this operator is defined by a set of \kl{rewriting rules}
...
\item   These elementary rewrites are not visible for the user. What she sees is
  ...
  sequence.
\item The definitions of the \kl{rewriting rules} in \reffig{DISL} and
...
In general, the action solves the \kl{goal} when the interaction ends with
...
\sys{LJ} for hypotheses. The exact mapping between click actions and
\kl{inference rules} is given in \reftab{click-rules}. A few remarks are in
order:
\begin{itemize}
  \item In the current implementation of \kl{Actema}, clicking on a blue item $A
  ...
  does not have any \kl{introduction rule}.
  ...
  the communication protocol between the frontend and backend of \kl{Actema}. Instead
  ...
  subexpression occurring in any item of the current \kl{goal}, and then
...
on locations deeper under connectives. This is the essence of \kl{Proof-by-Pointing},
and~\sidecite{PbP} provides ample description. Since we here focus more on
drag-and-drop actions, we do not detail further more advanced \kl{PbP} features.
However we stress that these features are essentially compatible with what we
describe in this work. In fact the version of \kl{Actema} presented in \refch{plugin}
...
solve the current \kl{goal}. These can be done respectively with the blue
...
object. The \texttt{+hyp} button will also create a new \kl{subgoal} requiring to
...
specific to the logic of \kl{Actema}. A desirable, but highly non-trivial feature
...
their \kl(dnd){forward} behavior with the rules \rsf{F\lor_1} and \rsf{F\lor_2}, in
...
instantiated following a \kl{linkage}:
$$
\begin{array}{rcl}
...
following \kl{linkage} is valid and solves the \kl{goal} through one action:
$$
\begin{array}{lll}
...
the following \kl{linkage}:
$$
\begin{array}{rcl}
  \forall x. \exists y. \select{R(x,y)}\back \exists y'.\forall x'. \select{R(x',y')}
\end{array}
$$
Indeed, there is no reduction path starting from this \kl{linkage} ending with the
...
Window inference has been used both for general-purpose logics like \kl{HOL}
...
justifies a \kl{DnD}. The window inference approach gives to the user a
...
%paragraph.
As for the KeY Prover, its usage of \kl{DnD} overlaps only a very small
