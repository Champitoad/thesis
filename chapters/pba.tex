...
\kl{Proof-by-Pointing} capabilities through click actions. \refsec{newitems}
shows very briefly how one can add new formulas and objects to the current
\kl{goal}. The two following sections explain, through further examples, how
...
end in \refsec{pba-rw} with a discussion of related works.
...
\begin{itemize}
\item A {\em red \kl{item}} which is the proposition to be proved, that is the
 {\em conclusion},
\item {\em blue \kl{items}}, which are the local {\em hypotheses}.
\end{itemize}
...
order:
\begin{itemize}
  \item In the current implementation of \kl{Actema}, clicking on a blue \kl{item} $A
  \limp B$ will work only if the conclusion is $B$. An alternative is to use the
  {\kl{{\limp}L}} rule from \kl{sequent calculus}, which is applicable in every
  context.
  \item There is no action mapped to red $\bot$ \kl{items}, simply because $\bot$
  does not have any \kl{introduction rule}.
  \item There is currently no action mapped to blue $∀$ and red $∃$ \kl{items}. The
  ...
  can drag the corresponding green \kl{item} and drop it on the quantified \kl{item} to
  instantiate it. It is also possible to select with the mouse an arbitrary
  subexpression occurring in any \kl{item} of the current \kl{goal}, and then
  drag-and-drop the \kl{item} holding the selected subexpression instead.
\end{itemize}
...
\begin{tabular}{ccc}
	\toprule
	\thead{Head connective} & \thead{Red \kl{item}} & \thead{Blue \kl{item}} \\
	\midrule
  $\top$ & \kl{\top R} & \kl{\top L} \\
	\midrule
...
Often in the course of a proof, one will want to add new \kl{items}: either a new
conjecture (blue \kl{item}), or a new object (green \kl{item}) that would be helpful to
...
in the conclusion. Similar rules (\kl{F{=}_1} and \kl{F{=}_2}) are
