...
\intro{Proof assistants} --- also called \intro{interactive theorem provers}
...
era --- now called \intro{Hilbert systems} --- featured very few inference
...
In \kl{Hilbert systems}, the only inference rule is that of \emph{modus ponens},
...
\reffig{calculi-NJ} shows the complete set of \kl{natural deduction} rules for all
connectives and quantifiers in \kl{intuitionistic} logic, that was introduced by
Gentzen under the name \intro{NJ}\sidenote{Gentzen simultaneously introduced a
\kl{natural deduction} calculus named \intro{NK} for \kl{classical} logic, which is just
\kl{NJ} with an additional rule modelling the principle of \emph{indirect proof}
--- i.e. the possibility to prove any proposition $A$ by deriving a
...
in the sense that the former is (\kl{intuitionistically}) provable if and only if the
...
assumption. In \kl{sequent calculus}, this relation of provability of a conclusion
...
for designing proof formalisms called \intro{deep inference}
...
top-level of sequents\sidenote{In fact, Schütte had already proposed a \kl{deep
...
\begin{emphpar}
\emph{All} the proof formalisms designed in this thesis are \kl{deep inference}
...
In \refch{sfl}, we ground the semantics of DnD actions in \kl{deep inference} proof
theory, by designing a variant of the subformula linking \kl{CoS} of Chaudhuri
introduced in \cite{Chaudhuri2013} for \kl{intuitionistic} \kl{FOL} with equality. Our
...
In the second part of this thesis, we explore a series of \kl{deep inference} proof
...
Thus we are not the first to attempt to design new proof systems based on \kl{deep
...
\kl{calculus of structures} for the logic \kl{BV}. However, some proof systems based
