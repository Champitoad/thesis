...
statements, here through \intro{rewriting rules} that transform a valid equation
...
$$\mathrm{prime}(n) \lor \neg\mathrm{prime}(n)$$
Another related principle is the \intro{law of non-contradiction}, which states
...
\kl{cut} rule satisfy the \emph{subformula property}: every formula $A$
...
not \emph{equivalent}\sidenote{In the standard system \kl{SKS} where the \kl{s}
rule originates, every context is \emph{positive}, in the sense that the hole
$\hole$ never occurs under a negation $\neg$ or left-hand side of an implication
$\limp$; otherwise the rule would need to be restricted to positive contexts to
stay valid. See \refsec{eg-completeness} for more details on \kl{SKS}.}. When
