...
statement can be reduced to the mechanical application of \kl{symbolic} manipulation
...
already shows the two main aspects of \kl{proof formalisms}: on the one hand,
...
statements, here through term rewriting rules that transform a valid equation
...
\kl{set theory} (\kl{ZF}, or \sys{ZFC} with the axiom of choice), which is the most
popular foundation nowadays because of its greater simplicity.
...
build (correct) proofs from \kl{axioms}/assumptions. Those form the theoretical basis
for both the \emph{interactive creation}, and the \emph{automatic checking} of
formal proofs in \kl{proof assistants}. The branch of \kl{proof theory} concerned with the
study of \kl{inference rules} is called \intro{structural proof theory}.
\end{emphpar}
...
Russell's paradox caused by the unrestricted axiom of comprehension in naive \kl{set
theory}. In order to restore absolute trust in the foundations of
(\kl{classical}) mathematics, Hilbert proposed in the early 1920s to prove
\emph{mathematically} the consistency of the \kl{axiomatic system} for
arithmetic introduced in 1889 by Peano\sidenote{A more involved \kl{axiomatic
system} was proposed one year earlier by Dedekind \cite{dedekind_nat}. Less
known is that Peirce had already published in 1881 an equivalent axiomatization
of natural numbers \cite{peirce_logic_1881}.}, i.e. that no contradiction can be
derived from Peano's axioms. Indeed, he believed that every mathematical truth
could be derived from the principles of arithmetic, thus reducing the problem of
the consistency of mathematics to that of arithmetic. Moreover, Hilbert's
program was to be carried by \emph{finitist} means, without resorting to any
reasoning principle involving infinite collections --- which were at the heart
of the controversy started by constructivists. This was the initial impulse for
developing \kl{proof theory}, since it provided a mathematical definition of
``mathematical proofs'' as \emph{finite} sequences of \kl{symbols} satisfying certain
properties.

Gentzen's work on \kl{natural deduction} was an integral part of this program, as an
attempt to render the \emph{metamathematics} of \kl{proof theory} more structured and
...
regarded as the cornerstone for most developments in \kl{proof theory} to this day.
...
lies in its splitting of elimination rules into two parts: \emph{left
introduction} rules, and the \emph{cut} rule. As their name indicates, left
introduction rules serve a purpose symmetric to right introduction rules: while
the latter define how to introduce a connective in the conclusion of a sequent,
the former define how to introduce a connective in one of its hypotheses. Then,
the only way to \emph{use} such an hypothesis $A$ is through the \rsf{cut} rule,
which erases $A$ from the context in the conclusion of the rule, by justifying
it with the proof of $A$ given as premiss. The \rsf{cut} rule can also be seen
as a generalization of the \textit{modus ponens} rule of \kl{Hilbert systems},
replacing the logical connective $\limp$ of implication by the ``structural
connective'' $\seq$ of sequents. The remaining \intro(rule){axiom} rule
\intro{ax} is in a sense dual to the \rsf{cut} rule: while the latter allows
justifying a hypothesis by an identical conclusion, the \rsf{ax} rule allows
justifying a conclusion by an identical hypothesis\sidenote{It is not clear
where the terminology ``axiom rule'' comes from. It might be because hypotheses
can be considered as \kl{axioms} in the sense we introduced earlier, i.e.
statements that we \emph{assume} to be true. In any case, it would be a
misattribution of category to consider the \rsf{ax} rule itself as an
\kl{axiom}, and some authors like Girard call it the \emph{identity} or \rsf{id}
rule to avoid the confusion.}. \reffig{PNC-LJ} shows a proof of the principle of
non-contradiction in \kl{LJ}.
...
is the first occurrence of the concept of \emph{analyticity} in \kl{proof theory},
...
Contrary to \kl{predicate logic} in traditional \kl{proof theory}, type theories break the
...
Almost all type theories are \intro{higher-order}: they give a first-class
...
\emph{$\lambda$-term} encoding the \kl{higher-order} function that takes a sequence
...
By contrast, the \kl{predicate logic} developed in the 19\textsuperscript{th}
century and studied in traditional \kl{proof theory} is \intro{first-order}:
...
\begin{emphpar}
In this thesis, we exclusively study \kl{proof formalisms} for \emph{\kl{first-order}}
...
\emph{combinatory logic}, and the \kl{axioms} of \kl{Hilbert systems} for implication
...
theory\sidenote{In the case of Mizar, a typed \kl{set theory} based on \kl{FOL}.}. The
...
\begin{itemize}
  ...
  done in \kl{symbolic} form, either with a custom ASCII notation or with Unicode
  characters corresponding to the standard \kl{symbols} in more modern \kl{proof
  assistants}. 
...
\emph{abstraction} capabilities, that can only be captured through the \kl{symbolic}
...
\begin{emphpar}
  ...
  \emph{goals} in a \intro{graphical user interfac} (\reintro{GUI}). To be as
  general as possible, we call such a paradigm ``\intro{Proof-by-Action}''
  (\reintro{PbA}).
\end{emphpar}
...
\begin{quote}
    ...
    such that the correspondence between composite ideas and their \kl{symbols} would
...
\begin{quote}
  ...
  qualitative features of the object, then the sign is an \kl{icon}. If the
...
techniques based on the direct manipulation of \emph{\kl{symbolic}} formulas in
sequents.
...
\kl{GUI} called Actema. In particular, \kl{DnD} actions can be seen as a graphical
...
\begin{description}
  ...
  introduces the metaphor of \emph{bubbles} as a way to \kl{iconically} represent the
...
\begin{itemize}
  ...
  representations for the proof state, that stray away from traditional \kl{symbolic}
  formulas.
\end{itemize}
