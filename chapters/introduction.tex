...
Gentzen under the name \kl{NJ}\sidenote{Gentzen simultaneously introduced a
natural deduction calculus named \kl{NK} for classical logic, which is just
\kl{NJ} with an additional rule modelling the principle of \emph{indirect
...
\begin{figure*}
  ...
  \caption{Natural deduction calculus \kl{NJ} for intuitionistic logic}
  \labfig{calculi-NJ}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{Sequent calculus \kl{LJ} for intuitionistic logic}
  \labfig{calculi-LJ}
\end{figure*}
...
logic called \kl{LK}, and one for intuitionistic logic called \sys{LJ}. Here we
focus on the intuitionistic system \kl{LJ}, whose rules are shown in
\reffig{calculi-LJ}.
...
shows a proof of the principle of non-contradiction in \kl{LJ}.
...
Inductive Constructions} (\kl{CoIC}), is also implemented in two leading proof
...
arguably the first diagrammatic proof system in history: the \intro{existential
graphs} (\reintro{EGs}). This will be the subject of \refch{eg}, and the basis
for the development of a \emph{metaphorical} proof system in \refch{flowers}.
...
\begin{description}
  ...
  intuitionistic sequent calculus \kl{LJ} of Gentzen (\reffig{calculi-LJ}). It
  ...
  calculus for classical logic called ``\kl{system B}'', where bubbles can be
  \emph{polarized} in addition to formulas. Intuitionistic, dual-intuitionistic
  and bi-intuitionistic logic can be recovered as fragments of \kl{system B},
  ...
  Peirce for propositional and first-order classical logic, which have been
  ...
  intuitionistic variant of \kl{EGs} where statements are represented
  ...
  Prover is also the first \emph{mobile-friendly} interface for ITPs that we
  know of.
\end{description}
...
calculus of structures for the logic \kl{BV}. However, some proof systems based
