...
connective. An example of \emph{instance} of this schema would be the
proposition ``$n$ is prime or $n$ is not prime'' with $n$ some natural number,
...
the converse), we will always focus first on the intuitionistic ``kernel'' of
our systems, designing the classical part as an extension of the former.
\end{emphpar}
...
designated by the abbreviated name \rsf{mp}.This reading of the rule
...
\todo{Margin figure for proof of PNC in NJ}

Many logical reasoning principles that were \emph{axiomatized} in Hilbert
systems can be \emph{derived} in natural deduction. For example, \reffig{PNC-NJ}
shows a proof of the principle of non-contradiction, built by \emph{composing}
instances of rules from \reffig{calculi-NJ}. The composition of a rule instance
\rsf{r1} with another rule instance \rsf{r2} simply consists in using the
conclusion of \rsf{r1} as one of the premisses of \rsf{r2}.
...
logic called \sys{LK}, and one for intuitionistic logic called \sys{LJ}. Here we
focus on the intuitionistic system \sys{LJ}, whose rules are shown in
\reffig{calculi-LJ}.
...
\todo{Margin figure for proof of PNC in LJ}
...
rule allows to justify a conclusion by an identical hypothesis. \reffig{PNC-LJ}
shows a proof of the principle of non-contradiction in \sys{LJ}.
...
inference system as early as 1977 \cite{schutte_proof_1977}, as did Peirce one
century before him with his \emph{entitative graphs} (as we will see in
\refch{eg}). But the idea did not generate much interest at the time.}. The
...
\begin{itemize}
  ...
  in predicate logic is \emph{undecidable}.} --- relieve mathematicians from the
...
\emph{combinatory logic}, and the axioms of Hilbert systems for implication
...
deduction system \sys{NJ} of Gentzen (\reffig{calculi-NJ}).
...
be shrinkable to 4 in the Mizar proof assistant \sidecite{debruijn_factor},
...
theory\sidenote{In the case of Mizar, a typed set theory based on FOL.}. The
...
Any proof assistant must provide the two following features in its statement
language:
...
graphical notations in the Lean proof assistant\sidenote{For another related
work that tackles this problem in the context of functional programming, see
\cite{omar-filling-2021}.} \sidecite{ayers_graphical_2021}.
...
\begin{description}[labelsep=0pt]
  \item[Imperative~]proof languages, like imperative programming
  ...
  prover~\sidecite{doi:10.1098/rsta.1984.0067}, they are called \emph{tactics}.
  ...
  \rsf{\lor R_1} and \rsf{\lor R_2} (resp. \rsf{\land R}) of \sys{LJ} in their
  backward reading (\reffig{calculi-LJ}).
...
\sidecite{proofcert}\sidecite{dedukti}).
...
Unfortunately, the user interfaces of state-of-the-art proof assistants are
...
As for the \emph{modification} phase of proofs, the PbA paradigm requires a way
...
and the communication of meaning.}, we define an iconic language as one whose
...
third kind of sign, \emph{indexes} \sidecite{atkin_peirces_2023}:
\begin{quote}
...
graphs}. This will be the subject of \refch{eg}, and the basis for the
development of a \emph{metaphorical} proof system in \refch{flowers}.
...
In \refch{sfl}, we ground the semantics of DnD actions in deep inference proof
...
In \refch{advanced}, we present a number of more advanced techniques in the PbA
...
In \refch{sfl-classical}, we investigate an extension of PbA to sequents with
...
share the ability to represent goals in two alternative ways: either
...
\begin{description}
  ...
  \refch{bubbles} presents the \emph{asymmetric} bubble calculus \sys{BJ} for
  intuitionistic logic, modelled after the \emph{asymmetric} sequents of the
  intuitionistic sequent calculus \sys{LJ} of Gentzen (\reffig{calculi-LJ}). It
  ...
  \refch{bubbles-symm} refines \sys{BJ} into a more general and symmetric
  calculus for classical logic called ``system \sys{B}'', where bubbles can be
  \emph{polarized} in addition to formulas. Intuitionistic, dual-intuitionistic
  and bi-intuitionistic logic can be recovered as fragments of system \sys{B},
  by forbidding certain inference rules that characterize the \emph{porosity} of
  bubbles. We also devise a fully invertible variant of system \sys{B}, that we
  ...
  In \refch{eg}, we give a complete review of the original EG systems of Peirce
  ...
  might be that EGs have been invented at the end of the 19\textsuperscript{th}
  century, \emph{before} the birth of proof theory as a discipline in the 1920s
  under the impulse of Hilbert.}. We propose in particular a novel inductive
  ...
  Finally in \refch{flowers}, we introduce the \emph{flower calculus}, an
  intuitionistic variant of EGs where statements are represented metaphorically
  as \emph{flowers}. We identify a so-called \emph{natural} fragment of the
  system where every rule is both analytic and invertible, and a \emph{cultural}
  fragment where every rule is non-invertible. We prove that the cultural
  fragment is admissible thanks to a completeness proof of the natural fragment
...
in \refch{plugin} is under active development by us and Benjamin Werner. All
...
\begin{digression}
  Sometimes we will develop ideas loosely related to the main text in
  \emph{digressions}. These will appear in the margin in boxes such as the
  current one, and they can be safely ignored --- at least on a first reading.
  We distinguish them from normal side notes, which are usually shorter and more
...
reasoning style in ITPs\sidenote{Note that during most of the time period when
window inference was developed, the terminology of ``deep inference'' had not
been introduced yet. Indeed, the first article on the subject appeared in 1999
\cite{Guglielmi1999ACO}, with very different motivations in mind: namely, the
development of a proof-theoretical approach unifying concurrent and sequential
computation, resulting in the calculus of structures for the logic \sys{BV}. But
some proof systems based on deep inference principles already existed and
inspired researchers in window inference, as witnessed by the reference to
Schütte's proof theory in the above quote.}. However, we believe our approach is
