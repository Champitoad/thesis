...
\AP
Almost all \kl{type theories} are \intro{higher-order}: they give a first-class
...
\kl{$\lambda$-calculus} \sidecite{e4d9a073-5bba-3a5b-90d3-81832cd433e5}, one may type
...
\intro{$\lambda$-term} encoding the \kl{higher-order} function that takes a sequence
...
be seen as \kl{types}, by having judgments such as $t : 1 + 1 = 2$ where $t$ is a
\intro{proof term} representing a proof of the proposition $1 + 1 = 2$. This
\intro{propositions-as-types} principle was rediscovered independently by Howard
...
Curry.} --- where \kl{$\lambda$-terms} in the simply-typed \kl{$\lambda$-calculus} are put
in one-to-one correspondence with proofs in the implicational fragment of the
\kl{natural deduction} system \kl{NJ} of Gentzen (\reffig{calculi-NJ}).

\AP Thus the core of \kl{type theory} is \emph{intuitionistic} in nature, and the
...
Martin-Löf (\reintro{ITT} or \reintro{MLTT}), which formed the basis for the
implementation of many \kl{proof assistants} like \kl{NuPrl}, \kl{Alf}, and most
recently \kl{Agda} \cite{geuvers_proof_2009}. A system closely related to
\kl{MLTT}, the \intro{calculus of inductive constructions} (\reintro{CoIC}), is
also implemented in two leading \kl{proof assistants}: \intro{Coq}
\sidecite{the_coq_development_team_2022_7313584} and \intro{Lean}
...
\kl{Type theories} are the logical foundation for the \emph{kernel} of \kl{proof
...
20 to translate a paper proof into a formalized proof in \kl{Automath}
...
process of compiling a high-level proof text into a low-level \kl{proof term} is
...
elaboration of abstract notations into \kl{terms} of the underlying \kl{type theory}
...
\begin{emphpar}
  ...
  \emph{goals} in a \intro{graphical user interface} (\reintro{GUI}). To be as
