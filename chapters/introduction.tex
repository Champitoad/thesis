...
\AP \intro{Proof assistants} --- also called \reintro{interactive theorem
provers} (\reintro{ITPs}) --- are software systems that allow to both create and
...
statement can be reduced to the mechanical application of \kl{symbolic}
...
this era --- now called \intro{Hilbert systems} --- featured very few
...
instance, the famous \intro{law of excluded middle} (\reintro{LEM}), that states
...
\kl{ex} of \intro{exchange} and \kl{ctr} of \intro{contraction} in
\reffig{calculi-LJ}. A third \kl{structural rule}, \kl{wkn} for
...
\AP
The main difference between \kl{sequent calculus} and \kl{natural deduction}
...
\intro(rule){axiom} rule \intro{ax} is in a sense dual to the \kl{cut} rule:
...
\emph{identity} or \rsf{id} rule to avoid the confusion.}. \reffig{PNC-LJ} shows
a proof of the \kl{law of non-contradiction} in \kl{LJ}.
...
\emph{cut-free} proof, i.e. a proof that does not make use of the \kl{cut}
...
\kl{cut} on $A$ as a way to invoke the lemma $A$ without duplicating its proof.
...
cut-free one. Thus the \kl{cut} rule is said to be \emph{admissible}, since any
provable sequent can be proved without it.

An important consequence of cut-elimination, which was the original motivation
of Gentzen, is the consistency of the logic (\kl{intuitionistic} \kl{predicate
logic} in the case of \kl{LJ}). This stems from the fact that all rules apart
from the \kl{cut} rule satisfy the \emph{subformula property}: every formula $A$
...
is the first occurrence of the concept of \emph{analyticity} in \kl{proof
theory}, and can be seen as a technical realization of the philosophical notion
...
in \refch{eg}). But the idea did not generate much interest at the times.}. The
first \kl{deep inference} system was the \intro{calculus of structures}
(\reintro{CoS}), which can be succinctly described as a \intro{rewriting system}
...
special subformula $\hole$ called its \intro{hole}, which can be \intro{filled}
...
difference is that most rules (including the \kl{switch rule} \rsf{s}) can only
...
\kl{s} rule originates, every context is \emph{positive}, in the sense that the
hole $\hole$ never occurs under a negation $\neg$ or left-hand side of an
implication $\limp$; otherwise the rule would need to be restricted to positive
...
\AP
Contrary to \kl{predicate logic} in traditional \kl{proof theory}, \kl{type theories} break the
syntactic hierarchy imposed upon mathematical objects, propositions and proofs,
by giving them a uniform representation as so-called \intro{terms} that can be
assigned a \intro{type}. The assertion that a \kl{term} $t$ has \kl{type} $T$ is usually
...
the \kl{term} $3$ has the \kl{type} $\nats$ of natural numbers, and $1 + 1 = 2 : \Prop$
states that the \kl{term} $1 + 1 = 2$ has the \kl{type} $\Prop$ of propositions.
...
\emph{\kl{first-order}} \kl{predicate logic} (``\reintro{FOL}'' hereafter).
\end{emphpar}
...
be seen as \kl{types}, by having judgments such as $~t : 1 + 1 = 2~$ where $t$
...
type theory} of Martin-Löf (\reintro{MLTT}), which formed the basis for the
...
(\reintro{CoIC}), is also implemented in two leading \kl{proof assistants}:
...
\begin{description}[labelsep=0pt]
  ...
  user needs to prove, and each \kl{goal} is presented in the form of a sequent
  ...
  The transformations performed by \kl{tactics} can be more or less sophisticated.
  ...
  instance, a \kl{goal} $\Gamma\seq A\lor B$ (resp. $\Gamma\seq A\land B$) can be
  turned into either a \kl{goal} $\Gamma\seq A$ or a \kl{goal} $\Gamma\seq B$ (resp. into
  two \kl{goals} $\Gamma\seq A$ and $\Gamma\seq B$), corresponding to the rules
  \kl{\lor R_1} and \kl{\lor R_2} (resp. \kl{\land R}) of \kl{LJ} in their
...
they coined the \intro{Proof-by-Pointing} (\reintro{PbP}) paradigm
...
called \intro{subformula linking} (\reintro{SFL}) or \intro{Proof-by-Linking}
(\reintro{PbL}), where instead of selecting expressions in isolation, one can
...
semantics of \kl{PbP} actions is still based on the \emph{shallow} \kl{inference rules} of
...
\begin{emphpar}
  ...
  the research programme initiated by \kl{PbP} and \kl{PbL}. The aim is to
  \emph{replace} textual proof languages with \emph{gestural} actions performed
  directly upon \emph{\kl{goals}} in a \intro{graphical user interface}
  (\reintro{GUI}). To be as general as possible, we call such a paradigm
  \intro{Proof-by-Action} (\reintro{PbA}).
\end{emphpar}
...
\end{quote}
He even went further by analyzing \kl{icons} into another
...
Interestingly, Peirce held that mathematics relies mostly on \emph{\kl{diagrammatic}}
thinking --- observation of, and experimentation on, \kl{diagrams}
...
of a proof lies in the dynamical construction of a \emph{\kl{diagram}} capturing the
...
\kl{diagrammatic} manipulations, or simply a retranscription of the commentary that
...
to expect that some \kl{diagrammatic} system should exist for it, that can express in
...
arguably the first \kl{diagrammatic} \kl{proof system} in history: the \intro{existential
graphs} (\reintro{EGs}). This will be the subject of \refch{eg}, and the basis
for the development of a \emph{\kl{metaphorical}} \kl{proof system} in \refch{flowers}.
...
We start in \refch{pba} with an introduction to \kl{PbP} and \kl{PbL}, by describing how
...
\kl{GUI} called \kl{Actema}. In particular, \kl{DnD} actions can be seen as a graphical
generalization of both the \texttt{apply} and \texttt{rewrite} \kl{tactics} of
\kl{imperative} proof languages.

In \refch{sfl}, we ground the semantics of \kl{DnD} actions in \kl{deep
inference} \kl{proof theory}, by designing a variant of the subformula linking
...
argue that the use of \kl{direct manipulation} greatly facilitates the handling of
...
\kl{elaboration}/compilation strategy implemented in \texttt{coq-actema}, a
plugin that integrates the \kl{Actema} web app as an interactive \kl{proof view} in \kl{Coq}.
We also discuss its current shortcomings and future directions for improvement,
in particular concerning the question of \kl{proof evolution}.
...
systems share the ability to represent \kl{goals} in two alternative ways: either
\emph{textually} through a standard inductive syntax, or \emph{graphically}
through a \kl{metaphorical} notation well-suited to \kl{direct manipulation}. The first
can be used as a machine representation in the backend of an \kl{ITP}, and the latter
as the substrate for \kl{GUIs} in the frontend.
...
\begin{description}
  ...
  \sidecite{brunnler_deep_2009}, that we reframe as local \kl{rewriting systems} with
  a graphical and topological interpretation. Bubble calculi enable an efficient
  sharing of contexts between \kl{subgoals}, making them well-suited to the
  ...
  introduces the \kl{metaphor} of \emph{bubbles} as a way to \kl{iconically} represent the
  separation and sharing of contexts between different \kl{subgoals}.
  ...
  Peirce for propositional and \kl{first-order} \kl{classical} logic, which have
  been consistently neglected in the \kl{proof theory} literature\sidenote{One
  reason might be that \kl{EGs} have been invented at the end of the
  19\textsuperscript{th} century, \emph{before} the birth of \kl{proof theory}
  as a discipline in the 1920s under the impulse of Hilbert.}. We propose in
  ...
  meta-theoretical results to design the Flower Prover, a prototype of \kl{GUI} in
  the \kl{PbA} paradigm that aims to unify the concepts of \emph{goal} and
  \emph{theory} in a \emph{modal interface}: \kl{goals} correspond to flowers
...
\begin{itemize}
  ...
  This path is for readers only interested in the \kl{proof theory} of subformula
  linking, which is the foundation for the semantics of \kl{DnD} actions on \kl{symbolic}
  formulas.
...
different motivations in mind: namely, the development of a \kl{proof-theoretical}
...
inference, as witnessed by the reference to Schütte's \kl{proof theory} in the above
quote.
\end{digression}
However, we believe our approach is unique in that it emphasizes two aspects:
\begin{itemize}
  \item the use of \emph{\kl{direct manipulation}} on \kl{goals} to perform proof steps
  ...
  representations for the \kl{proof state}, that stray away from traditional \kl{symbolic}
  formulas.
\end{itemize}
...
can integrate modern interfaces for proofs in the \kl{Lean} \kl{proof assistant}, while we
