...
features:
\begin{itemize}
  \item it can only handle goals expressed in many-sorted \kl{intuitionistic}
...
\texttt{coq-actema}, a \kl{Coq} plugin that directly connects \kl{Actema} to a running
...
in a goal-directed manner. This includes other popular PAs such as \kl{Lean} and
...
Coq and \kl{Actema}. \refsec{protocol} describes in more details the main usage
...
broader class of \kl{Coq} goals, as well as prospective solutions to the problem of
...
high-level for our purpose. Typically we need to (partially) translate \kl{Coq} goals
into \kl{iFOL} goals, which can be done much more easily with a direct access to
Coq's low-level API for manipulating \kl(PA){kernel} \kl{terms}.
...
view} for manipulating already-parsed goals. One should think of \kl{Actema}'s actions
...
\begin{description}
  ...
  \kl{tactic} call, and then executed to generate new subgoals that are sent again to
  \kl{Actema}. The \texttt{actema} \kl{tactic} finishes its execution either when:
  \begin{itemize}
    \item all subgoals are proved (in \kl{Actema});
  ...
  recompile it into one big \kl{tactic}, and execute it on the current subgoal.
\end{description}
...
\texttt{actema} \kl{tactic} is to ask the \kl(PA){kernel} for the data of the subgoal $G$
...
string identifier in the \kl{Coq} goal, even though we do not display it in \kl{Actema}'s
...
\begin{remark}
  ...
  map two different \kl{Coq} goals to the same \kl{Actema} goal, because strictly
...
request to \kl{Actema}, whose body contains the translated subgoals $\trgoal{Gs}$. To
...
\begin{itemize}
  ...
  the subgoal under focus in \kl{Actema}. The \process{Plugin} can therefore compile
  $A$ into a \kl{tactic} $\traction{A}$ which is executed on the $n^{\text{th}}$ Coq
  subgoal, giving a new list of subgoals which is sent again to \kl{Actema} for
...
interpreting actions: \kl{Coq}'s goal $G$, its \kl{Actema} translation $\trgoal{G}$, and a
...
of \emph{subformula linking traces}, which is used in the compilation of \kl{DnD}
...
Coq \kl{tactics} \texttt{induction}, \texttt{simpl} and \texttt{red}. The only
...
the embedded \kl{deep inference} semantics to \kl{Coq} goals. Originating from the
...
indeed we want to translate the statements of \kl{Coq} goals into \kl{first-order}
...
inspect the structure of \kl{Coq} goals. Here we use the standard {\ltac} language,
which provides powerful constructs for pattern-matching on goals\sidenote{A
...
unification of the two linked subformulas. Then we formulate in \kl{Coq} two theorems
...
\begin{remark}
  ...
  allows to export subformula linking derivations built with \kl{Profint}'s
...
\refsec{edukera} and \refsec{arith}. But there are many \kl{Coq} goals that cannot be
...
\kl{higher-order} goals, thus work remains mostly on the side of the translation
module for \kl{Actema} written in OCaml.} and the ATD datatype definitions enabling
communication between the two.

One central question in the transition to \kl{higher-order} logic is how unification
...
protocol of \texttt{coq-actema}, by allowing the \process{Backend} of \kl{Actema} to
...
manipulate directly statements in the goal. Now that we are connected to \kl{Coq}, we
...
diagrammatic proofs in category theory to the traditional \kl{proof script} workflow.
...
access to the current goal, but also to the full global environment of \kl{Coq} where
...
\texttt{coq-actema} system, is that it has not been designed with \kl{proof
...
sequence of actions into a proof text. The question of generating \kl{declarative}
proof texts from \kl{imperative} \kl{proof scripts} has already been explored by some
...
Lean \kl{proof assistant} \cite{LeanIPAM}.}. Our hope is that the structure of proofs
in the \kl{PbA} paradigm might be well-suited to the generation of
...
\begin{itemize}
  ...
  \kl{tactics}, because they also take as argument the subformula linking trace,
  ...
  Hopefully, relying on \kl{Coq}'s unification instead of \kl{Actema}'s should mitigate
