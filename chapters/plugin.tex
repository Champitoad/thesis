...
\begin{itemize}
  \itemAP it can only handle \kl{goals} expressed in many-sorted
    \intro{intuitionistic first-order logic} (\reintro{iFOL}), whereas all
...
interacts, written in \kl{HTML}/\intro{CSS}/\kl{JavaScript} with the \intro{Vue.js}
framework \cite{Vuejs}; and a \emph{backend} that implements the proof engine,
written in \intro{OCaml} and compiled to \kl{JavaScript} (\intro{JS}) with
\intro{js\_of\_ocaml} \sidecite{vouillon_bytecode_2014}. The two components
