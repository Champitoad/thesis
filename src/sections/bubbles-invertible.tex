...
\begin{description}
  ...
  \item[\textbf{Implication/Exclusion}]
    ...
    introduction rules of \sys{DBiInt}. Also like in \sys{DBiInt}, we removed the
    contraction rules \rsf{c{-}} and \rsf{c{+}}, which are now merged with these
    ...
    what is done in \sys{DBiInt}, we did not incorporate contractions in other
    $\mathbb{H}$-rules. Thus we cannot simulate exactly all the introduction rules
    of \sys{DBiInt} in \sys{B_{inv}}.
...
\begin{description}
  ...
  \item[\textbf{Linking}] Try to bring together every pair of dual atoms, so
    ...
    want to apply the following \emph{rule of thumb}\sidenote{Pun intended.}:
...
\begin{itemize}
  ...
  immediately destroyed by the \rsf{i{\downarrow}} rule; or
