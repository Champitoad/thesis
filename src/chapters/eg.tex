...
structures \sys{SKS} of Brünnler and Tiu \cite{brunnler_local_2001}. In
\refsec{beta}, we illustrate the original mechanism of \emph{lines of identity}
used by Peirce to handle quantifiers and equality in his \sys{Beta} system. We
end in \refsec{gardens} by showing how to recast lines of identity in a more
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item[\rsf{Dcut{\downarrow}}, \rsf{Dcut{\uparrow}}] We have
...
\begin{definition}[Structure]
  The \emph{structures} $S, T, U$ of \sys{SKS} are generated by the following
  grammar:
  $$S, T, U \Coloneq a \mid \dmdual{a} \mid \top \mid \bot \mid S \land T \mid S
  \lor T$$
\end{definition}
...
implicitly applicable in any \emph{context} $S\hole$ of arbitrary depth, with
...
\begin{description}
  \item[Symmetry] 
    ...
    with $\downarrow$, the dual is the rule with the same name ending with
    $\uparrow$. This corresponds to all rules except the \emph{switch} rule
    ...
    \begin{fact}[Duality]\labfact{eg-duality}
      ...
      \begin{align*}
        ...
        \dmdual{\rsf{Dcut{\downarrow}}} &= \rsf{Dcut{\uparrow}} & \dmdual{\rsf{Dcut{\uparrow}}} &= \rsf{Dcut{\downarrow}}
      \end{align*}
    \end{fact}
  ...
  \item[Locality]
    ...
    the exception of the rules of double-cut (which are best seen as structural
    equations like those of CoS), all rules depend heavily on the creation,
    duplication or erasure of subgraphs of arbitrary size. This is exemplified
    by the derivation of Peirce's law in \reffig{eg-peirce-law}, where not a
    single rule is instantiated on atoms. In fact it is quite hard to see how to
    build a derivation of Peirce's law that performs only local transformations.
    ...
    (and forgetting that they are derived through non-local transformations),
    one gets a fully local subsystem of \sys{Alpha}!
\end{description}
...
translation of each rule of \sys{SKS}:
...
\end{theorem}
\begin{proof}
  By inspection of each rule of \sys{SKS}.
\end{proof}
