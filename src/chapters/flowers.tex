...
through a subtle topological distinction added to the graphical representation,
which reflects in the inductive syntax. In \refsec{Quantification}, we
illustrate quickly the original mechanism of \emph{lines of identity} used by
Peirce to handle quantifiers and equality in his \sys{Beta} system, and show how
to recast it in a more traditional syntax based on binders and variables. In
...
entails the admissibility of the rest of the rules, and as a consequence the
...
formulation of EG by Peirce.}\sidecite{girard-linear-1987}. The full story has
yet to be told, but we hope that our work will constitute one more step towards
...
already the case for modal logic at his time), with so-called \emph{tinctured
...
The most fundamental concept of \sys{Alpha}, is that of the \emph{sheet of
assertion}. It is the space where statements are scribed by the reasoner,
typically a sheet of paper or a blackboard. In a proof assistant, this would
either be the buffer of a text editor holding the theory the user is developing,
or the proof view displaying goals to be proved, depending on who the reasoner
is (the user or the computer, respectively).
...
In Peircean terms, the empty bouquet is the blank sheet of assertion, which can
be interpreted as the symbol of \emph{absolute truth}\sidenote{To be more
precise, it represents literally the \emph{absence} of any assertion. Peirce had
an \emph{externalist} conception of truth, where the assertions made by the
\process{Graphist}, i.e. the person scribing on the sheet, refer to the world
\emph{outside} of the sheet. By not scribing anything, the \process{Graphist}
thus refrains from asserting anything about the world, only assuming implicit
truths. This is illustrated by the following quote from \cite{Roberts+1973},
reminiscent of an interaction between a proof assistant (the \process{Graphist})
and her user (\process{the Interpreter}): \textit{``One of the parties, called
the Graphist, is responsible for scribing the original graphs at the beginning
of the investigation or discussion; the other, called the Interpreter, draws
inferences from these graphs by changing them in accordance with the permissions
of the system. The [...] sheet, before anything is scribed on it, represents
whatever is taken for granted at the outset by the Graphist and
Interpreter.''}.}. Then in our proof construction paradigm, proving a bouquet
amounts to erasing it completely from the sheet of assertion, thus reducing it
to absolute truth.
