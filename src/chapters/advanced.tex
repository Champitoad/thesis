...
\refch{future} for a discussion on a type-theoretical higher-order extension of
Actema.}.
...
point at different subterms appearing in the goal\sidenote{The details of how
Actema's interface enables the designation of subterms through pointing shall be
investigated elsewhere in this thesis (\refch{plugin}?). Basically we want to
mention:
\begin{itemize}
  \item The mapping of \emph{spans} of text in the linear, textual notation to
  the underlying tree structure of terms, exploiting the tree structure of the
  HTML DOM tree (same technique used in the ProofWidgets framework
  \cite{ayers_graphical_2021}). The data required for this mapping should be
  provided by the proof engine when sending the goal to the interface, and
  \cite{ayers_graphical_2021} describes how they had to modify the
  pretty-printer of Lean upstream in order to expose this data. We do not know
  of any API in Coq that exposes this data, at least not in the low-level OCaml
  API we use for the plugin, and a forciori neither in higher-level APIs like
  those of ELPI or SerAPI. Thus we might need similar upstream changes to Coq in
  order to support interactive selection of subterms in pretty-printed form.
  \item When multiple spans overlap at a pointed location, we disambiguate by
  selecting the one \emph{deepest} in the term structure (this would not work in
  the other direction, since this would always select the root term!). This is
  implemented easily in HTML/JavaScript by disabling the mechanism of
  \emph{event bubbling} with the \texttt{stopPropagation()} method of the
  \texttt{Event} interface (TODO: cite Mozilla documentation).
\end{itemize}}
--- either in the context or the conclusion --- and then choose from a list of
available actions taking the selection as argument. In our case we can select
the whole conclusion, and then choose to apply the \action{Unfold} action:
...
\begin{description}
  \item[In \sys{ZF} set theory]
    ...
    in the current context\sidenote{This feature is currently implemented solely
    in the standalone version of Actema, with a toy database of 3 lemmas about
    arithmetic. This is because the standalone version is based on a custom
    proof engine in first-order logic, which cannot reuse libraries of lemmas
    from a real proof assistant.}. An alternative and more precise way of
    retrieving a lemma is to search by \emph{content} of the statement instead
    of searching by name. State-of-the-art proof assistants usually provide
    facilities for this: for instance Coq has a \texttt{Search} command which
    can take \emph{patterns}, i.e. terms with holes or so-called
    \emph{metavariables}, in order to filter out results that do not match the
    given pattern. In Actema, we implemented a novel feature which replaces
    patterns by a selection of subterms in the current goal, similarly to what
    is given as argument to contextual actions like \action{Unfold}. Then the
    system will only look for lemmas which can be used in a DnD action involving
  ...
  annex of \cite{bartzia_proof_nodate}. To perform $\beta$-reduction in Coq,
