...
\begin{description}
  \item[In \sys{ZF} set theory]
    ...
    in the online version of Actema, with a toy database of 3 lemmas about
    arithmetic. This is because the online version is based on a custom proof
    engine in first-order logic, which cannot reuse libraries of lemmas from a
    real proof assistant.}. An alternative and more precise way of retrieving a
    ...
    \emph{patterns}, i.e. terms with holes or metavariables, in order to filter
    out results that do not match the given pattern. In Actema, we implemented a
    novel feature which replaces patterns by a selection of subterms in the
    current goal, similarly to what is given as argument to contextual actions
    like \action{Unfold}. Then the system will only look for lemmas which can be
    used in a DnD action involving precisely the current selection of subterms.
    In this case, selecting the full statement of $(1)$ and searching:
