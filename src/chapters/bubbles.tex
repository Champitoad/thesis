...
\refsec{bubbles-pba} we import back ideas from this bubble calculus to the realm
of GUIs for interactive proof building, analysing their possible impact for UX
improvements.

\todo{Maybe relocate in a new chapter starting from here?}

In \refsec{non-determinism} we motivate our quest for a system where all
...
of) inference rules with premiss $T$ and conclusion $S$\sidenote[][-14cm]{The
...
\begin{itemize}
  ...
  \item the {\rnmsf{i{\downarrow}}} rule corresponds to the most basic DnD
  action between dual occurrences of a formula;
  \item the {\rnmsf{c}} rule was not discussed in \refch{pba}, because DnD
...
\begin{marginfigure}
  ...
  \end{array}
  $$
  \caption{Linkage creation rules for \sys{BJ}}
...
\begin{description}
  ...
  \item[\textbf{\heating}]
    ...
    \begin{itemize}
      ...
      \begin{align*}
        ...
        % \delta &\Coloneq x \coloneq \beta \\
        % \beta &\Coloneq \emptyset \mid t
      \end{align*}
      % In this syntax, $x \coloneq t$ represents a local definition with name $x$
      % and body $t$, and $x \coloneq \emptyset$ a local variable. That is,
      % variables are seen as definitions without a body. 
      ...
      {\rnm{\mathsf{f}\delta}} in \reffig{bubbles-vars}).
      ...
      rules of \sys{BJ} become \emph{analytic}: if $S \step T$ and $S_0 \subsol
      S$, then $S_0 \subsol T$\sidenote{Except the {\rnmsf{i{\downarrow}}} rule
      which will be shown to be admissible in \refsec{bubbles-completeness}, and
      the {\rnm{\delta}} rule of \reffig{bubbles-vars}.}.
    \end{itemize}
  ...
  \item[\textbf{\identity}]
    ...
    {\rnm{\delta}} for adding new definitions (\reffig{bubbles-vars}). Their
