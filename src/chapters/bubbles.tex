...
\todo{Reorganize layout of \reffig{sequent-BJ} to match that of
\reffig{graphical-BJ}}
...
$S$\sidenote{The direction of the arrow is from conclusion to premiss, to stay
...
\todo{Side figure illustrating a sequent-style proof in \sys{BJ}}
...
While the sequent-style presentation of \sys{BJ}'s rules clearly shows its
...
\sys{BJ} are \emph{local}. This enables a more concise and graphical
presentation of the rules in \reffig{graphical-BJ}, where \sys{BJ} is seen as a
multiset rewriting system just like the {\cham} thanks to \refdef{solution}.
...
\end{definition}\todo{add definition of multiset inclusion somewhere?}
...
notions of derivation (\refdef{BJ-deriv}) and proof (\refdef{BJ-proof}) stay
...
metaphor, \sys{BJ} exhibits many features that are interesting both at the
...
\begin{itemize}
  ...
  \item The tree structure of subgoals is immediately apparent in the proof
    ...
    existential graphs \cite{Roberts1973-inactu}. We will come back to this
    soon.}. There are multiple ways to visualize trees on a planar surface, but
