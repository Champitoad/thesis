...
mainstream proof assistant, are presented in \refch{beyond}.
...
The chapter is organized as follows. \refsec{setting} briefly outlines
its logical setting. \refsec{aristote} describes the basic features of
a graphical proof interface based on our principles, and illustrates them with a
famous syllogism from Aristotle. \refsec{clicks} shows how it can
integrate basic proof-by-pointing capabilities. \refsec{newitems} shows
...
\refsec{correctness} introduces the notions of context and polarity, in
order to prove the correctness of our system. \refsec{linkages}
explains how DnD actions are specified by the user interactively, through
schemas called \emph{linkages}. \refsec{action} describes how linkages
...
% Note however that the correctness of the system is ensured by the following
% properties:
% \begin{property}
%   If $A \vdash B \step C$, then $A, C \vdash B$ is provable. If $A \forw B
%   \step C$, then $A, B \vdash C$ is provable.
% \end{property}
...
% As is common, the user interacts with one goal at a time. A first way
% to do so is by clicking on an item of the goal. This will result in a proof
% construction step; that is the goal is replaced by a list of new
% goals. In the case where this list is empty, the goal is solved.
% The precise effect of the click action depends on:
% \begin{itemize}
% \item the nature of the item (hypothesis or conclusion),
% \item the statement of the item (whether it is a conjunction,
%   disjunction, {\em etc}\dots),
% \item possibly the location of the click (on which subformula of the
%   item's statement it occurs).
% \end{itemize}

% The simple cases correspond to the introduction rule (for the
% conclusion) or the elimination rule (for the hypotheses) of the head 
% connective or quantifier of the statement. For instance, clicking at
% the root of a conclusion $A\land B$ will transform the goal into two
% goals of respective conclusions $A$ and $B$.

% We describe this behavior through the following notation:
% $$
% \Gamma\vdash A \/\select{\land}\/ B  \steps [\Gamma\vdash A ;~ \Gamma\vdash
%   B]
% $$
% The squared connective is the location where the click is performed. On
% the right-hand part, we see a list of two sequents, because this
% action replaces the goal by two new subgoals.

% A simple case where the right-hand list is empty is for the
% trivially true proposition $\top$:
% $$
% \Gamma\vdash \select{\top}\steps []
% $$

% \begin{remark}
%   At this stage we should point out that this notation is not meant to
%   lay out the rules underlying the system or its implementation. We
%   just use it to illustrate its behavior on examples, in the way one
%   would do during a demonstration.
%   %% The logical mechanisms which allow
%   %% to achieve this behavior are detailled later on.
% \end{remark}


% The following cases correspond to the introduction rules for
% implication and disjunction. Note that in the latter case, one chooses
% between left and right introduction by clicking on the corresponding
% subformula:
% $$
% \begin{array}{c}
%   \Gamma\vdash A\/ \select{\limp\hbox to -4pt{\vbox to
%   5pt{\vfill}}}\/ B \steps [\Gamma,A\vdash B]\\[6pt]
%   \Gamma\vdash \select{A}\lor B \steps [\Gamma\vdash A]\qquad\qquad
%   \Gamma\vdash A\lor \select{B} \steps [\Gamma\vdash B]
%   \end{array}
% $$
% Symmetrically, here are cases corresponding to elimination rules for
% the propositional connectives:
% $$
% \begin{array}{rcl}
%   \Gamma, A\select{\land}B\vdash C &\steps& [\Gamma,A,B\vdash C]\\
%     \Gamma,A\select{\lor}B\vdash C &\steps& [\Gamma,A\vdash C;~
%       \Gamma,B\vdash C]\\
%     % \Gamma,A\select{\limp}C\vdash C &\steps& [\Gamma,A \limp C\vdash A]\\
%     \Gamma, \select{\bot}\vdash C & \steps & []\\
% \end{array}
% $$
% For quantifiers, we only provide behaviors corresponding to the
% introduction rule for $\forall$ and the elimination rule for
% $\exists$, the dual rules being better taken care of by drag-and-drop
% actions:
% $$
% \begin{array}{rcl}
%   \Gamma,\select{\exists x.}A\vdash B&\steps&[\Gamma,A\vdash B]\\
%   \Gamma\vdash\select{\forall x.}A&\steps&[\Gamma\vdash A]
% \end{array}
% $$
% Note that in both cases we make freshness assumptions for the variable
% $x$.

% We should point out that it is perfectly possible to extend or combine
% these behaviors and allow clicking under nested connectives or
% quantifiers; something like:
% $$\Gamma\vdash (A\lor\select{B})\land C\steps [\Gamma\vdash B;~ 
%   \Gamma\vdash C].$$
...
\section{Correctness}\labsec{correctness}
...
Thus for such actions to be logically correct, we have to make sure that our
...
\begin{lemma}[Contextual correctness]\label{lemma:rewriting-valid-in-context}
  \phantom{a}
  \begin{itemize}
...
Detailed proofs of all the lemmas can be found in \refsec{dnd-correctness}.

\section{Linkages}\labsec{linkages}

In demonstrating correctness, we focused solely on the two \emph{items} involved
...
\todo{ Give the details of how the unification problem is defined, to be used in
  the fully detailed proof of productivity. }
...
\todo{ Put the more general polarity condition from my internship report, and
  ensure it works correctly in the fully detailed proof of productivity (in
  annex). }
...
% Given a context $A\square$ seen as a tree, one can always start from
% the root and traverse the branch of $A\square$ that leads to its hole
% $\square$. We write $\lvar(A\square)$ the list of all variables
% quantified along the way. This list is ordered, the variables closer
% to the root coming first.

% $\lvar(A\square)$ can be seen as the interleaving of two sublists
% $\lvarp(A\square)$ and $\lvarn(A\square)$ of \emph{positively} and
% \emph{negatively unifiable} variables, in the following precise sense:
% $x \in \lvarp(A\square)$ (resp. $x \in \lvarn(A\square))$ iff there
% are contexts $B\square$, $P\square$ and $N\square$ such that
% $A\square$ is either $P\select{\exists x. B\square}$ or
% $N\select{\forall x. B\square}$ (resp. $N\select{\exists x. B\square}$
% or $P\select{\forall x. B\square}$).

% For instance, if $A\square = \forall x. \exists y. (B \land ((\exists x'. \forall
% y'. \square) \limp \forall z. C))$, then we have:
% \begin{mathpar}
%   \lvar(A\square) = [x, y, x', y'] \and
%   \lvarp(A\square) = [y, y'] \and
%   \lvarn(A\square) = [x, x']
% \end{mathpar}


% % The three lists are defined recursively over the structure of $A\square$ by the
% % following equations:$$
% % \begin{array}{c}
% % \begin{array}{rclrcl}
% %   \lvar(\square)\eqb []&  \lvar(\forall x.A\square) \eqb x::\lvar(A\square)\\
% %   \lvar(P(t_1, \dots , t_n)\eqb []& \qquad   \lvar(\exists x.A\square) \eqb x::\lvar(A\square)\\
% % \end{array}\\
% % \begin{array}{rcl}
% %     \lvar(A\square\lor B),  \lvar(B\lor A\square),  \lvar(A\square\land B), \lvar(B\land A\square),&&\\ \lvar(A\square\limp B), \lvar(B\limp A\square)\eqb \lvar(A\square)\\
% %   \lvarn(\square), \lvarp(\square),
% %   \lvarn(P(t_1, \dots , t_n)),\lvarp(P(t_1, \dots , t_n))\eqb []\\
  
% %   \lvarn(\forall x.A\square) = x::\lvarn(A\square)&\qquad&
% %   \lvarn(\exists x.A\square) = \lvarn(A\square)\\
% %     \lvarn(A\square\lor B),  \lvarn(B\lor A\square),  \lvarn(A\square\land B), \lvarn(B\land A\square)\eqb \lvarn(A\square)\\
% %     \lvarn(A\square\limp B)\eqb \lvarp(A\square)\\
% %       \lvarp(\forall x.A\square) \eqb \lvarp(A\square)\\
% %   \lvarp(\exists x.A\square \eqb x::\lvarp(A\square)\\
% %    \lvarp(A\square\lor B),  \lvarp(B\lor A\square),  \lvarp(A\square\land B), \lvarp(B\land A\square)\eqb \lvarp(A\square)\\
% %      \lvarp(A\square\limp B)\eqb \lvarn(A\square)\\
% % \end{array}
% % \end{array}
% % $$
...
%   \begin{itemize}
%   \item $\lvarn(B\square)\cup\lvarp(C\square)$ if $\mathcal{L} = B\select{A}\back C\select{A'}$, and
%   \item $\lvarn(B\square)\cup\lvarn(C\square)$ if $\mathcal{L} = B\select{A}* C\select{A'}$.
%   \end{itemize}
% \end{definition}
...
%   %% $\square$ is defined trivially by $\square[\sigma] = \square$.
% \end{definition}
...
%      which is an interleaving of $\lvar(B\square)$ and $\lvar(C\square)$ such
%      that, given a unifiable variable $x$ in the domain of $\sigma$, all
%      variables occuring in $\sigma(x)$ are placed before $x$ in $l$:
%      $$\forall
%      y\in\FV(\sigma(x))\cap(\lvar(B\square)\cup\lvar(C\square)), y <_l x.$$
%      \end{enumerate}
% \end{definition}
...
\todo{ Put Backward, Forward and Unit rules side by side on one page (removing
  the margin). }
\begin{figure}
...
%%     $A\select{B * C}$,
%%   \item in the first case, $A\square$ is a positive context (that is
%%     $\inv(A\square)$ is even, in the second case, it is a negative
%%     context ($\inv(A\square)$ is odd),
...
%% The \emph{size} of a context $A\square$ is the positive integer
%% $\size{A\square}$ corresponding to the depth at which the hole $\square$ occurs
%% in $A\square$.

%% The \emph{size} of a linkage $\mathcal{L} = B\select{A} \link C\select{A'}$ is
%% defined as $\size{\mathcal{L}} = \size{B\square} + \size{C\square}$.
%% \end{definition}
...
\begin{lemma}[Valid Progress]\label{thm:vprogress}
...
% \end{lemma}
% \begin{proof}
%   By induction over the sizes of $B\square$ and $C\square$...
% \end{proof}

\subsection{Choosing the Best Derivation}\labsec{invert}
