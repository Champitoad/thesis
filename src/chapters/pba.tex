...
In the original formulation of subformula linking \cite{Chaudhuri2013}, a
...
\rnmsf{L\lor_1} first, which leads to the following behavior:
...
this scheme is already present implicitly in the proof of the productivity
theorem in \refsec{vprogress}.}:
\begin{enumerate}
...
\begin{remark}
  ...
  \refsec{invert}. The gap between this theoretical completeness and the
  ...
  implementation of subformula linking in Coq as presented in \refch{beyond},
...
models in an intuitionistic setting. }. Another possibility would be to use some
new representation of proofs that is in-between syntax and semantics, like the
combinatorial proofs of Hughes \cite{Hughes_2006} which are known to be closely
related to deep inference proofs.

Then to state the completeness theorem, we need a notion of truth for our
formulas. As just mentioned, we simply rely on the completeness of cut-free
sequent calculus by performing a proof by simulation, whose details are layed
out in \refsec{app:dnd-completeness}.
...
\cite{dragalin_mathematical_1990}. Dyckhoff notes that such calculi are more
suited to implementation of automated proof search procedures
\cite{dyckhoff_contraction-free_1992}, notably because they allow sharing code
with classical calculi.

Still we do not know of any proof assistant, classical or intuitionistic, that
exposes multi-conclusion sequents as goals in its user interface. One reason is
that most proof/tactic languages are based on the rules of natural deduction,
which use single-conclusion sequent. Another reason is that having one
conclusion removes the need to designate it with an explicit name or number, as
is the case with hypotheses\sidenote{The current trend is to use user-chosen or
automatically generated strings for names as in Coq and Lean, but some provers
like HOL Light ask for the position in the list as an integer to designate a
particular hypothesis.}. And the explicit handling of names in tactic
invokations is known to be tedious and time-consuming, to the point that some
tactic languages like SSReflect have been designed around this problem
\cite{SSR}. Thus having multiple conclusions would only double the effort for no
compelling reason.
...
language processing and voice recognition, one could also imagine instead a
system of vocal commands based on the spelling of formulas. This could be an
important alternative for users with impaired vision and/or motricity. This
remark also extends to click and DnD actions, which could be triggered by voice
commands once the items/subterms they apply to have been selected (also
vocally).}. This opens up the possibility of exposing multiple conclusions in
the interface, with associated graphical proof actions. In the layout of Actema,
this corresponds to having zero or multiple red items instead of a single one.
While we did not implement such an extension, we explore in this section the
theoretical foundations that would be necessary for it to come to fruition.
