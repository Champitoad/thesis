...
application with its own proof engine \sidecite[10em]{Actema:link}. This is
...
constructor encodes a particular \emph{type} of action. An action's type is
...
actions that perform subformula linking as described in \refch{sfl}.
...
\subsection{Implementing deep inference semantics}
...
\begin{itemize}
  ...
  \item Design a new architecture/protocol that supports \emph{continuous,
  incremental document checking} (typically by inserting tactic calls in the
  proof script for each action, maybe coq-lsp can do that?)
