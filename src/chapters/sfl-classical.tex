...
explore in this chapter its design, and the theoretical foundations behind it.
...
\begin{itemize}
  ...
  Gentzen (named {\rnmsf{{\limp}R*c}} in \reffig{multi-imp-intro}), and
  ...
  {\rnmsf{{\limp}R*i}} in \reffig{multi-imp-intro}).
\end{itemize}
...
delayed\sidenote{In \refch{bubbles}, we will see how to overcome this limitation
by using a \emph{nested sequent} system.}. On the other hand the
{\rnmsf{{\limp}R*c}} rule \emph{is} invertible: this is known to allow the
formulation of sequent calculi for classical logic where \emph{all} rules are
invertible, like the \sys{G3c} calculus of \sidecite{negri_structural_2001}. In
...
undecidability in predicate logic as noted by Girard\sidenote{Section 3.3.2 of
The Blind Spot \cite{girard:hal-01322183}.}. This is already what happens in
...
introduction rule of \sys{G3c} ({\rnmsf{\forall L*}} in \reffig{multi-inst}).
...
to the invertible right introduction rule of \sys{G3c} ({\rnmsf{\exists R*}} in
\reffig{multi-inst}).
...
When looking for a proof of a sequent with multiple conclusions, there is this
idea that unless we commit to proving one conclusion and give up on the others
by applying the {\rnmsf{{\limp}R*i}} rule, we can switch freely our focus
between the different conclusions. Thus in a way, we are looking for proofs of
all the conclusions \emph{in parallel}, and we stop as soon as we find
one\sidenote{This is also related to the $\parr$ connective of linear logic
which uses the {\rnmsf{\lor R}} rule of \reffig{multintro}, and whose spelling
``par'' is historically motivated by an understanding of the multiplicative
fragment of \sys{LL} as a logic of parallel computation
\cite{girard-linear-1987}.}. Hence I choose to call this third kind of
interaction between two conclusions \emph{parallel} reasoning. It will be
captured by the $\para$ operator, whose notation is suggestive of the parallel
...
through an instance of the new backward rule {\rnmsf{L{\limp}_1}}. It is
important to note that this rule is only sound \emph{classically}; indeed we can
now come back to the example of \refsec{polarity} and give the following
...
\section{Soundness and Validity}
...
remove the rules {\rnmsf{P{\limp}_1}}, {\rnmsf{P{\limp}_2}} and {\rnmsf{P\forall
s}} from the system, in addition to the {\rnmsf{L{\limp}_1}} rule. Indeed those
...
{\rnmsf{L{\limp}_1}} and {\rnmsf{P{\limp}_1}} in particular has the consequence
...
always terminate on an instance of either the {\rnmsf{id}} rule (backward mode)
...
our paradigm. Then if we trust that the rules of \reffig{parallel} are the only
ones possible to get a correct subformula linking semantics, it seems that
parallel reasoning only makes sense in a classical setting. While we will not be
able to show the uniqueness of our set of rules, in the rest of this chapter we
confirm that it works correctly by extending the results of \refch{sfl} to the
classical, multi-conclusion setting.
...
{\rnmsf{L{\limp}_1}} rule. We also need to add a case for the $\para$ operator
...
{\rnmsf{L{\limp}_1}} is sound classically.
...
\begin{itemize}
  ...
  same polarity.
\end{itemize}
Finally, the proof of Lemma \ref{lemma:rewriting-valid-in-context} is also
extended straightforwardly. We only write the added case for $\para$ in the
...
\section{Productivity and Focusing}

About focusing, we can just remark that some rules that were not invertible in
intuitionistic logic become invertible in classical logic. Therefore the
dynamics of focusing should be different, and it might be interesting to compare
the behaviors of intuitionistic and classical DnD actions on specific examples.
